...
export const BRIDGE_ROUTE_CLASSIFICATION: readonly RouteClassification[] = [
  ...
  { method: 'POST', route: '/api/studio/authoring/start', classification: 'stub-actions', guard: 'spawn-helper',
    reason: 'spawnAgentDispatch (R4-21 T3 BLOCKER-2 fix — staged authoring session) — the session dir, status.json and prompt.md are REAL bookkeeping and still land; only the agent dispatch is skipped with marker + event, exactly as the onboarding-start row above' },

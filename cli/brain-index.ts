...
export type LoadBrainIndexOptions = {
  ...
  cwd?: string;
  /** Project scope. When set, also loads `brain/projects/<scope>/{profile,patterns,antipatterns,decisions}.md` (central, forge-owned brain layout — ADR 035). */
  scope?: string | null;
...
export function loadBrainIndex(opts: LoadBrainIndexOptions = {}): string {
  ...
  if (opts.scope) {
    // Project brains are forge-owned and CENTRAL (ADR 035) at
    // brain/projects/<scope>/<file> — not inside the managed project's own repo.
    const brainRel = `brain/projects/${opts.scope}`;
...
function inventoryProjects(
  ...
  for (const entry of readdirSync(projectsRoot).sort()) {
    ...
    // A project brain counts if it has a profile.md OR a themes/ dir —
    // profile.md is optional (e.g. a themes-only project brain with no
    // profile written yet, like brain/projects/demo-project).
    const profile = join(dir, 'profile.md');
    const hasThemes = existsSync(join(dir, 'themes'));
    if (!existsSync(profile) && !hasThemes) continue;
    out.push({ name: entry, description: readProjectDescription(profile), brainDir: dir });
  }
  return out;
}
...
export function regenerateBrainIndex(
  ...
  const projectListing = projects
    .map((p) => {
      const profilePath = join(p.brainDir, 'profile.md');
      // Link the profile when present; otherwise link the themes/ dir directly
      // (a profile-less, themes-only project brain is still counted + listed).
      const linkTarget = existsSync(profilePath) ? profilePath : join(p.brainDir, 'themes');
      const rel = relative(brainRoot, linkTarget);
...
- **Brain 3 (per project):** project brain is forge-owned and **central** ([ADR 035](../docs/decisions/035-forge-owned-central-artifacts.md)) at \`brain/projects/<name>/\` (**${projectThemeCount} total project-level theme pages** across ${projects.length} active project(s)).

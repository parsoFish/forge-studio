...
async function handleDemoBuilder(
  ...
  // BEFORE any fs call, then resolved via `realpathSync` and required to
  // land inside `ctx.projectsRoot` (the same shape `resolveSafeSessionDir` /
  // `resolveContainedProjectDir` use elsewhere) — this is an IMPROVEMENT over
  // the legacy architect/instructions/project-brain `/start` siblings' own
  // session-dir join (`architectSessionDir` et al., unguarded, filed as
  // `forge-28o`), not a repeat of their gap: the project root itself is
  // realpath-verified before `_onboarding/<sessionId>` is ever joined onto
  // it, and `sessionId` is generated by THIS code (`newArchitectSessionId`),
  ...
  if (method === 'POST' && url === '/api/studio/onboarding/start') {
    try {
      ...
      let realProjectDir: string;
      try {
        realProjectDir = realpathSync(join(ctx.projectsRoot, project));
      } catch {

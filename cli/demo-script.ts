...
export function buildDemoScriptSystemPrompt(_brainCwd?: string): string {
  return [
    ...
    '- `kind: "harness"` — for behaviour that is ONLY measurable at the',
    '  Node/test layer (a headless simulation, a throughput/flow computation',
    '  that runs in unit tests, not the browser). Do NOT re-derive it: REUSE',
    '  the project\'s own existing measurement test. Add a manifest-level',
    '  `harness` block: `command` (run in the worktree root — typically the',
    '  exact command that un-skips & runs that test), optional `env` (a flag',
    '  that un-skips it), `timeoutMs`, and `metrics[]` — each `{ label,',
    '  pattern, kind, unit, deltaTolerancePct }` where `pattern` is a regex',
    '  with ONE capture group read off that test\'s emitted output (read the',
    '  test to see the exact lines it prints). The orchestrator runs the',
    '  command in BOTH trees and renders a before/after metric table + parity',
    '  verdict. Pair a harness checkpoint with a `screenshot` "settled state"',
    '  checkpoint when a still adds visual confirmation.',
...
export function renderDemoScriptUserPrompt(input: DemoScriptUserPromptInput): string {
  return [
    ...
    'title for screenshot/video), `kind` ("screenshot" | "video" |',
    '"harness"), and `caption`. Screenshot labels map to `<label>.png`;',
    'video labels to a harvested `<label>.webm`. A `harness` checkpoint has',
    'NO `test()` and NO capture file — it is driven by the manifest-level',
    '`harness` block (command + metric regexes, run in both trees by the',
    'orchestrator). Use `harness` when the behavioural delta is only',
    'measurable at the Node/test layer and reuse the project\'s OWN existing',
    'measurement test rather than re-deriving it.',

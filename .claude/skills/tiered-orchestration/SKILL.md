...
| `lane-heartbeat-daemon.sh <campaign> <lane> [--job-pattern …] [--worktree-glob …] [--exclude-tree …] [--log-glob …] [--interval N] [--recent-min N] [--stop-file …] [--once]` | the lane's own instrument for its quiet stretches: detached-job process counts + worktree-file recency, called through `heartbeat.sh` only — never a second writer; `lanes.sh launch`/`kill` start and stop one per lane automatically | `scripts/lane-heartbeat-daemon.test.ts` |
...
| `merge-slot.sh <pr> <out>` | the merge protocol: a `merge-state-precheck.sh` call runs FIRST — DIRTY refuses at once (rc 18, no checks are ever run on a DIRTY head), UNKNOWN re-reads a bounded number of times then refuses (rc 19) if it never settles, anything else proceeds — then the slot holds MUTATIONS only (`update-branch`, merge), the CI wait sits outside it (third attempt holds across the wait — a PR cannot lose the race indefinitely); the pin precondition (`PIN_GATE_LOG`) and a cap check on the post-update head gate the merge itself. Requires `MERGE_SLOT_CAMPAIGN_DIR`; `MERGE_SLOT_REPO_FOR_CAPS` names the repo the cap check clones from — neither is guessed. Exit codes are the table at the top of the script (`EXIT_*`) | `scripts/merge-slot.test.ts` |
| `merge-state-precheck.sh <pr>` | is this PR's head even capable of running checks — the bounded `mergeStateStatus` read/retry/classify `merge-slot.sh` calls before touching the PR at all, split out to keep `merge-slot.sh` under the 800-line cap (forge-8vfn.8.1.26); exit 0 proceed, 18 DIRTY, 19 UNKNOWN/unreadable/unrecognised | `scripts/merge-slot.test.ts` (via the doors, same as `ci-terminal.sh`'s own siblings) |
| `pin-precheck.sh <gate-log> <repo> <campaign> [--expect-pin-fail <manifest>:<path>] [--changed-paths-file <file>]` | does this merge's own diff account for every FAILED pin, path-level, re-derived against the tree now — never a per-manifest blanket | `scripts/pin-precheck.test.ts` |

...
- `scripts/lanes.sh peek <lane>` · `send <lane> '<text>'` · `list <campaign>` · `kill <campaign> <lane>` — T1's eyes, relay and retirement. A park-point ruling ("approved: H2 run 1, ceiling $20") is typed into the parked lane, which resumes on its own model budget. A merge-ready lane whose session has ended is still finished by T1 with git/gh.
...
- `scripts/watch-heartbeats.sh <campaign>/heartbeat` is the cron watcher (`*/5 * * * *`): liveness = max(mtime of `<lane>.log`, `<lane>.tmux.log`), so a lane that forgets to heartbeat is still judged by whether its pane moves; ceiling 30 min; `STALL-<lane>` flag for T1 to poll and clear.

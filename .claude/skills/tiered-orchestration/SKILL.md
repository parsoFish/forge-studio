...
- **Confirm a relay by its effect, never by the payload's presence.** The relay hole has a form one step earlier than the bullet above, and it cost a lane 99 minutes parked on a ruling that had already been made. Measured three times on 2026-08-29: `lanes.sh send` printed `sent to forge-m1-d` while the ruling sat in the pane as `[Pasted text #1 +1 lines]`, unsubmitted; an operator's own draft had been sitting in that same pane unsent for an unknown period; and `lanes.sh launch` printed `launched forge-m1-a` over a lane at **0 tokens / $0.00**. In all three the text was plainly visible in the pane and nothing had run. The cause is not delivery — tmux delivers every byte, terminator included — but that a terminator landing in the same read chunk as the payload is taken by the consumer's paste heuristic as pasted *content*. So: write the payload with declared boundaries (`tmux load-buffer` + `paste-buffer -p`), write the terminator separately, and then **watch the input line until the payload leaves it**. A relay that cannot confirm its effect must fail loudly and name the lane; it must never print a success line. The pane showing your text is the *precondition*, never the proof — and `· done`-style pane markers are the same trap one layer up.
...
- `scripts/watch-heartbeats.sh <campaign>/heartbeat` is the cron watcher (`*/5 * * * *`): liveness = max(mtime of `<lane>.log` and every path the lane declares in `<lane>.liveness`, e.g. its cycle's `events.jsonl`); ceiling 30 min, measured from that lane's **own** arm stamp (`.armed-<lane>`) — a single global arm stamp meant one lane closing out cleanly re-armed every other lane and the watcher went blind for 79 minutes; `STALL-<lane>` flag for T1 to poll and clear. **The tmux pane log is not liveness** — an idle Claude TUI redraws its status bar, so the pane never goes quiet (a 2-hour idle lane never flagged, 2026-08-28).

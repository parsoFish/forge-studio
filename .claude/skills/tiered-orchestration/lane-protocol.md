...
1. Every question, park point, stop-rule finding and status change goes to T1, never to a human in this window: SendMessage(to: "$T1") whose first line is one of
   PARK $LANE: <the question, with the artifact path and your default if unanswered>
   ...
   After a PARK or STOP, END YOUR TURN. The ruling arrives as a cross-session message and wakes you. AskUserQuestion is blocked here by a hook. Your subagents cannot ask either — relay their question the same way.

...
- ✅ **End-to-end integration bench** — landed. [`benchmarks/e2e/`](./benchmarks/e2e/) drives the full autonomous-loop cycle (PM → developer-loop → review-Ralph → merge) against a sample initiative with a **human-simulator agent** providing verdicts at each review round. Single fixture (`slugifier-basic`, TS lib). Gate `cycle_completed` + 5 weighted criteria (`merged` 0.40, `converged_within_budget` 0.25, `spec_satisfied` 0.20, `cost_within_budget` 0.10, `no_regression` 0.05; pass threshold 0.7). Smart `gh` shim handles `pr create` + `pr merge` locally (writes `_pr-metadata.json`, fast-forwards initiative branch into main); recorder shims (`vhs`, `npx playwright`) extracted to [`benchmarks/_lib/recorder-shims.ts`](./benchmarks/_lib/recorder-shims.ts) and shared with the per-phase review-loop bench. Closure took 7 iteration passes, **~$10.57 total** (initial runs uncovered: orchestrator merge outcome lying, gh checkout-vs-uncommitted-changes, spec-check glob too narrow, dev-loop wedging on empty `tests/`, dev-loop's leftover PROMPT.md polluting reviewer's workspace). Each fixed-and-rerun. See [`brain/log.md`](./brain/log.md) for the full closure entry. **Future expansion**: more fixtures (Python/bash/web), production CLI (`forge review <id>`), architect-bench-output → e2e-fixture-manifest piping.

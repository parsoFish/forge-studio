 ...
 *     (`apps/forge/tests/integration/bridge-studio-phase-log-raw.test.ts`, tests 1-2).
...
import { resolveNodeLogFromResponse } from '../../lib/flow-node-log.ts';
...
test('a real 200 body maps raw lines through the SHARED deriveLogLine, not a second classifier — kills a hand-rolled kind switch that drifts from run-log-line.ts', () => {
  ...
  // `apps/forge/tests/integration/bridge-studio-phase-log-raw.test.ts` proves the raw endpoint

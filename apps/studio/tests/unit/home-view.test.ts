...
} from '../../lib/home-view.ts';
import type { Flow, Agent, Project, Kb, Run, FlowNode, KbLintSummary, SessionIndexRow } from '../../lib/studio-client.ts';
import type { ProjectAttentionItem } from '../../lib/bridge-client.ts';
...
test('SOURCE: lib/home-view.ts stays pure after buildKbAttention — no fetch(, no /api/, no await, no subscribe(', () => {
  const src = readFileSync(join(dirname(fileURLToPath(import.meta.url)), '..', '..', 'lib', 'home-view.ts'), 'utf8');

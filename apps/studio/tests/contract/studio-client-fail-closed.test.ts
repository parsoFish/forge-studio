...
import * as sc from '../../lib/studio-client';
...
const ROWS: Row[] = [
  ...
  { name: 'fetchProjectStarters', call: () => sc.fetchProjectStarters(), path: '/api/studio/projects/starters', okBody: { appTypes: [{ id: 'cli', label: 'CLI', language: 'TypeScript' }] }, positive: (v) => expect(v).toEqual([{ id: 'cli', label: 'CLI', language: 'TypeScript' }]), mode: 'throws', on404: 'throws' },
  ...
  // W7-B2 — the action-group gate + KB run-history reads (both status-shaped).
  { name: 'fetchKbActiveJob', call: () => sc.fetchKbActiveJob('k'), path: '/api/studio/kbs/k/active-job', okBody: { ok: true, job: null }, positive: (v) => expect((v as { ok: boolean }).ok).toBe(true), mode: 'status-shaped', on404: 'status-shaped' },
  { name: 'fetchKbRuns', call: () => sc.fetchKbRuns('k'), path: '/api/studio/kbs/k/runs', okBody: { ok: true, runs: [] }, positive: (v) => expect((v as { ok: boolean }).ok).toBe(true), mode: 'status-shaped', on404: 'status-shaped' },
...
test('table covers EVERY bridge read exported by studio-client (a new studioGet/studioRead/studioReadOr404/raw-GET export with no row fails here)', () => {
  const source = readFileSync(resolve(__dirname, '..', '..', 'lib', 'studio-client.ts'), 'utf8');

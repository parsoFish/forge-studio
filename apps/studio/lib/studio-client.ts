...
// (description/library/surface/executor/budgets) without breaking its own
// 800-line file-size ratchet (scripts/baselines/file-size.json) — the same
// "pure functions move out, fetch machinery stays here" split
// run-cost-guards.ts used (#842, forge-byh). See agent-wire.ts's own header
// for why the two modules safely reference each other. Every name this file
// used to export directly is re-exported below so no existing importer of
// `Agent` (or its siblings) from `studio-client.ts` has to change.
import {
...
// itself now live in `./agent-wire` (the Agent wire type + parser split —
// see this file's header, above) — re-exported here so every existing
// `from './studio-client'` import of any of these keeps resolving unchanged.
export type { Agent, AgentRuntime, AgentCapabilityDescriptor, ModelTier, AgentFanout, AgentCapability, AgentBudgets };
export { parseCapability, parseFanout, parseAgentCapability };
...
// agents-15 (forge-6gv.5.1): `parseCapability`, `parseFanout`,
// `parseAllowedTiers`/`isModelTier` and `parseAgentDefinition` now live in
// `./agent-wire` (imported above) — this file's own `fetch*` functions below
// call straight through to the imported `parseAgentDefinition`.
...
// agents-15 (forge-6gv.5.1): `AgentCapability` and `parseAgentCapability`
// now live in `./agent-wire` (imported above) — `fetchAgentCapability`
// below calls straight through to the imported `parseAgentCapability`.

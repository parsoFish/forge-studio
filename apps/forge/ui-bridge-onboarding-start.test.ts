...
// session" (1.0.md §3, row S9). S9 beat 11 asserts the onboarding kickoff
// renders `data-model-tier-picker="range"`; it rendered `fixed`, because the
// agent's SKILL.md declared `strategy: fixed` and there was no envelope to
// offer. The fix is the SKILL edit ADR 043's own rationale named ("widening it
// is a SKILL.md edit rather than a UI decision"), and this route is the door
// the widened envelope has to survive: a tier chosen on the form is worth
// nothing if the route drops it, and worse than nothing if the route accepts
// one the agent's own skill forbids.
//
...
// `skills/onboarding-agent/SKILL.md` stops declaring the envelope.
// ---------------------------------------------------------------------------

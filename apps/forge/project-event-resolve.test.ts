...
test('(RED) [F3 #3] a payload repo matching a declared repo resolves to the ENUMERATION id — never the raw payload repo, never the raw dir name', async () => {
  ...
  try {
    // A mixed-case + underscore dir name — under the W7-A4 id rule the
    // enumeration id IS the directory name (normalizeProjectId is the identity
    // for any rule-shaped name), so the resolver must return exactly that.
    ...
    assert.equal(result, 'My_Project', 'the enumeration id is the case-preserving directory name (W7-A4)');

 ...
 *
 * Harness pattern copied from `apps/forge/tests/integration/ui-bridge-agent-run.test.ts`:
 ...
 * and `apps/forge/tests/unit/studio-lint.test.ts`'s `tmpRoot`, both of which seed a real
...
function manifestText(initId: string, cycleId: string, flowId: string): string {
  return [
    ...
    'iteration_budget: 5',
    'cost_budget_usd: 20.0', 'class: code',
    `cycle_id: ${cycleId}`,
    `flow_id: ${flowId}`,
    '---',
    '',
 ...
 * `apps/forge/tests/integration/ui-bridge-demo-generations.test.ts`'s `rawGet` and
 * `apps/forge/tests/integration/dry-bridge.test.ts`'s raw-request idiom — read both before writing

 ...
 * route's own dispatch is covered by `apps/forge/tests/integration/bridge-studio-affordances.test.ts`.
 *
 ...
 *  on disk — mirrors `apps/forge/tests/regression/bridge-studio-kb-seeding-pins.test.ts`'s own
 ...
 * once for GET (`apps/forge/tests/integration/ui-bridge-agent-history.test.ts`'s "D5 (ROUND 4, RAW
 * WIRE)" `rawGet`) and once for POST
 * (`apps/forge/tests/integration/ui-bridge-demo-generations.test.ts`'s `rawPostBody`) — this is the

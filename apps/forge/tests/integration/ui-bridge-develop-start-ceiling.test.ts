 ...
 *
 * Mirrors the `apps/forge/tests/integration/ui-bridge-agent-run-ceiling.test.ts` harness conventions
 ...
 * `apps/forge/tests/integration/ui-bridge-develop-start.test.ts` fixture shape (`pendingManifest`,
 * `flow_id: forge-architect` + `specs` so `hasDecompositionEvidence` passes
 * and the enqueue doesn't short-circuit on `not-planned`).
 *
 * RUN COMMAND:
 *   node --experimental-strip-types --test apps/forge/tests/integration/ui-bridge-develop-start-ceiling.test.ts
 *
...
import { resolveCostCeilingOverride } from '@forge/flows';
import { MAX_KICKOFF_COST_CEILING_USD } from '@forge/kernel';
import { DERIVED_CEILING_MARGIN_SHARE, persistManifestCostCeiling } from '@forge/flows';

 ...
 * package.json. `apps/forge/tests/regression/onboard-born-green.test.ts` states the headline
...
before(async () => {
  ...
  // Probe symlink availability once (mirrors apps/forge/tests/regression/bridge-studio-project-create-containment.test.ts).
...
test('POST /api/studio/projects: an EXISTING package.json in the project root is left byte-unchanged (never-clobber)', async () => {
  ...
  // scaffold under test (same precedent as apps/forge/tests/regression/onboard-born-green.test.ts's
...
test('(RED) POST /api/studio/projects: a symlinked package.json pointing outside the project root is REFUSED, not written through', async (t) => {
  ...
  // the never-clobber test above and apps/forge/tests/regression/onboard-born-green.test.ts's

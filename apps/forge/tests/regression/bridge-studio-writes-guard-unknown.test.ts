 ...
 * inside `composition/guard-unknown` (`@forge/agents`)
 ...
 * `apps/forge/tests/regression/studio-lint-guards-migration.test.ts`, which drives the SAME check
 ...
 * `apps/forge/tests/integration/bridge-studio-write.test.ts`'s "empty purpose → 400 + findings,
 ...
 *
 * Fixture: mirrors `apps/forge/tests/integration/bridge-studio-write.test.ts`'s harness exactly
 ...
 * (`apps/forge/tests/integration/bridge-studio-writes-guards-migration.test.ts`) never needed a
...
function makePutAgentBody(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    // fenced — unrelated to what this file tests (composition.guards).

...
/** Ruling 92 (SEC review) + ruling 92 follow-up (forge-8vfn.8.1.2, operator
 *  item 92): the ORIGINAL mechanism only ever regenerated by replacing an
 *  offending line that wrongly ignores `TRACKED_CONFIG_PATHS` (a blanket
 *  `.forge/`, `.forge/*`, …) — a `.gitignore` that never wrongly ignores
 *  tracked config, but simply predates a `SCRATCH_PATHS` GROWTH (e.g. the
 *  once-canonical 5-line stanza before `.forge/live-evidence/` and
 *  `.forge/preflight.json` joined it), had no offending line to anchor a
 *  replacement to and fell through to `unchanged` — silently leaving the new
 *  entries un-ignored. `missing` is now computed unconditionally and
 *  ALSO drives regeneration: with a rewritable line, it replaces that line
 *  (unchanged shape); with none, it is APPENDED to the file (there is
 *  nothing to remove — the fix is purely additive). Reads through
...
function computeGitignoreDrift(projectDir: string): GitignoreDrift {
  ...
  if (rewritableLines.size > 0) {
    ...
    // No wrongly-ignored tracked-config line to replace — the stanza's own
    // shape is fine, it is just missing newer SCRATCH_PATHS entries. Append
    // after the last line, preserving a trailing blank line (from a
    // trailing newline in the source file) so the result still ends in one.

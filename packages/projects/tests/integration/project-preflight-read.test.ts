 ...
 *   - `apps/forge/tests/contract/id-rule.test.ts`'s "per-project routes: every :id route the
 *     walkthrough saw 404/400 now resolves ... / exact match — the
 *     lowercased id is unknown (404)" family, applied to preflight and
 *     repo-status.
 *   - `apps/forge/tests/integration/bridge-studio-preflight-resolve.test.ts`'s
 *     "GET preflight/fix-agent/:runId returns a state" test.
 *   - `apps/forge/tests/integration/bridge-studio-save-repo.test.ts`'s repo-status coverage.
 */
 ...
 *  (mirrors apps/forge/tests/integration/bridge-studio-preflight-resolve.test.ts's fixture). The exact

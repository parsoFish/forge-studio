/**
 * demo-record.ts — the forge-owned demo recorder (D8 deliverable 2).
 *
...
async function launchRecording(): Promise<Recording> {
  ...
  try {
    ...
    throw new DemoRecordError('demo-record: chromium failed to launch', err);
  }
  try {
    ...
    rmSync(videoDir, { recursive: true, force: true });
    throw new DemoRecordError('demo-record: failed to open a recording context', err);
  }
}

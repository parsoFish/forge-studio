...
export type DemoAgentPipelineResult =
  ...
  | {
      ...
      reason:
        ...
        | 'tooling-unavailable'
        | 'capture-failed'
        // bead forge-8vfn.17: a capture that exited without stamping is a
        // capture FAILURE, and must not share a reason code with tampering —
        // an operator triages the two completely differently.
        | 'capture-not-stamped'
        | 'nonce-mismatch'
        | 'scope-violation'
        | 'budget-exhausted'
...
export async function runDemoAgentPipeline(
  ...
  for (let attempt = 1; attempt <= MAX_AUTHOR_ATTEMPTS; attempt += 1) {
    ...
    if (demoJsonWantsCapture(demoJsonAbs)) {
      ...
      const verdict = judgeCaptureNonce(nonce, stamped);
      if (!verdict.ok) {
        emit('demo.capture', { capture_ok: true, nonce_match: false, nonce_verdict: verdict.reason, capture_nonce: nonce, stamped_nonce: stamped ?? null }, { event_type: 'error' });

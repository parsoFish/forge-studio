 ...
 * carved out of `apps/forge/ui-bridge.ts`.
 *
...
export async function handleKickoffRoutes(
  ...
  // `resolveContainedProjectDir` (`packages/projects/contract-stages.ts`) the sibling
  ...
  // `apps/forge/ui-bridge-onboarding-start.test.ts` AT-7/AT-8 for the live-execution
  ...
  if (method === 'POST' && url === '/api/studio/onboarding/start') {
    try {
      ...
      // apps/forge/ui-bridge-onboarding-start.test.ts is exactly that check), the

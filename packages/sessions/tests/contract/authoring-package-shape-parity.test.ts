 ...
 *
 * This rule is hand-copied on BOTH sides of the server <-> forge-ui
 * boundary (forge-ui never imports the server half at runtime — see
 ...
 * (`packages/library/bridge-studio-authoring.ts`) but never to the GENERIC verdict
 * route's own copy of the rule (`packages/sessions/kinds/authoring.ts`'s
 ...
 * array (`packages/sessions/kinds/authoring.ts`'s exported
 ...
 * imports the server half at runtime; only this NODE-SIDE TEST crosses it).
 *
 * RUN: node --experimental-strip-types --test packages/sessions/tests/contract/authoring-package-shape-parity.test.ts
 */

import { test } from 'node:test';
import assert from 'node:assert/strict';

import { AUTHORING_PACKAGE_SHAPES as SERVER_SHAPES } from '../../kinds/authoring.ts';
...
test('the server-side and client-side authoring-package-shape arrays are byte-for-byte identical', () => {
  assert.deepEqual(
    ...
    `apps/studio/lib/authoring-package-shape.ts's AUTHORING_PACKAGE_SHAPES has drifted from ` +
      `packages/sessions/kinds/authoring.ts's own copy — a shape was added/changed on one side ` +

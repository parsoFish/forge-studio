 ...
 * and write the filesystem, which contracts may never do.
 *
 * Why it moved: a manifest is the shared shape flows, factory and sessions all
 * read, not a flows concept the others borrow. With the type here,
 * `packages/sessions` naming it was a layer-order violation that injecting the
 * functions could not close, because the boundary check counts type-only
 * imports too.
 */
export type { ManifestPhase, InitiativeOrigin, InitiativeManifest } from '@forge/contracts/manifest-types.ts';

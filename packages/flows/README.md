...
| work items and their worktrees | `DEV_WORK_ITEM_ID_PATTERN` · `WORK_ITEM_FILE_PATTERN` · `createMergeQueue` · `createWiWorktree` · `enqueueDemoFixWorkItems` · `enqueueGateFixWorkItems` · `gateRequiredPaths` · `mergeAndPublish` · `mergeWiIntoCycle` · `parseWorkItem` · `readWorkItemsFromDir` · `removeWiWorktree` · `reviewCapExhaustedPath` · `runConcurrentDispatch` · `serializeWorkItem` · `topologicalOrder` · `validateWorkItemSet` · `wiWorktreePath` · `writeMergeGateConfigErrorMarker` · `writeReviewCapExhaustedMarker` · `writeWorkItem` · `writeWorkItemStatus` |
...
| artifacts, demo paths and budgets | `CostCeilingError` · `DEMO_JSON_BASENAME` · `DEMO_MD_BASENAME` · `WedgeDetector` · `WedgeKillError` · `demoFixSpecJsonPath` · `reviewFindingsJsonPath` · `validateDemoFixSpec` · `validateReviewFindings` · `worktreeDemoDir` · `worktreeDemoJsonPath` · `worktreeDemoRelDir` · `writeDemoFixSpecJson` · `writeReleaseJson` · `writeReviewFindingsJson` |

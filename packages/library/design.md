...
`runHookScript`/`runHookScriptAsync` used to spawn `bash` on `scriptPath` directly — the approval fingerprint (`hookRunState`), `prepareHookRun`'s own `readFileSync`, and that spawn were three separate opens of the same path, so the bytes could change in place between the gate's read and exec (forge-8vfn.8.3.2). The fix: `prepareHookRun` hashes its own read with `hashHookScript` (the ledger's own hasher, reused not reimplemented) against the ledger's stored `scriptHash` and refuses on mismatch, then copies those exact bytes to a fresh, private, read-only file — `mkdtempSync` under `os.tmpdir()` (a fresh, unpredictable name only this process ever learns), `chmodSync` 0700 on the dir (a guarantee, not an assumption about umask), `writeFileSync(..., { mode: 0o500, flag: 'wx' })` on the file (`wx` = `O_CREAT|O_EXCL`, refuses to write through anything already at that path; `0o500` makes the file immutable, including to this same process, the instant the write returns). Both tails then run `bash -c 'source "$<env var>"' <realScriptPath>` against that private copy — never the original path again, and the private path travels by an env var set directly on the spawn's own env object (never a positional parameter, so a hook's own `$1..` stay unset as before).

Two mechanisms considered and rejected, both confirmed empirically rather than assumed:
...
- **`BASH_ARGV0=<real> bash <privateCopyPath>`**, to set `$0` while spawning the private copy directly. Wrong: `BASH_ARGV0` is a FALLBACK bash consults ONLY when it has no other source for `$0` — an explicit script argument always wins, so this silently sets `$0` to the private copy's own path, breaking `$(dirname "$0")`-relative sibling sourcing (PIN D, `hook-runtime.test.ts`). The `-c 'source "$<var>"' <realScriptPath>` form instead gives bash an EXPLICIT name argument for `$0` while `source` reads the private copy in the same shell/stdio — stdin untouched, `$0` correct.
...
Pinned by three TOCTOU cases in `hook-runtime-toctou.test.ts`: content swapped between the gate's read and `prepareHookRun`'s own read (refuses, fingerprint mismatch); content swapped AFTER that read (the approved bytes still run — nothing left to refuse against, and there is no later reopen of the original path to pick up the swap); and a hook's own `read` seeing an empty line rather than the next line of its own script source.

...
### Hook exec: verify once, copy once, never reopen the path

`runHookScript`/`runHookScriptAsync` used to spawn `bash` on `scriptPath` directly — the approval fingerprint (`hookRunState`), `prepareHookRun`'s own `readFileSync`, and that spawn were three separate opens of the same path, so the bytes could change in place between the gate's read and exec (forge-8vfn.8.3.2). The first fix re-verified and copied the ENTRY script alone: `mkdtempSync` under `os.tmpdir()` (a fresh, unpredictable name only this process ever learns), `chmodSync` 0700 on the dir, `writeFileSync(..., { mode: 0o500, flag: 'wx' })` on a single private file (`wx` = `O_CREAT|O_EXCL`, refuses to write through anything already at that path; `0o500` makes the file immutable, including to this same process, the instant the write returns), then `bash -c 'source "$<env var>"' <realScriptPath>` against that private copy, with the private path travelling by an env var set directly on the spawn's own env object so `$0` stayed the REAL script path (never a positional parameter, so a hook's own `$1..` stayed unset).

**That `$0`-faking was itself the residual hole (M7-C PKG, forge-8vfn.8.3.6).** A hook's sibling — sourced via `. "$(dirname "$0")/lib.sh"` (PIN D, `hook-runtime.test.ts`) — was never copied, so `$(dirname "$0")` kept resolving to the REAL, mutable package directory for the whole run: the entry script was pinned, but anything it sourced was read live, and a sibling swapped after the approval gate ran executed unverified. The fix widens the copy to the WHOLE package. `prepareHookRun` re-reads every file through `readHookPackage` (the same guarded, capped, symlink-refusing walk the approval ledger's own `snapshotHookPackage` uses — one walk, not a second implementation), re-hashes it with `hashHookPackage` (canonicalizing `hook.yaml`'s body identically to the ledger, via `canonicalHookYamlBody`) and compares against the ledger's stored `packageHash`, refusing on mismatch. `packageHash` strictly subsumes the old entry-only `scriptHash` check (`hashHookPackage`'s own doc comment), so this single whole-package re-verify replaces it rather than running beside it. `writePrivatePackageCopy` then writes EVERY verified file into the private tree, preserving the package's own relative layout (`mkdirSync` + `chmodSync` 0700 per directory level, `writeFileSync(..., { mode: file.executable ? 0o500 : 0o400, flag: 'wx' })` per file — the mode mirrors the package's own recorded executable bit, the same bit `hashHookPackage` folds into its fingerprint, so the copy's permissions never diverge from what was approved).

With every sibling now living alongside the entry script in the private copy, the `$0`-faking machinery is no longer needed at all: both tails exec `bash <privateEntryPath>` directly. Plain `bash <path>` sets `$0` to that path by default, so `$(dirname "$0")` resolves INSIDE the pinned copy — exactly where the pinned siblings are. This drops the `FORGE_HOOK_VERIFIED_SCRIPT_PATH` env var, the `-c`, and the `source` builtin entirely; the invocation is simpler than the entry-only design specifically because faking `$0` is no longer necessary once the thing `$0`'s directory points at is itself pinned.

Two mechanisms were considered and rejected for the ENTRY-ONLY design, both confirmed empirically rather than assumed, and still the reason exec is a plain script-argument invocation rather than either of these:

- **`bash -s` fed the script on stdin.** `-s` reads its own commands incrementally from that same stream while executing them, so a hook that itself reads stdin (`read`, `cat`/`jq` with no file arg, `while read …`) consumes the SCRIPT'S OWN remaining source lines as data instead of an empty pipe — `bash -xs` tracing showed the line after a `read` never executes at all. Silent, and unrelated to the TOCTOU this closes.
- **`bash -c <scriptBody>`** (the bytes as the argv command string, no private-file copy at all) avoids stdin entirely, but a script can be as large as `MAX_PACKAGE_BYTES` (5 MiB) — past this host's measured 2 MiB `ARG_MAX` — so an approved, in-cap hook could fail to spawn (E2BIG).

A third rejection applied only to the entry-only design and no longer arises: **`BASH_ARGV0=<real> bash <privateCopyPath>`**, to fake `$0` back to the real path while spawning the private copy directly, was wrong because `BASH_ARGV0` is a FALLBACK bash consults ONLY when it has no other source for `$0` — an explicit script argument always wins, so it silently set `$0` to the private copy's own path instead, breaking sibling sourcing. The whole-package copy sidesteps the problem this was trying (and failing) to solve: `$0` now SHOULD be the private copy's own path, so bash's default behaviour for `bash <path>` is exactly correct with no override needed.

Cleanup (`rmSync` of the private temp dir) runs from a `finally` in both tails, on every exit path, and never throws from that `finally` — a cleanup failure would otherwise override whatever the `try` was about to return or throw, masking a genuine result; it is logged as its own event instead.

Pinned by three TOCTOU cases in `hook-runtime-toctou.test.ts` (entry script; unchanged by the M7-C PKG widening — see the call-count argument in that file's own header): content swapped between the gate's read and `prepareHookRun`'s own read (refuses, fingerprint mismatch); content swapped AFTER that read (the approved bytes still run — nothing left to refuse against, and there is no later reopen of the original path to pick up the swap); and a hook's own `read` seeing an empty line rather than the next line of its own script source. `hook-runtime-package-pin.test.ts` pins the same two shapes for a SIBLING file: swapped before `prepareHookRun`'s own whole-package re-verify (refused) and swapped after it (the approved sibling content still runs, from the private copy — proven by reverting the copy step to entry-only, which reds exactly this door and no other).

**`cwd` is the one thing this fix cannot pin (M7-C PKG follow-up, forge-8vfn.8.3.6).** The private copy protects any reference rooted at `$0`; it does nothing for one resolved against the CURRENT WORKING DIRECTORY, and `cwd` stays the real, mutable `studio/hooks/<id>/` directory on purpose — `pre-pr-security-review`'s own shipped script runs bare `git`, which needs a real repository checkout under it, so moving `cwd` into the private copy is not an option. A hook that sources a sibling via a cwd-relative path (`. ./lib.sh`, `source lib.sh`) therefore still reads live, unverified bytes for that one line even after the whole-package fingerprint passed. There is no runtime fix for this shape, so it is closed at APPROVAL time instead: `hook-scan.ts`'s `scanUnpinnedSource` flags any `source`/`.` argument that is not textually rooted at `$(dirname "$0")` / `${0%/*}` (and is not itself absolute) as a `critical`, `unpinned-source` finding, which blocks the verdict exactly like `obfuscation` or a dangerous `file-read` — an unpinned sibling defeats the fingerprint the same way a secret-shaped grant defeats the credential fence, so it gates the same way. Both shipped hooks (`post-merge-brain-ingest`, `pre-pr-security-review`) were checked against it and neither sources anything, so neither trips it.

 ...
 * apps/studio/lib/session-lifecycle-client.ts's and apps/studio/lib/session-
 ...
 *
 * RUN: node --experimental-strip-types --test packages/library/tests/contract/tool-fence-required-names-parity.test.ts
 */
...
test('the lint-side and client-side TOOL_FENCE_REQUIRED_NAMES arrays are byte-for-byte identical', () => {
  assert.deepEqual(
    CLIENT_NAMES,
    LINT_NAMES,
    `apps/studio/lib/tool-fence-required-names.ts's TOOL_FENCE_REQUIRED_NAMES has drifted from ` +

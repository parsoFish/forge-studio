...
/** `listTsFiles`'s complement: the TEST files, for locating a guard by its own
 *  content rather than by a path that every carve invalidates (§15.93). */
...
describe('hook dispatch covers every spawn site (the enumeration ratchet)', () => {
  ...
  it('the ratchet\'s own premise holds: exactly one file may import the SDK query directly, and it is the pinned seam', () => {
    ...
    // The lock is located by CONTENT, never by a hardcoded path. §15.93: a
    // guard self-test that names a production path is coupled to every carve,
    // and this assertion proved it — the exit-row-6 re-bucket moved the lock
    // into `tests/contract/` and this line went ENOENT. Failing loudly was the
    // good outcome; re-pointing it at the new path would only re-arm the same
    // trap for the next move. Finding it by its own defining symbol also says
    // something the path never did: that there is EXACTLY ONE such lock. Two
    // would mean the premise is being asserted twice and could drift; zero
    // means it is gone, which is the thing this check exists to notice.
    // Keyed on the DEFINING form, not the bare token — the first version of
    // this check searched for the token and counted TWO locks, because the
    // second one was this file, quoting the token in order to look for it. A
    // scanner satisfied by the scanner is the §15.39 shape one door along, and
    // it inverted the result: the check passed when the lock was renamed away
    // and failed while it was present. The regex's own source text does not
    // match the regex, so this file cannot find itself.
    const DEFINES_LOCK = /const\s+WRAPPER_RELATIVE_PATH\s*=/;
    const locks = listTestFiles(join(FORGE_ROOT, 'packages/agents')).filter((f) =>
      DEFINES_LOCK.test(readFileSync(f, 'utf8')),
    );
    assert.equal(
      locks.length,
      1,
      `the structural lock this ratchet depends on must still exist, exactly once — found ${locks.length}: ${locks.map((f) => relative(FORGE_ROOT, f)).join(', ') || '(none)'}`,
    );
    const direct: string[] = [];

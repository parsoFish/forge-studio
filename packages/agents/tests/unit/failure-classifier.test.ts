/**
 * `classifyCycleFailure` — the rule table, unit-tested one rule at a time.
 *
 * Fix B: `unifier.failed` is phase-agnostic. The classifier drops the
 * `phase === 'developer-loop'` guard from the unifier.failed check — the
 * message is unifier-specific and the retag to `phase: 'unifier'` must not
 * break classification.
 *
 * SPLIT FROM a 1,235-line file, at the seam the file itself declared. Every
 * rule below is a pure call/return assertion on one module, which is what
 * `unit/` means (tests/README.md). The W8-F3 cluster — deterministic failures,
 * the blob-scan precision evidence, the archived-corpus replays — is a named
 * defect pinned against its own regression, so it moved to
 * `regression/failure-classifier.w8f3.test.ts` with the file-system and
 * scheduler machinery it needs; `matchesRateLimitSignature`'s event-type
 * scoping is already `regression/failure-classifier.rate-limit.test.ts`, and
 * `classifyCrash` is already `unit/failure-classifier.crash.test.ts`. The
 * split retires this file's `scripts/baselines/file-size.json` row rather than
 * re-keying it: a move cannot retire an exemption, only a split can.
 *
 * The rules kept here, in the order the file states them: the phase-agnostic
 * unifier terminal and its attempt windowing · N10 gate-timeout as ENVIRONMENT
 * · G4 the uwi loop cap · G10 transient golangci-lint contention · N9
 * rate-limit death · N7 the `environment` flag itself · 2.11 the PM turn
 * economy · R4-10-F1 the successor nodes' delivery gates · the PM structural
 * terminals · W8-A2 CostCeilingError.
 */

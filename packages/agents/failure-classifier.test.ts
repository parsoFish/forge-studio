...
// ---------------------------------------------------------------------------
// forge-rofi — the N9 every-event-type scan is scoped to `log`.
//
// MEASURED against the whole archived corpus before a line was changed
// (/home/parso/forge/_logs, 356 events.jsonl, 1,126,655 events): the four
// high-specificity signatures appear 14 times, and ALL FOURTEEN are
// `event_type: 'log'`. Nothing else has ever carried one — not the 36,578
// `tool_use` events, not the 6,117 `file_change`, not the 1,024,605
// `agent_heartbeat`. So `log` is the channel, and the other nine event types
// were being scanned for a string they have never carried.
//
// The structured `metadata.rate_limited` flag is NOT scoped: the dev-loop
// stamps it on a `ralph.end` (an `end` event), and it is structured data
// rather than text, so it cannot be tripped by prose in the first place.
// ---------------------------------------------------------------------------

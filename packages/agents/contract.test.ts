 ...
 *
 * T1 ruling 31. `packages/agents/index.ts` was `export {}` for the whole
 * campaign, which made every claim about "the public API of this package"
 * unfalsifiable — an importer following the README got `undefined`, and no
 * test could tell.
 *
 ...
 * regressed to `export {}` alongside a README that was emptied to match would
 * be two near-empty sets agreeing with each other, and this file would report
 * success.
 */
...
const exported = (): string[] => Object.keys(agents).sort();
...
test('contract: the README advertises a non-trivial API — a list that shrank to nothing would make the assertion above vacuous', () => {
  ...
  assert.ok(values.length >= 50,
    `README.md advertises only ${values.length} value exports. The measured external surface of this package is 57 ` +
    'values across 25 module paths; a list this short means the README was emptied rather than the package, and the ' +
    'equality assertion above would then be comparing two near-empty sets and reporting success.');
  assert.ok(types.length >= 6, `README.md advertises only ${types.length} types; expected the 7 the package exports.`);
});

test('contract: SPEC.md §1 — the package owns ONE seam, and the door says so', () => {
  const text = readFileSync(README, 'utf8');
  assert.match(text, /run one agent/i,
    'README.md must state the seam this package owns (SPEC.md §1: "run one agent"); a door that does not name its ' +
    'seam cannot be checked against one.');
  assert.match(text, /rank 3/i,
    'README.md must state the package\'s rank, because every injection in this package exists BECAUSE of it — the ' +
    'route deps, the band deps and the usage index are all consequences of what rank 3 may not import.');
});

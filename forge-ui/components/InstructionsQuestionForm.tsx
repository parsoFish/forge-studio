...
import { answerInstructions, type ArchitectQuestion } from '@/lib/bridge-client';

/**
 * Stage A — renders the instructions-creator's file-handoff interview round
 * (`questions.json`, the same `StructuredQuestion` shape the architect uses) as
 * one option-group per question. Every question must be answered before submit;
 * answers POST back to `/api/instructions/answer`, which appends the round and
 * spawns the next runner turn.
 *
 * Mirrors {@link ArchitectQuestionForm} exactly (free-text override included);
 * only the submit fn (`answerInstructions`) and the outer section name differ.
 * The per-question / per-option data-* anchors are kept identical so the same
 * harness can drive both forms.
 */
export function InstructionsQuestionForm({
  project,
  sessionId,
  round,
  questions,
  ...
  async function onSubmit(): Promise<void> {
    ...
    try {
      ...
      const res = await answerInstructions({ project, sessionId, answers });
  ...
  return (
    <div
      data-section="instructions-interview"
    ...
    >
      <div style={{ fontSize: 13, fontWeight: 600, color: '#e6edf3', marginBottom: 12 }}>
        Instructions interview — round {round}
      </div>
      {questions.map((q, i) => {

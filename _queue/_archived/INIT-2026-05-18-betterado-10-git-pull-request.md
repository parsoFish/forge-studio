---
initiative_id: INIT-2026-05-18-betterado-10-git-pull-request
...
features:
  - feature_id: FEAT-1
    title: betterado_git_pull_request resource + mock tests
    depends_on: []
  - feature_id: FEAT-2
    title: betterado_git_pull_request_thread / comment + mock tests
    depends_on:
      - FEAT-1
  - feature_id: FEAT-3
    title: betterado_git_pull_request_label + mock tests
    depends_on:
      - FEAT-1
  - feature_id: FEAT-4
    title: Docs + examples
...
# betterado git pull requests

## Why

Git collaboration is severely partial: repositories/branches/files exist but
pull requests, threads and labels (`POST /git/repositories/{id}/pullrequests`
and friends) are absent. `git_sdk_mock` exists.

## Scope

- `betterado_git_pull_request`: repository_id, source/target ref, title,
  description, draft, auto_complete.
- `betterado_git_pull_request_thread` (+ comment): pull_request_id, content,
  status, thread context.
- `betterado_git_pull_request_label`.
...
- FEAT-1 — **Given** a gomock git client returning a fixture
  `GitPullRequest`, **when** CRUD runs, **then** SDK args match and state
  round-trips; tests pass; name registered; `go build ./...` exits 0.
- FEAT-2/3 — thread/comment/label create payloads match under mock and
  round-trip with no drift.

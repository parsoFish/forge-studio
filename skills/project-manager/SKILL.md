...
- `<worktree>/.forge/work-items/WI-<n>[<letter>].md` — one file per work item, frontmatter + spec body. Schema locked in [ADR 015](../../docs/decisions/015-work-item-format.md) (id regex `/^WI-\d+[a-z]?$/`; the optional single lowercase letter is the **split suffix** — see "Splitting a WI" below). **Written incrementally, one file per decided WI** (see Turn economy).
...
3. **Decompose the initiative body's GWT ACs directly into atomic outcome-sized work items.** The body is your single source of intent — no `features[]` list. **The initiative TITLE is a filing label, NOT the spec — if title and body disagree, the body wins** (a past cycle hallucinated off a "release-folder" title and built unrelated release-NOTES markdown). **Before drafting any WI, restate the target in one line** — the concrete resource/file/module the body asks for and where it lives in this project's source tree. Put that line in the first WI body. Every WI's `files_in_scope` must sit under that source tree. Each GWT block in the body MUST be exercised by ≥1 WI's `quality_gate_cmd`. Do not invent work outside the body's ACs. For each WI:
   ...
   **Sharp-gate patterns (mirror these):**
   ...
   - **A change to a live-resource's schema / config surface MUST be gated on the LIVE acceptance test, not an offline unit test.** Offline gates (and the whole CI gate, which strips the live trigger) cannot catch live-only failures — e.g. a Terraform `ConfigMode: SchemaConfigModeAttr` conversion compiles and unit-passes but makes every nested attribute *required* at `apply` time, which only the live `apply→read→destroy` test surfaces. When the project declares `testProcess.acceptance` (`.forge/project.json`), the WI that proves a resource/schema change must set `quality_gate_cmd` to the live acceptance command (e.g. `['go','test','-tags','all','-run','TestAcc<Name>','./…/acceptancetests/']`); it runs live because the serve env carries the live trigger (`TF_ACC` etc.), and `testProcess.acceptance.requiresEnv` errors fast if the env is missing rather than false-passing. Writing the acceptance test but gating the WI on an offline run is the trap — the live bug then slips to the PR unproven.
...
5. **Splitting a WI — a split MUST chain.** When one WI is too large, split it into `WI-<n>a`, `WI-<n>b`, … (a single lowercase suffix; `WI-4a1` / `WI-4-a` / `WI-4A` are invalid ids and fail the pass). The halves came out of one unit of work, so they **share the files that unit touched** — which is exactly the hidden-coupling shape. Therefore: **every split sibling after the first `depends_on` the one before it** (`WI-4b depends_on WI-4a`, `WI-4c depends_on WI-4b`), and each of them inherits the original WI's own `depends_on` list. Serialising the halves is what makes the shared file safe; a fan-out of unchained siblings off a common parent is a guaranteed merge conflict and `detectHiddenCoupling()` REJECTS it. This is not hypothetical: `INIT-2026-08-14-betterado-gap-registry` emitted `WI-4a` and `WI-4b` both depending only on `WI-1`, and failed three times at ~$2.40 each.
6. **File-scope discipline.** If two WIs edit the same file: (a) split the file by concern, (b) merge WIs, or (c) add a `depends_on` edge. Two WIs sharing a file with no edge is a guaranteed merge conflict; `detectHiddenCoupling()` REJECTS the cycle (the 2026-05-23 betterado dogfood failed this way: WI-1 + WI-5 shared a schema file with no edge → cycle failed at PM phase, $1.54 wasted).
7. Write the dependency graph as `_graph.md` (mermaid `graph TD`; edges must agree exactly with the union of all `depends_on` lists).
8. **Self-check — MANDATORY.** Run the per-WI checks AS you write each file (not batched at the end — a capped run must leave valid WIs behind), then walk the set-level checks once, fixing with `Edit`, before stopping:

   **Per work item — frontmatter completeness:**
   - `work_item_id` (matches `/^WI-\d+[a-z]?$/` and filename), `initiative_id`, `status: pending`
   - if the id carries a split suffix, the **chaining edge is present** — see "Splitting a WI"

...
**Writes:** `_logs/<id>/user-feedback.md` — answer each numbered question, then add free-form feedback. Stage 3 distils this into `retro.md` Section 2 (answers) + Section 3 (free-form). Contract: `packages/stations/phases/reflector-binding.ts`, `packages/stations/phases/reflector.ts`. If the file is absent when the reflector runs, record `_(no feedback supplied this cycle)_` and continue.

...
- **Never fabricate a visual for a non-visual change.** An internal refactor, an
  ...
  not invent an on-screen overlay or re-shoot an unrelated screen.

## How forge runs a demo

1. **Author `demo/<initiative-id>/demo.json`** to the contract above.
...
| **Standard** (a feature or fix with a clear behavioural delta) | Full checkpoints grounded in ACs; metrics if measurable; media only if `demo.shape: "browser"`. |
| **Large / multi-feature** | A checkpoint per distinct delta (still tight per checkpoint); media for the visual ones; a metrics table for harness shapes. |
...
- **`browser`** — a rendered UI. Author checkpoints, then `forge demo capture` to
  back-fill before/after screenshots/video. `kind: "screenshot"` for a settled UI
  state; `kind: "video"` for time-dependent behaviour (a still cannot prove
  parity of a moving system). Requires `preview_command` in project.json.
- **`harness`** — behaviour measurable only at the test layer. Run the project's
  ...
  test — never re-derive the measurement.**
- **`cli-diff`** — run the project's demo command twice (baseline + HEAD), capture
  stdout, encode the before/after in `beforeNote`/`afterNote`. No media.
- **`artifact`** — a generated file/output; describe the before/after artefact in
  notes (and diffStat). No media.
- **`none`** — infra-only / no observable surface. A single checkpoint that is a
  rationale block: what changed and why it's correct. No before/after.

## Media capture (the optional, best-effort step — folds in the old demo-capture)
...
It runs the two-worktree (baseline `main` vs changed `HEAD`) Playwright capture
(`generateComparisonDemo` in `cli/demo.ts`), merges the captured
`before/<label>.png` + `after/<label>.png` into the matching `demo.json`
checkpoints by `label` as inline `data:` URIs (size-capped), and re-renders
`DEMO.md`/`DEMO.html`. Discipline:
...
- Every checkpoint becomes a `CheckpointCard` (caption + before/after media or
...
- The screen mirrors structured state to `data-*` (`data-section="demo-comparison"`,
  ...
  DOM-as-metrics convention, so automation can read the demo the way the operator does.
...
and (for `browser`) media capture has been attempted best-effort.

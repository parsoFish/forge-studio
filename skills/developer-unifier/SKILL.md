...
3. **Author the structured demo** at the demo dir named in PROMPT.md (ADR 021) — `demo/<initiative-id>/demo.json` for a default-layout project, or `<artifactRoot>/history/<initiative-id>/demo/demo.json` when the project sets an `artifactRoot` (e.g. betterado's `forge/`). **PROMPT.md gives the exact path; write there.**
   ...
   - **Prefer REAL captured output over prose.** For every behavioural checkpoint set `command` to the EXACT argv whose stdout IS the evidence (the built CLI invocation, the gate command) and leave `beforeOutput`/`afterOutput` empty; use `beforeNote`/`afterNote` only as a one-line caption or when no command can produce the evidence. Then run `forge demo capture <initiative-id>` — forge runs each `command` on `main` AND on the branch HEAD and back-fills the real before/after terminal output (and screenshots for browser checkpoints) that the review page renders side-by-side. **A demo whose checkpoints carry only prose notes is NOT acceptable visual verification when a command could have produced the evidence.**

...
The frontmatter is missing a required field (title, description, or category — a missing date field is auto-fixed upstream and never reaches this skill). Add the missing field to the YAML front-matter block between the `---` delimiters. Choose `pattern`, `antipattern`, `decision`, `operation`, or `reference` as appropriate for `category`.
...
### `length.hard-cap`
The file exceeds the hard line-count limit. This requires significant pruning. Summarise or remove the least essential sections. Prefer removing duplicate content, over-long examples, or verbose prose. Keep all load-bearing facts.

### `length.soft-cap`
The file exceeds the soft line-count limit. Lightly trim: shorten verbose sections, remove redundant examples, or consolidate related bullet points. Do not remove factual content.

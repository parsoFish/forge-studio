...
- **Demos / mockups / journeys:** delete `demos/verify` (2.5 GB untracked); keep `demos/e2e` (92 MB curated gallery), regenerated only on a beat change. Extract the facts `mockups/studio-endstate-v2` still supplies to `home-view.ts`, `roadmap-dag-layout.ts` and `story-registry.mjs` into one reference doc, then delete `mockups/`. Journeys are replaced by the ten operator stories + one smoke story (`docs/roadmaps/1.0.md` §3, catalogue in `docs/product/user-stories.md`); the seven roadmap-slug pins become per-package contract tests; costless stories run in CI; the old harness is deleted in M6.
...
2. **Two factories, nine stories.** The OOTB develop factory merges real initiatives on gitpulse; a second factory built from data only merges too; deleting the example package leaves the platform running; the ten operator stories (`docs/roadmaps/1.0.md` §3; catalogue `docs/product/user-stories.md`) run green on a clean checkout in one recorded run.
...
4. **Capped weight.** Per-package LOC caps, ≤ 25 hand-written docs, ten stories + smoke and no other e2e harness, identity lint, one roadmap; campaign notes never in `docs/`.

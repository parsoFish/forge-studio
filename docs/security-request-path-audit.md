...
Two new sink sites, both introduced by this initiative (R4-17, T3/WI-1) and
flagged by `scripts/check-request-path-sinks.mjs` on first use — the ratchet
working exactly as designed, catching its own initiative's new surface before
merge. Neither reopens a shape this document has already closed.

| file:line | op | request field | class | evidence |
|---|---|---|---|---|
| `cli/contract-stages.ts` (`resolveContainedProjectDir`) | `realpathSync` ×2 | `GET /api/studio/sessions/onboarding/:sessionId?project=`, `GET /api/studio/projects/:id/contract-stages`, and (indirectly, via the write route below) `POST /api/studio/onboarding/start` — all feed `projectId`/`project` | guarded `[read]` | `SLUG_RE` + length cap first (rejects before any fs call — AT-25). Then `realpathSync(join(projectsRoot, projectId))` compared against `realpathSync(projectsRoot)` with a `startsWith(root + sep)` boundary check — the SAME shape as `resolveSafeSessionDir` (`cli/bridge-studio-sessions.ts`, row above), **deliberately NOT** `resolveGuardedPath`/`isContainedProjectRepoPath`: those two enforce a STRICTER per-segment IDENTITY check that rejects a symlinked `<root>/<id>` pointing at a DIFFERENT real object under the SAME root even when that object is legitimate — this module's own AT-28 (a false-rejection control) requires exactly that shape to be ACCEPTED (a relative symlink resolving back inside `projectsRoot`), so the stricter guard is the wrong tool here, not an oversight. See `cli/contract-stages.ts`'s own module header for the full note. Verified by DIRECT-EXECUTION tests in `cli/contract-stages.test.ts` (AT-27: a real symlink escaping to an outside directory never surfaces its content; AT-28: the false-rejection control is accepted; AT-25: a non-slug id is rejected before any lookup; AT-29: two projectIds never cross-contaminate) — not an HTTP-route-level probe, so classified `[read]` per this document's own convention (mirrors `orchestrator/studio/session-transcript.ts`'s identical `[read]` classification at the row above, despite THAT module's own AT-35/36/37 also being direct-execution symlink tests, not HTTP probes). **Completeness note:** the `contract`/`instructions`/`secrets`/`demo` rows read `.forge/project.json` via `orchestrator/project-config.ts`'s `loadProjectConfig`, which internally calls `readAgentInstructionsFile`/`readQualityGateSidecar` — both already filed `unguarded [read]` → `forge-2zz` in Table 2 above. This module is a NEW caller reaching that pre-existing, already-tracked gap; not a new finding, but named here per the "one sink, many entry points" completeness rule. D3 (names-only secrets) is enforced structurally — `secrets.env` is never opened anywhere in this module, pinned by `cli/contract-stages.test.ts` AT-12's sentinel-value plant. |
| `cli/ui-bridge.ts` (`POST /api/studio/onboarding/start`) | `realpathSync`, `mkdirSync`, `writeFileSync` ×2 | body `project` (+ `inputs`) | guarded `[exec]` | `project` is `SLUG_RE` + length-cap validated (reusing `invalidGenerationProjectReason`, the same helper the demo-generation routes already use) before any fs call. The project directory is then `realpathSync`'d and every subsequent write (`_onboarding/<sessionId>/status.json`, `.../prompt.md`) is joined onto that REAL, verified directory; `sessionId` is generated by this route's OWN code (`newArchitectSessionId`) and is never request-derived, so the join cannot itself introduce an escape — an improvement over the legacy architect/instructions/project-brain `/start` siblings' unguarded `architectSessionDir`-shaped join (row filed `forge-28o` above), not a repeat of that gap. D5's headline guarantee — that a caller-supplied `projectRepoPath` has ZERO effect, because the route has no such field to read at all — is executed live: `cli/ui-bridge-onboarding-start.test.ts` AT-3 plants a real, independent outside directory, sends it as `projectRepoPath` in the body, and asserts byte-for-byte both that the real session lands at `<projectsRoot>/<project>/_onboarding/<sid>/` AND that nothing whatsoever is created at the injected path. AT-1/AT-2 (`cli/ui-bridge-onboarding-start.test.ts`) additionally pin that a malformed/unknown `project` is rejected (400/404) before any `_onboarding` directory is created anywhere. |

**Not ratchet-tracked, guarded regardless:** `cli/agent-run.ts`'s new
...
through `spawnAgentDispatch`'s new optional 6th argument), not raw request
text — but the write is still guarded rather than trusted blindly: the
function realpath-resolves `sessionDir` itself and refuses to follow
`status.json` if it turns out to be a symlink escaping that resolved
directory. Pinned by `cli/agent-run-dispatch.test.ts`'s AT-D7-1/AT-D7-2 (write
happens on both success and failure) and AT-D7-3 (D6: omitting `--session-dir`
touches no session `status.json` anywhere, byte-identical to before this
flag existed).

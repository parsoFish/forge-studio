...
- kb-cleanup's approve delegates to `enqueueConsolidate`/`runBrainConsolidateNow` (`cli/bridge-studio-kbs.ts`, already audited) — this route adds NO new call to either function's own internal sinks, only a new CALLER, and that caller passes `kbId` sourced exclusively from the session's own `status.kb_id` (never a URL segment — this route carries no `:id` segment at all, so the mismatch class the bespoke `/cleanup/apply` route's DEFECT B row (Table 1, "Guarded ... R1-06") had to guard against structurally cannot arise here).
...
Security tests: `cli/bridge-studio-affordances.test.ts` (32 tests) — traversal sessionIds (literal `..` via raw `http.request`, an `isSafeRunId`-rejecting embedded `..`, an absolute-path-shaped id) all `[exec]` 404 with no path echoed in the response body; unknown session kind / unknown affordance id / wrong-phase affordance all `[exec]` 404/409 naming the offending value + the allowed/available set; malformed and oversized (>1 MiB) bodies `[exec]` rejected (mirrors `cli/bridge-studio-write.test.ts`'s own oversized-body precedent — the server destroys the socket, so either a non-200 response or a connection error is acceptable, never a 200); a rejected (409/422) call leaves `status.json`'s `phase` byte-unchanged on disk, `[exec]`-verified per test.

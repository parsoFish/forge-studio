...
| `orchestrator/agent-dispatch.ts` (`discoverStagedMaterials`) | `readdirSync` on `<logsRoot>/<runId>/materials` | — | guarded `[exec]` | R6-04 WI-1. **Supersedes this file's "no request-derived path" row above for `agent-dispatch.ts`.** Lists the staged materials directory so the references reach the agent's prompt. `runId` is server-minted (`_agent-<slug>-<stamp>`, minted in the bridge route from a `SAFE_AGENT_SLUG_RE`-validated slug) and `logsRoot` is config-derived — neither is request-supplied AT THIS CALL SITE. Read-only listing; the errno is discriminated (ENOENT silent, anything else surfaced) rather than swallowed. |

...
| `cli/bridge-studio-writes.ts` `scaffoldContractArtifacts` (W7-B6, projects-11) | `realpathSync` ×2 | `POST /api/studio/projects` body `repoPath` (via `projectRoot`) | guarded `[read]` | Identity COMPARISON only (`git rev-parse --show-toplevel` output vs `projectRoot`, both realpathed) deciding whether to `git init`; `projectRoot` was already validated by the caller's `isContainedProjectRepoPath` before this function runs, and nothing is read or written through the realpathed values. |

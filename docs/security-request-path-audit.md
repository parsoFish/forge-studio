...
**29 rows carry `[exec]`** (live repro executed), **24 carry `[read]`**
(classified by reading), and exactly **one row carries both** (the
`/demo/`+`/fragment/` row, where the escape is `[exec]` and the
no-plant-primitive-found reasoning is `[read]`). 29 + 24 − 1 = **52**, the

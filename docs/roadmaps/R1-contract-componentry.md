...
The pieces the KB contract's four processes will formalise already exist as scattered mechanisms: reflector-written Brain-3 themes (ADR-010/018/035), the `brain-ingest` skill (Brain-2 ingest from `brain/_raw/cycles/`), guidance append + bootstrap + maintenance bridge routes (`POST /api/studio/kbs/:id/guidance` (M5-3), `/bootstrap` (P3), `/maintenance` (K3) in `packages/knowledge/bridge-studio-kbs.ts`), the lint-resolution agent (`orchestrator/brain-fix-runner.ts`, spawned via `spawnBrainFix`), and the onboarding Brain-3 builder (`orchestrator/project-brain-seed.ts`, `project-brain-builder-runner.ts`, UI `/project-brain/[sid]`).

### R1-B5 Unified project contract, two faces

ADR-017 (original C1–C6 derivation) + ADR-034 (Studio-aligned unification): Face A = five Studio fields in `.forge/project.json` (`northStar`, `instructions`, `demoProcess`, `skills`, `kb`) checked by `forge-ui/components/studio/project-builder/ContractReadiness.tsx`; Face B = operational clauses C1–C10 + DEMO/DEMO-SKILL/ARTIFACTS/BRAIN checked by `packages/projects/preflight.ts`. `data-flow-ready = uiAllReady && preflightOk`. SSOT document: `docs/forge-project-contract.md` (enforcement table at its foot). Hand-checked residue: C1b gate-discrimination facet, C2 build-artifact facet, C7 external-resource clause, C9 fixtures (ADR-017/034 notes) — R1-05's target.

### R1-B6 Demo process as-built

`demoProcess[]` typed capture/verify/present steps (ADR-027 §3, ADR-034 §2 threshold: ≥1 capture + ≥1 verify). Naming disambiguation (adversarial review A4): the surface at `/demo/[sid]` is the **demo-builder** (`skills/demo-builder`, `orchestrator/demo-builder-runner.ts`, journey `scripts/journeys/demo-builder.mjs`) — it authors demo-page machinery; **`skills/demo-design`** is a different skill, the save-time generator that runs when a project's `demoProcess` is saved (wired via `apps/forge/bridge-studio-writes.ts` + `preflight-resolve.ts`) and stays the save-time step under R1-03-F2; **`skills/demo`** is the demo-contract SSOT whose composer moves from the retired unifier to the R4-07 demo agent (disposition table: R4-B6). Evidence capture is **orchestrator-owned** per ADR-036 (`orchestrator/phases/orchestrated-capture.ts`, per-run nonce `FORGE_CAPTURE_NONCE`, producibility preflight) — agents author WHAT to capture, never the evidence.
...
- **Features:**
  - **R1-02-F1 — Route the planning-context read through KbBackend.** `loadBrainIndex`/`loadBrainNavigation` consumers (`orchestrator/phases/pm-binding.ts`, `orchestrator/phases/reflector-binding.ts`, `orchestrator/architect-runner.ts`, `orchestrator/cli.ts` — binding paths per R4-01-F2, 2026-07-24) read via `getKbBackend` so the brain's *planning* influence is backend-swappable (the ADR-027 amendment's named "riskier reroot" — it gets its own change, per that note). The `usage` process (R1-01-F1) is the declaration this read honours. Acceptance:
...
- **Implemented notes (2026-07-24, wave-4 session 2, branch `feat/r1-03-contract-processes`):**
  - **F1 — typed `testProcess` object.** `orchestrator/project-config.ts` gathers `local`/`ci`/`acceptance` into one declared `testProcess` object; the flat keys (`quality_gate_cmd`, `ci_gate`, `ci_fix_cmd`, `ci_gate_unset_env`, `acceptance_gate`) are REJECTED with a migration-mapping error message (no permanent back-compat shim). `validateProjectConfig` derives the flat accessors consumers still read (`quality_gate_cmd`, `ci_gate`, `ci_fix_cmd`, `ci_gate_unset_env`, `acceptance_gate`) from the typed object, so the ~20 existing call sites are unchanged. The `.forge/quality_gate_cmd` sidecar is re-rooted onto `testProcess.local.cmd` (single-sourced when the JSON omits it; JSON wins when both are present). Declared `timeoutMs` on `local`/`ci`/`acceptance` wires the env-override-wins-over-declared-wins-over-default ordering ADR-036 already established (`FORGE_GATE_TIMEOUT_MS` / `FORGE_CI_GATE_TIMEOUT_MS` still win over a declared `timeoutMs`). Real preflight checks for C1b and C7 were added, closing the enforcement-table gap that previously listed them as hand-verified only.
  - **F2 — demo-builder folded into `/projects/[id]`.** The standalone `/demo/[sid]` session route is replaced by a panel on the project page, plus a redirect stub for the old route; the `demo-builder` journey was rewritten to exercise the new in-page flow.
  - **F2 REVERSED (2026-08-15, wave-6, W6-B10 — the demo-builder entrypoint repair).** The inline `DemoBuilderPanel` is DELETED, along with the project page's `?demo=` deep-link handling and `activeDemoSid` state — the demo builder is now a **dedicated session screen** (`/sessions/demo/<sid>`, the ONE session screen every interactive kind shares, W6-B6), reached from `DemoTimeline`'s launch button, `ContractResolutionPanel`'s DEMO-clause resolve button, and the roadmap's "demo builder →" link (which now routes honestly — resumes an in-flight session or opens `/sessions/demo/new?project=<p>&initiative=<id>` — rather than the old fake `setTab('editor')`). Rationale: consistency with the instructions session (already a dedicated screen, never inline) and with the generic-interactive-surface direction (ADR-043) — a demo-only inline exception was the odd one out, not a load-bearing design choice. `/demo/[sessionId]` graduates from a data-dependent client shim to a pure `next.config.mjs` wire redirect once the destination screen learned to resolve its own project (the SAME `listDemoSessions()` lookup, just server-side of the navigation instead of client-side of it). The gap this surfaced: the generic session-shell's `demo` panel table had NO row for the `briefing` phase — every demo session is minted straight into it — so a session opened on the dedicated screen before this fix could never get the agent started; `studio/session-kinds.yaml` gained a `{ phase: briefing, step: noop, awaits: questions }` row and `cli/bridge-studio-affordances.ts` gained `handleDemoBrief`, mirroring `POST /api/demo-builder/brief`. Known, disclosed regression (not fixed here — needs a real affordance-model extension, out of this lane's scope): the old inline panel's "apply feedback & regenerate" free-text loop, per-element iterate-with-its-own-prompt, and "view a previous locked demo" history have no equivalent on the generic panel (only approve[+optional generation pick]/reject) — `VERDICT_VALUES` is a closed `['approve','reject']` set with no "revise in place" semantics yet.
  - **F3 — DEMO-ALIGN advisory clause.** A token-heuristic check (`packages/projects/preflight.ts checkDemoAlignment`) flags a `demoProcess` capture step that doesn't reference any `testProcess` command token; always advisory (flags, never blocks) — confirmed on betterado's live-evidence demo, which legitimately diverges from the declared test process and is flagged, not failed.
  - **F4 — relocation spec landed, verdict pending ⚑.** The dual-boundary full-suite gate relocation spec is written in `docs/forge-project-contract.md` ("The merge-boundary full-suite gate" section) and [`docs/decisions/036-orchestrator-owned-gate-execution.md`](../decisions/036-orchestrator-owned-gate-execution.md) (its 2026-07-24 amendment) records the operator-verdict gate. **Nothing is enforced yet** — R4-10-F2 (the runnable-side build-and-prove owner) remains blocked until the verdict is recorded.
...
- **Features:**
  ...
  - **R1-04-F2 — Typed release process.** Expand `releaseProcess` from opt-in flag territory into a typed step list akin to `demoProcess`: `notes` (release communications/changelog — today's draft-changelog standing AC), `cd` (the CI release workflow / publish step), `validation` (post-release verification, e.g. `terraform init` handshake for betterado, `npx <pkg>` smoke for mdtoc). C10 checks each declared step's substrate exists (changelog file, workflow file, validation command). The release-finalizer consumes the typed steps instead of convention. Acceptance:
    - `orchestrator/project-config.ts` schema + validator; C10 in `packages/projects/preflight.ts` upgraded to per-step checks; mdtoc + betterado configs migrated; finalize path behaviour-identical on migrated configs (the `hasReleaseProcess` opt-in gate semantics preserved).

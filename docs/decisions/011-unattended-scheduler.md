...
A **persistent process named `forge serve`** runs the scheduler. The original scaffold target was ~150 LOC; the **as-built reality is larger** (see the LOC reconciliation below and `docs/_archive/architecture/as-built-snapshot-2026-05-17.md` §B — post-stripback `scheduler.ts` is ~676 LOC, `cycle.ts` is a 404-LOC thin spine, the orchestrator total ≈10,857 non-test LOC, most of the bulk being the shared bench↔live `*-invocation.ts` contracts ≈1,170 LOC). The target stands as a *pressure*, not a measured fact. Components:

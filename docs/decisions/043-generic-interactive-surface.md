...
- **Related roadmap:** `docs/roadmaps/archive/R4-ootb-suite.md` — R4-21 (OOTB authoring agent) is consumer #1.
...
**Architect stays a named structured step-handler variant, not pure data.** Architect carries brain-first planner prompt injection, three *different* structured sub-turns (interview/explore/draft) via a local `runStructured` predating the shared one, a fail-open `exploring` step, a forced-emit retry (`orchestrator/architect-runner.ts` ~L912–943), and a completeness-critic gate that can send the draft round again before the operator is ever asked (operator ruling 380, 2026-09-07; it previously bounced the session back AFTER the approve press). That is branching control flow, not a linear phase table. Tabling it would silently lose the retry and fail-open behaviors. Architect migrates last and remains a **registered step-handler variant** (an honest partial), with golden-capture mandatory before swapping its local `runStructured` for the shared `runStructuredTurn`. Architect also stays outside the ADR-039 flow-runner executor registry.
...
## Amendment — 2026-09-25 (forge-7m2): `stagingDirName` — the committing phase's own authored field, retiring the finalizer's hardcoded staging literal

§1's worked example (and the real `authoring` descriptor, `studio/session-kinds.yaml`) declares `analyzing: { step: agent, writes: [staging], … }` but left the `committing: { step: finalize, finalizer: copyStagingToLibrary, … }` row silent on which directory that finalizer reads FROM — `copyStagingToLibrary` (`packages/sessions/interactive-finalizers.ts`) instead hardcoded the literal `'staging'` at its two call sites (`discoverStagingEntries`'s `resolveGuardedPath(sessionDir, ['staging', …])` and its `stagingRoot = join(sessionDir, 'staging')`), kept honest only by a source-text ratchet test (`packages/sessions/tests/unit/interactive-finalizers-toctou.test.ts`) rather than a real single source of truth. A session kind authoring a non-default staging dirname would have its finalizer silently look in the wrong (literal) path.

Fix, per §1's own additive-optional discipline (a `TurnSpecPhase` field, the same class as `writes`/`awaits`/`verdicts`/`requires`): a `step: finalize` phase row may declare `stagingDirName: <dirname>` — the read-side twin of an earlier `agent`-step row's `writes: [<dirname>]`. `authoring`'s `committing` row now reads:

```yaml
- { phase: committing, step: finalize, finalizer: copyStagingToLibrary, stagingDirName: staging, next: committed }
```

`FinalizerContext` (`interactive-finalizers.ts`) gains an additive-optional `stagingDirName?: string` (ADR-042 boundary #2 — optional on the TYPE because `writeToRepoRoot`/`recordLockedDemo` never consume it); `runFinalizeStep` (`interactive-agent-step.ts`) threads `phaseRow.stagingDirName` onto it, never defaulting when the row omits one. `copyStagingToLibrary` itself now REQUIRES the field present at call time and throws a named `InteractiveFinalizerError` naming the omission when it is not — never a silent fallback to a literal. `stagingDirName` is structural-only at load time (like `writes`, per its own EXPIRY CONDITION) — `validateSessionKinds` does not check it against a closed vocabulary; no such vocabulary exists.

The yaml row is now the single source of truth for both the write side (`analyzing`'s `writes: [staging]`) and the finalize side (`committing`'s `stagingDirName: staging`) — the only hardcoded `'staging'` literal left anywhere in this path is `session-artifact-derivers.ts`'s `PACKAGE_DIRNAME` (the read-side artifact-rendering twin, out of `FinalizerContext`'s scope), and the toctou test file's RATCHET test now compares that constant against the yaml's authored value (via `loadSessionKinds`) instead of scanning `interactive-finalizers.ts`'s source text for literals that no longer exist there.

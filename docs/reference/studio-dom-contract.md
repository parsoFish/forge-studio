...
- **`/community`, `/community/[kind]/[id]`** (R3-07-F1/F2/F3) — the ONE
  ...
  desc, id, **category** (W8-B5 — the word the registry files rows under;
  it reached the client nowhere before, so it had to join
  `CommunityItemWire` first), hub label, signals attribution and upstream
  URL; it carries an `aria-label`, W8-B5/community-32 — a placeholder is not
  an accessible name), kind-filter buttons
  ...
  **Browse state is URL state (W8-B5, community-35).** The five browse
  ...
  bare `/community` and the page never rewrites its own URL on mount). The
  root `data-kind-filter`/`data-hub-filter`/`data-sort-key`/`data-sort-dir`
  attributes are rendered FROM that parsed state, so the DOM contract and
  the address bar cannot disagree. Kind / hub / sort changes `push` (Back
  restores them); the search box keeps a local draft and mirrors itself into
  the URL on a 350 ms trailing debounce with `replace`, so a typing burst is
  one navigation, not one per keystroke. A router write is ASYNCHRONOUS, so a
  second interaction landing before the first is applied builds on the FIRST
  one's state (`writeBaseState`, same module) — picking a kind and immediately
  clicking a hub chip does not drop the kind; the pending write expires the
  moment the URL moves, so a Back press never resurrects it.
  ...
  **W8-B5 (community-29): the required set is stated ONCE.**
  ...
  "description" (not required) and "upstream" (not a field on this form)
  while leaving `category` and `provenance` unnamed. The reason is now
  generated by naming the required fields actually still empty, so a field
  added to that list changes the message with no string edit anywhere.

  **W8-B5 (community-30): `?edit=<id>` for an id that does not exist renders
  ...
  **W8-B5 (community-18, the half the earlier fix missed):** for an mcp/tool
  the owning `/connections/<id>` page is reachable REGARDLESS of install
  state. `open-owning` covered the already-present cases; a NOT-installed
  connection — the state where an operator most needs the env vars, the
  probe explanation and the how-to — had no route to it at all. So
  `[data-section="install"]` additionally carries `[data-connection-link=
  "true"]` and renders `[data-component="connection-page-link"]` holding a
  second `[data-action="open-owning-page"]` (one word for one destination,
  never a new vocabulary) BESIDE whatever install arm is showing — the
  install action is untouched. It is computed by `connectionPageLinkFor`
  (community-view.ts), which returns null whenever the primary action
  already links that page (no duplicate) and for skill/hook (whose owning
  pages only exist once something is installed there).
...
- **`/sessions/[kind]/new` — the ONE kickoff screen for every session kind
  ...
  (`@forge/agents`'s `isStudioAgent`), so the roster never

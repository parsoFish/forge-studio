...
`cli/brain-lint.ts` `existsSync` 21 → 22 and `readdirSync` 10 → 11 (the slug
universe now walks every `brain/*/themes`, so an operator-created KB's edges are
visible to the audit), and `cli/bridge-studio-kbs.ts` `readFileSync` 3 → 4 (the
draft re-audit at approve time). `cli/kb-drain-edit-soundness.ts` `rmSync` goes

...
| `packages/flows/finalize-merged.ts:229` | `readdirSync`/`rmdirSync` | `POST /api/verdict` approve → `finalizeAfterMerge` → `pruneMergedWorktrees`: manifest `worktree_path` + `initiative_id` | guarded `[exec]` | `resolveGuardedPath(dirname(worktreePath), ['wi', initiativeId])`; `worktree_path` is already contained by `isContainedWorktreePath` in `finalizeMergedReadyForReview` before any use, and the id rides as its own segment. Only an EXISTING, EMPTY `wi/<id>/` is removed (non-recursive `rmdirSync`). Bead forge-8vfn.8.1.3. |

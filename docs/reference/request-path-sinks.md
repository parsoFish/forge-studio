...
| `packages/sessions/turn-cost-rows.ts` (`recordEmitFailure`) | `appendFileSync` 0 -> 1 | NOT a bridge route — the session/story spend emitters' FAILURE path | `accidentally-safe` -> **not request-derived** `[read]` | **New in M6-A, bead `forge-8vfn.7.6.103`, and the reason the baseline grows here.** No caller-supplied value reaches the sink. The path is `join(dirname(logger.logFilePath), EMIT_FAILED_SIDECAR)`: the directory is the one the `EventLogger` ITSELF already appends `events.jsonl` to (`packages/kernel/logging.ts:165`), so this adds an operation to an EXISTING trusted root rather than opening a new taint path — the same argument as `pm-rejected-set.ts` above. The basename is the module-level constant `EMIT_FAILED_SIDECAR` (`.emit-failed`), never a caller value and never joined from one; `logFilePath` is minted by `createLogger(cycleId, logsDir)` where `logsDir` is config-derived and `cycleId` is containment-checked upstream by the routes that mint sessions. Classified `[read]`: no escape shape was planted against this path because no request-derived component reaches it, and the containment property the doors assert is the one this bead exists for — that a run whose ledger cannot be WRITTEN stops, verified through `ceilingHaltVerdict` rather than through the emitter. **Why the surface had to grow at all:** the row that records an unpriced turn is itself the row that fails to write, so the signal cannot BE a row; a sidecar in the same directory is the only place a reader already walks. The alternatives considered and rejected were throwing (destroys a turn that already ran and cost money — ruling 849) and leaving the `catch {}` (the defect). **The precondition that would make this live** is a route accepting a caller-supplied log path into `createLogger` — which is what the session routes' own guards refuse. **Named residual, doored rather than described:** when this `appendFileSync` ALSO fails, a distinctive stderr marker is the only trace and the run continues blind; `scripts/stories/emit-fails-open.test.ts` asserts that with a `chmod 0500` logdir, including the non-halt, so closing it later requires changing that door. |
| `packages/kernel/claude-cli-path.ts` (`resolveClaudeCliPath`) | `statSync` 0 -> 1 (plus `accessSync`) | NOT a bridge route — the SDK spawn path, read once per `query()` | `accidentally-safe` -> **not request-derived** `[read]` | **New in M6-A, bead `forge-8vfn.7.6.116` (P0), and the reason the baseline grows here.** The single input is the `FORGE_CLAUDE_CLI` environment variable, set by a LAUNCHER and never by a route: the three non-test occurrences repo-wide are its own constant, the `AGENT_ENV_ALLOWLIST` entry, and a comment — no handler, phase or request path assigns it, and nothing in `apps/forge` or `packages/sessions` writes `process.env` from a request at all. The calls do not OPEN or WRITE anything: `statSync` asks whether the named path is a regular file and `accessSync(X_OK)` whether it is executable, and both exist to REFUSE. **The function is the guard, not a consumer of one** — it rejects a relative path (it would resolve against an inherited cwd nobody chose), a path absent from disk, a directory, and a non-executable file, each with its own sentence. **Why the surface had to grow at all:** the pinned SDK spawns its own bundled CLI 2.0.77, whose Oct-2025 Consumer Terms gate began exiting 1 before any turn — every real-spawn run on this box died at its first agent turn, S9 run 1 at beat 8 for $0 of $25 — and naming the binary means checking that the name is usable. The alternative considered and rejected was a PATH fallback to the bundled binary, which IS the bug and would restore it silently. Classified `[read]`: no escape shape was planted because no request-derived component reaches it; the containment property asserted by `packages/kernel/tests/unit/claude-cli-path.test.ts` is the REFUSAL set. **The precondition that would make this live** is a route that writes `FORGE_CLAUDE_CLI` into the process environment. |
...
| `packages/sessions/kinds/demo-builder.ts` (`withStudioRepo`'s post-dispatch commit) | none — a boolean probe, per this repo's standing rule "guard the paths you WRITE, not the paths you merely probe" | `status.project_repo_path` joined with **fixed literals** (`.forge/demo`, `.forge/skills/demo-design/SKILL.md`, `.forge/skills/demo`) — no request-derived segment reaches it | accidentally-safe `[read]` | **`existsSync 4 -> 5`, bead `forge-8vfn.7.3.6`.** The commit was `git add -A -- .` and swept whatever else was dirty in the ground into a `forge-studio: demo machinery (…)` subject — two funded S1 runs committed 9 and 10 files of the ONBOARDING agent's output with no demo in either. Scoping the commit needs to know which of its own roots exist, so the probe is the fix, not a new surface: every joined segment is a literal in this file, the repo path is the one `invalidProjectRepoPath` already validates at `/start` (the row below), and the result is a boolean that only decides whether to stage — no bytes flow through it. |
| `packages/sessions/bridge-studio-session-index.ts` (`GET /api/studio/sessions`, plus the four per-kind readers it aggregates) | `resolveGuardedPath` / `guardedSessionDir` on every session dir it enumerates | `project` and `sessionId` as directory ENTRY NAMES read by `readdirSync`, never a route parameter — the index takes no untrusted input of its own | guarded `[exec]` | **M4 session-routes carve, final block — relocated and conserved:** `existsSync 26 -> 18` and `readdirSync 17 -> 9` in the host plus 8 each here, `readFileSync 4 -> 3` plus 1, with 1346 total sink calls repo-wide unchanged. This module carved LAST because it aggregates all four families' readers, so it could not move until they had. It is also the least request-derived surface in the carve: the route has no path parameters at all, and every name it touches comes from a `readdirSync` of a trusted root rather than from the URL. The `orchestrator/project-brain-builder-runner.ts mkdirSync 1 -> 0` tightening in the same run is that runner's shell losing its last reachable sink as the reader moved — handoff K21's own row shrinking as a side effect. **forge-7kzj — `existsSync` 8→9, `readdirSync` 8→10, the reason the baseline grows here.** `discoverUnknownSessionKindIds` surfaces unregistered-kind session dirs (a retired kind's leftover sessions, or a kind dropped without a migration) as the index response's `unknownKinds` diagnostic, by scanning the SAME trusted `projectsRoot` in the opposite direction: every project's OWN `_*` subdirectories, not just the registered kinds' dir names. One `existsSync(projectsRoot)` plus one `readdirSync(projectsRoot, …)` collects the project-NAME list (identical shape to the per-kind loop above it in this file); one more `readdirSync(join(projectsRoot, project), …)` per project — `project` itself a readdirSync-enumerated NAME off the trusted root, never request data — collects each project's `_*` directory-entry NAMES. No route parameter or body value reaches either call, and nothing joined beyond that one `join(projectsRoot, project)` is ever passed to a sink. It reads directory entry NAMES only (`withFileTypes: true`), never opens or reads a file's contents, and returns a bare deduped `string[]` of bare kind ids to the caller — there is no leaf file to route through `resolveGuardedPath`/`guardedSessionDir`, so none is called; **accidentally-safe** for this specific addition (no guard mechanism invoked, but no request-derived component exists to escape with) — the row's overall `guarded [exec]` classification is unaffected because the pre-existing per-kind readers it sits beside are unchanged. Verified `[read]` by `apps/forge/tests/integration/ui-bridge-sessions-index.test.ts`'s unknown-kind-dir case. |
| `packages/sessions/studio/session-transcript.ts:149-171,189-213` | `readFileSync`/`readdirSync` | session dir from the guard above | guarded `[read]` | Real `realpathSync` containment on both dir and joined path; only fixed literal filenames accepted, never a client-supplied one. |
| `packages/library/studio/hook-library.ts:164-198` | boundary + realpath | `hook.yaml`'s own `script:` field | guarded `[read]` | `resolveHookScriptPath` rejects literal *and* percent-decoded traversal. Note it does **not** protect against the hook *directory* being a symlink — that was a separate finding, fixed below. |
| `packages/library/studio/hook-runtime.ts` (`readFileSync(scriptPath)`; `spawnSync('bash',[scriptPath])` in the sync tail `runHookScript`; `spawn('bash',[scriptPath])` in the async tail `runHookScriptAsync`, forge-9a3) | script read + **process spawn** — TWO spawn sinks since forge-9a3 (`spawnSync` 1, `spawn` 1, new) | any route that spawns an agent, via `packages/agents/studio/hook-dispatch.ts` | guarded `[read]` for containment · `[exec]` for the approval refusal | **W8-B6 ADDED THIS REACHABILITY DELIBERATELY, and it is the only row in this table that ends in an `execve`.** Before W8-B6 `runHookScript` had zero production callers, so both sinks scored a baseline of 0 in `scripts/check-request-path-sinks.mjs`; wiring hook dispatch into the seven claude spawn sites makes them reachable from every agent-spawning route. Three independent gates, none of them relaxed by the wiring. (1) `hookDir(id)` calls `assertSkillSlug` before any path is built, so the id is a validated single segment `[read]`. (2) `def.script` is contained by `resolveHookScriptPath` — the row directly above `[read]`. (3) `runHookScript` refuses unless `hookRunState(forgeRoot,id).runnable`, i.e. an operator approval whose ledger entry still hash-matches the current script bytes, permissions and trigger. **The refusal is `[exec]`**: `packages/agents/tests/unit/hook-dispatch.test.ts` binds an unapproved hook to a real fixture agent, fires the real callback, and asserts the side-effect file the hook script would have written is ABSENT — and a second case approves a hook, edits its script afterwards, fires, and asserts the same absence. Dispatch calls `runHookScript` unmodified and passes **no** `parentEnv`, so `buildHookChildEnv`'s narrow `HOOK_ENV_BASE_ALLOWLIST` (never `AGENT_ENV_ALLOWLIST`) still governs the child env — pinned live by `hook-dispatch.test.ts`'s canary, which sets a var in the parent at dispatch time and asserts it is `ABSENT` in the child. **Hardened in the same lane, 2026-08-24, after a hostile review of this exact row's sinks (W8-B6 FIX-1):** the fence is a TWO-layer composition and was enforced on one. `HOOK_ENV_BASE_ALLOWLIST` closed the base, but `buildHookChildEnv` copied every `permissions.env` name out of the real `process.env` into `buildChildEnv`'s `overrides`, which apply unconditionally by design — so a manifest that simply *asked* for `ANTHROPIC_API_KEY` got the real value, proven with a real spawned child. `HOOK_ENV_CREDENTIAL_EXCLUSIONS` is now consulted by BOTH layers and the refusal is emitted as a `hook-env-grant-refused` event; and gate (3) got materially stronger, because `computeVerdict` now blocks on ANY `critical` finding (the older env-read + network-egress PAIRING was evadable through the egress detector's own documented blind spots — `/dev/tcp/`, `python3 -c`, `ssh`, `dig`, all of which are now detected), so a secret-shaped grant costs an explicit, reasoned `overrideHookBlock` instead of a one-click approve. **Standing limit, unchanged and inherited from `hook-runtime.ts`'s own header:** `permissions.read`/`permissions.network` are declared and statically scanned but NOT enforced at spawn, and file writes are not modelled at all. Making a hook fire does not change that — but it does mean an operator who authors, approves and binds a hook is now running that code for real, which is the whole point and must be read as such. **FORGE-9A3 (2026-09-25) — the second spawn sink, classified the SAME way, not a new gate:** `runHookScript` was the ONLY spawn tail, and it is `spawnSync` — a genuine OS-level blocking call — so a firing hook blocked forge serve's whole event loop (scheduler + Studio bridge) for the hook's duration, and `PostToolUse` fires per tool call. The fix extracted gates (1)–(3) above plus the pre-spawn logging into ONE shared `prepareHookRun` step and gave it a second, thin, ASYNC tail (`runHookScriptAsync`, built on `child_process.spawn`) that `hook-dispatch.ts` now awaits instead of calling the sync tail. `prepareHookRun` is the ONLY place any of gates (1)–(3) run — there is no second copy of the id-validation, the script-path containment, or the approval check beside the new spawn call, so the new `spawn` sink is reachable ONLY after the exact same three gates the existing `spawnSync` sink already passed. Verified `[exec]` the same way: `packages/library/tests/integration/hook-runtime-async.test.ts` binds an unapproved hook to the async tail directly and asserts the marker file a real spawn would have written is ABSENT, and a second case plants a secret-shaped var in the parent env and asserts it is `ABSENT` from the async child's output — mirroring the sync tail's own non-tautological pins. |
...
| `packages/library/studio/template-library.ts:519-531` | recursive read | `GET` scaffold `:id` | guarded `[read]` | `id` must match a name the server's own `listProjectStarters` `readdirSync` produced. |
| `packages/library/bridge-studio-community.ts` fetch arm `rmSync` ×1, and `packages/library/studio/community-install.ts` `existsSync` 3 -> 4 (M6-D, ruling 477 security review) | `rmSync` ×1 (rollback), `existsSync` ×1 (occupancy probe) | `POST /api/studio/community/skill/:id/install`, fetch arm | guarded `[read]` | Both take the SAME path the row above constructs — `vendoredPackageDir(forgeRoot, 'skill', id)` — which slug-validates the id, boundary-checks the resolved path and realpaths an existing directory. The `rmSync` removes ONLY the directory `vendorFetchedPackage` just returned, and only when `installSkillPackage` throws after a successful vendor: without it a failed install leaves a stranger's package in the repo-tracked tree, where the next install of the same id would route to the vendored pipeline and treat it as forge's own. Nothing request-derived reaches either call except `id`, which `assertSkillSlug` has already refused if it is traversal-shaped, non-slug or over-length — at the top of `routeCommunityInstall`, before any lookup. |
| `packages/flows/gh-pinned.ts` (M6-D, operator ruling 597(a), bead `forge-8vfn.7.6.15`) | `execFileSync('git', remote get-url / remote)` ×1, `execFileSync('gh', …)` ×1 | the PR lifecycle — reached from a cycle, never from a request body | accidentally-safe | **Fixed argv, no shell, and nothing request-derived reaches either call.** The `git` invocation is `['-C', worktreePath, 'remote', 'get-url', <remote>]`, where `worktreePath` is the scheduler's own worktree and `<remote>` is either the literal `'origin'` or a name `git remote` just printed — not a caller's string. The `gh` invocation is kernel's own `defaultExec`, re-stated here only so the token-read memo has something to fall through to; its argv is built by `gh-identity.ts`, and the **credential rides in the child ENV, never in argv** (`ps` shows argv to every user on the host). **Read-only in effect**: this file writes no path and creates no file; the owner it derives selects a CREDENTIAL, which is why the parse insists on `github.com` — a GitLab URL must not choose a GitHub identity — and why an owner is matched against `^[A-Za-z0-9][A-Za-z0-9._-]*$` before it can reach a `--user` argument. |
| `packages/library/studio/community-fetch-package.ts` `vendorFetchedPackage` (M6-D, operator ruling 477) | `mkdirSync`/`writeFileSync` ×1 each, `renameSync` ×1, `rmSync` ×1 | `POST /api/studio/community/skill/:id/install`, for a row whose package is not yet on disk | guarded `[read]` | **Two independent guards, and the second is the one that matters.** The destination is `vendoredPackageDir(forgeRoot, 'skill', id)` — the same construction the rest of this surface uses: `assertSkillSlug` on the id, a boundary check on the resolved path, and a realpath identity check when the directory exists. The staging directory is that path plus a `randomBytes` suffix, so it inherits all three. **The bytes themselves are the new surface**: a package fetched from a third-party repo carries its own relative paths, and `../escaped.md` in an upstream tree is an ordinary thing to find there. Every file path is therefore `resolve`d against the staging root and refused unless it is INSIDE it — `startsWith(resolve(staging) + sep)`, never a string test on the untrusted path. `leaves nothing behind when a file in the package cannot be written` is that refusal's test, and it asserts the destination is absent afterwards as well: the write stages then renames, so a refusal part-way leaves no half-vendored package for `routeCommunityInstall` to later mistake for an installable one. `rmSync` only ever removes that staging directory, on the failure path. |
| `packages/flows/pr-branch-sync.ts` `countRevs` (M6-D, bead `forge-8vfn.7.6.10`) | `execFileSync('git', rev-list --count)` ×1 | the dev-loop close, reached from the cycle — never a request body | accidentally-safe | Fixed argv. The ONLY interpolated value is `${localHead}..${originHead}`, and BOTH sides are 40-char SHAs that `revParse` just read out of git itself in this same function — not a branch name, not a path, and nothing a caller supplies. `cwd` is the worktree the close is already operating in. Read-only: `rev-list --count` writes nothing, and its only consequence is the WORDING of a failure message (ahead-only vs genuinely diverged) — a failed count returns `null`, which reads as "unknown" and never as zero, so a broken git cannot manufacture a shape. |
| `packages/projects/project-create.ts` (W7-B6) | `execFileSync('git', init/add/commit)` ×3 | `POST /api/studio/projects/create` body `name` (slugified) | guarded `[read]` | Fixed argv (no request-derived tokens beyond the commit message's already-`SLUG_RE`-validated `id` + allowlisted `appType`); `cwd` is the server-built `.staging-<id>-<rand>` under the config-resolved projectsRoot — `id` passes `SLUG_RE` + `isReservedId` before any path construction, `<rand>` is server entropy. `stdio:'ignore'`, no shell. |
| `packages/projects/project-create.ts` (W7-B6, projects-35) | `lstatSync` ×2 | same route | guarded `[read]` | Read-only existence probes on `projects/<id>` + `brain/projects/<id>` (fixed roots, `SLUG_RE`-validated id) whose ONLY consequence is a fail-safe REFUSAL — the create never deletes or writes through either probe (the old `rmSync` reconcile these replace was the projects-35 data-loss defect). |
| `packages/projects/project-create.ts` (`sweepStaleCrossIdStagingLeftovers`, bead forge-9fp) | `readdirSync` ×1, `lstatSync` ×1, `rmSync` ×1 (baseline `readdirSync` 2→3, `lstatSync` 2→3, `rmSync` 3→4) | NOT a bridge route directly — reached from `POST /api/studio/projects/create` via this file's own `runCreate`, unconditionally on every create, for BOTH `projectsRoot` and `brainProjectsRoot` | guarded `[exec]` | **M7-C 2026-09-25 (T2 review follow-up, lane-ratified under ruling 666) — classifying growth already on this branch from bead forge-9fp's own fix (`3a152334`), never written up here or accepted into the baseline until now.** Sibling to `sweepStagingLeftovers` immediately above (which sweeps only THIS create's own id's `.staging-<id>-*` leftovers): this function opportunistically sweeps every OTHER id's stale `.staging-*` dir too, since nothing else in forge ever does and they are otherwise invisible (dot-prefixed, filtered from every project/brain listing). No request-derived value reaches any of the three sinks: `root` is one of the two same fixed, config-derived roots every sibling call in this function already uses (`projectsRoot` / `brainProjectsRoot`, never caller input), and every name it acts on comes from `readdirSync(root)` itself — an enumeration of what the SERVER's own filesystem already contains, not anything a caller supplied. Two independent gates bound the blast radius before `rmSync` runs: the name must literally start with the constant prefix `.staging-` (so an operator's own dir, or a real project id, can never match), and the entry's `lstatSync(full).mtimeMs` must be OLDER than `STALE_STAGING_AGE_MS` (1 hour) — young enough to be a live concurrent create's own staging dir is left alone by construction, not by a lock, so this function cannot race a sibling request's in-flight staging. `lstatSync` (not `statSync`) is deliberate: a symlinked `.staging-*` entry is identified as itself, never followed, before the `rmSync` that acts on it. Both `readdirSync` and `lstatSync` swallow their own failure (root absent / entry vanished mid-race) and skip rather than throw, since a concurrent sweep or create winning the same race is expected, not exceptional. Pinned by `packages/projects/tests/regression/project-create-atomicity.test.ts` (`0c521b85`, "red — stale cross-id .staging-* leftovers never swept") against a planted stale dir (swept) and a planted RECENT dir (left alone, proving the age gate is load-bearing and not merely present). |
...
| `packages/flows/finalize-merged.ts:238` | `readdirSync`/`rmdirSync` | `POST /api/verdict` approve → `finalizeAfterMerge` → `pruneMergedWorktrees`: manifest `worktree_path` + `initiative_id` | guarded `[exec]` | `worktree_path` is contained by `isContainedWorktreePath` in `finalizeMergedReadyForReview` before any use; `initiative_id` must pass `isSafeCycleId` (one safe segment) or nothing is touched. Only an EXISTING, EMPTY `<dirname(worktree_path)>/wi/<id>/` is removed (non-recursive `rmdirSync`). Bead forge-8vfn.8.1.3. |
| `packages/kernel/process-liveness.ts:55` (`isProcessRunning`) | `readFileSync` | **none** | not-request-derived `[read]` | Reads `<procRoot>/<pid>/stat`. `procRoot` defaults to `/proc` (callers pass a fixture only in tests), and `pid` comes from the daemon's own pid file (`readPid`, int-parsed) or a live child; the function itself refuses anything but a positive integer before the path is built. Reachable from a bridge route only via `spawnServeDetached`/`daemonState`, which never take a request field. Bead forge-8vfn.8.1.6. |
| `packages/knowledge/brain-write-lease.ts` (`acquireBrainWriteLease`'s holder record) | `statSync` 0 -> 1, `writeFileSync` 0 -> 1, `unlinkSync` 0 -> 1 (plus `readFileSync`) | NOT a bridge route — the brain-write lease every brain writer (reflector, KB drain/consolidate/brain-fix) takes | `accidentally-safe` -> **not request-derived** `[read]` | **New in M7-A row 117, bead `forge-8vfn.8.1.21`, and the reason the baseline grows here.** `proper-lockfile` reclaims a lock on mtime staleness alone, and under load a LIVE holder's lease was reclaimed (two brain writers). The holder now records its PID beside the lock and a live PID refuses before any reclaim. Every path is `lockfilePath` (default `${brainRootDir(forgeRoot)}.lock`, `forgeRoot` config-derived; a test-only override otherwise) or that path plus the module constant `.holder-pid` — no caller-supplied value is joined into either. `statSync` reads the lock's age, `readFileSync`/`writeFileSync` the holder's own PID, `unlinkSync` removes it on release. Classified `[read]`: the property the tests assert is the refusal set (`packages/knowledge/tests/unit/brain-write-lease.test.ts`), not an escape; **the precondition that would make this live** is a route passing a caller-supplied `lockfilePath`, which no route does. |
| `packages/flows/cycle-pr-open.ts` (`openPrInline`) | `existsSync` 0 -> 2 (and `packages/flows/cycle-helpers.ts` `existsSync` 6 -> 4) | NOT a bridge route — the develop flow's PR-open step | `accidentally-safe` -> **not request-derived** `[read]` | **Relocated and conserved, M7-A row 121, bead `forge-8vfn.8.1.24`:** `openPrInline` moved out of `cycle-helpers.ts` unchanged in its path handling when the row grew that file past the 800-line cap, so no new sink appeared. The two probes are `worktreeDemoMdPath(input.worktreePath, input.initiativeId)` and `resolve(input.worktreePath, '.forge', 'pr-description.md')`, each a boolean prerequisite check. `worktreePath` is the scheduler-claimed worktree, containment-checked upstream by `isContainedWorktreePath`, and every other segment is a literal or the manifest's initiative id. |

### Fixed in this sweep

| file:line | op | request field | was | evidence |
|---|---|---|---|---|
| `packages/knowledge/brain-paths.ts:74` (`resolveKbBrainDir`) | `existsSync` | `:id` on every `/api/studio/kbs/*` route | unguarded `[exec]` | `resolve()` + `existsSync()`, no realpath, no identity, no `nlink`, on **both** candidate roots. The choke point for the whole KB family. Now `resolveGuardedPath` on each root with `kbId` as its own segment. |
| `packages/knowledge/kb-graph.ts` themes/`_raw`/`_guidance`/`INDEX.md`/node-leaf joins | `readdirSync`/`readFileSync` | `GET /api/studio/kbs/:id`, `/nodes/:nodeId` | unguarded `[exec]` | Bare `join()` off an unresolved kb dir. A **real** `brain/<id>/` whose `themes` was a symlink leaked outside theme bodies into the graph response. Now every nested join goes through a `guardedKbPath` helper. |
...
AT-9 (`apps/forge/tests/integration/ui-bridge-onboarding-start.test.ts`) before the fix. **Closed** by
...
`:390`), `packages/flows/finalize-merged.ts:301`, `packages/flows/drain-fix-loop.ts:136`
...
`apps/forge/bridge-studio.ts` (×4), and `packages/flows/mint-triggered-initiative.ts`
...
(`packages/flows/scheduler.ts`, `orchestrator/demo-fix-loop.ts`,
`packages/flows/gate-fix-loop.ts`, `packages/flows/bridge-studio-runs.ts:560`) were
...
`resolveContainedProjectDir` ATs above): `packages/flows/tests/regression/manifest-path-fields.test.ts`'s
...
branch is itself pinned, by mutation, in `packages/flows/tests/regression/manifest-path-fields.test.ts`
...
Pinned by `packages/agents/tests/integration/agent-run-dispatch.test.ts` (the bead forge-c6h characterization
...
`apps/forge/tests/contract/ui-bridge-agent-dispatch-projects-root.test.ts` (the argv-builder seam:
...
the `packages/agents/tests/integration/agent-run-dispatch.test.ts` AT-D7 fixtures then built their session
...
`scripts/check-request-path-sinks.mjs` flagged `packages/library/studio/skill-install-ledger.ts`
...
`writeFileSync` 2→4; `packages/sessions/studio/session-transcript.ts`: `readdirSync`
...
| `packages/flows/flow-run-requests.ts:77` | `POST /api/hooks/:hookId` | The path's only variable component is read from the matched flow's own `flow.yaml` (operator config), not from the HTTP payload; payload fields are written as JSON *content* only. `[read]` |
...
- `packages/knowledge/kb-health.ts` shares `resolveKbBrainDir` (now fixed) but is reflector-internal; no HTTP route reaches it today. Flagged for future exposure.
...
| file | sites | class |
|---|---|---|
| `packages/knowledge/brain-paths.ts` | `resolveKbBrainDir` | **fixed** |
| `packages/knowledge/kb-graph.ts` | themes / `_raw` / `_guidance` / `INDEX.md` / node-leaf / `listPendingGuidance` / `deleteGuidanceFile` | **fixed** |
| `packages/sessions/studio/session-transcript.ts` | `safeReadFileInSession`, `listDirEntries`, `listDirEntriesTyped` (R4-21 T3, BLOCKER-1 fix — `deriveFilePackage`'s recursive `package/` walk) | guarded |
| `packages/library/studio/hook-library.ts` | `resolveHookScriptPath`; `hookDir` / `hookYamlPath` | guarded; the latter two **fixed at the call sites** |
...
| `packages/projects/project-config.ts` | `readAgentInstructionsFile`, `readQualityGateSidecar` | unguarded → `forge-2zz` |
| `orchestrator/studio/skill-library.ts`, `community-install.ts`, `community-index.ts` | package install/read | unguarded → `forge-q80` |
| `packages/flows/requeue-resume.ts`, `flow-artifacts.ts`, `logging.ts` | manifest-carried `worktree_path` / `cycle_id` | **fixed in SEC-02** (`forge-d1f`) |
| `packages/flows/mint-triggered-initiative.ts` | mints a queue manifest from a flow's `project` field (background flow-trigger sweep, fed by `PUT /api/studio/flows/:id`) | **fixed in SEC-03** — previously bypassed `writeManifest` entirely; see "Fixed in SEC-03" above |
| `packages/flows/review-comments.ts`, `enqueue-*.ts`, `project-create.ts` | charset-gated | guarded |
...
| `packages/library/studio/connection-library.ts`, `daemon.ts`, `agent-dispatch.ts` | — | no request-derived path reaches a filesystem call |
...
`packages/sessions/interactive-finalizers.ts` grew three new tracked sinks
...
| file:line | op | request field | class | evidence |
|---|---|---|---|---|
| `packages/sessions/interactive-finalizers.ts` (`commitRegistryDraft`) | `writeFileSync` | none — `tempPath = join(dirname(destPath), '.registry.yaml.tmp-<random>')`, where `destPath = resolveGuardedPath(forgeRoot, ['studio','community','registry.yaml']).realPath`; the random suffix is `randomBytes(6)` generated IN-PROCESS, never request-derived | guarded `[read]` | `dirname(destPath)` is a directory the guard already identity-verified; the leaf filename is a fixed literal prefix plus server-generated entropy, never a caller-supplied string — there is no path segment here an attacker can influence at all. |
| `packages/sessions/interactive-finalizers.ts` (`commitRegistryDraft`) | `renameSync` | none — both arguments are `tempPath` (as above) and `destPath` (the same already-guarded `realPath` used throughout the rest of this function, e.g. `draftGuard`/`registryGuard`) | guarded `[read]` | `renameSync` only ever moves the just-written temp file (created by the `writeFileSync` row above, same directory) onto the guard-verified destination — the atomic swap step of a check-then-write-then-atomically-publish sequence, not a caller-influenced path operation. |
| `packages/sessions/interactive-finalizers.ts` (`commitRegistryDraft`) | `unlinkSync` | none — `tempPath`, identical to the `writeFileSync` row | guarded `[read]` | Best-effort cleanup ONLY on the `renameSync` failure path (orphaned-temp-file removal); wrapped in its own `try/catch` so a failed cleanup never masks the real error. Same non-request-derived path as the write. |
...
`packages/sessions/interactive-finalizers.ts` `writeFileSync`/`renameSync`/
...
correct): `packages/sessions/tests/unit/interactive-session.test.ts` pins the pure
...
`packages/sessions/interactive-finalizers.ts` `readFileSync` at 1,
...
| `packages/sessions/bash-fence.ts` (`realish`) | `realpathSync` | the resolved candidate of a `Bash` tool call's OWN command text — a redirection target / write-utility operand / `dd of=` / `cd` operand (model-derived, request-shaped), resolved for IDENTITY only: walked up to its deepest EXISTING ancestor's realpath + the literal remainder, then COMPARED against the turn's already-realpath'd `writeRoots`; never opened, never written through | guarded `[read]` | This IS the Bash half of the write-root containment check (bead forge-w08 — the sibling of `isUnderWriteRoot`'s row above): a `realpathSync` that fails all the way to the filesystem root yields `null` → DENY. Only reachable when a fenced kind opts into `turnSpec.bashFence: inspect` (`authoring` today); every other fenced kind denies Bash outright and never reaches this sink. Verified `[exec]` by `packages/sessions/tests/unit/bash-fence.test.ts` (an in-root symlink to an outside dir cannot be written through; relative escapes and normalised traversal are denied; the REAL `authoring` descriptor through `runInteractiveTurn` denies an out-of-root `printf > file`). |

`node scripts/check-request-path-sinks.mjs --write` accepted this delta —
`scripts/request-path-sinks.baseline.txt` now records
`packages/sessions/bridge-studio-lifecycle.ts` `openSync` at 1 and `statSync` at 2, and
`packages/sessions/bash-fence.ts` `realpathSync` at 1.
...
1. **A refused refresh writes nothing at all.** A missing/invalid `GH_TOKEN`,
   ...
   code, in `packages/library/tests/integration/community-refresh-cmd.test.ts` (R1/R2/R3/R4/R6) and through
   the real bridge in `packages/library/tests/integration/bridge-studio-community-refresh.test.ts`
...
2. **The credential never reaches a path, a log, a response or a written
   ...
   (`packages/kernel/spawn-env.ts`), so no spawned agent child ever sees it.
...
`packages/flows/flow-runner.ts` was carved into `packages/flows/flow-runner.ts`
(QUARRY owns it to `flows`, disposition `rewritten`). One `(file, sink)` row
changed path and nothing else.

| was | is now | sink | why it moved |
|---|---|---|---|
| `packages/flows/flow-runner.ts` | `packages/flows/flow-runner.ts` | `readFileSync` 1 | the runner is flows' runtime; `SPEC.md` §2 puts the Station engine in the package, and the move is what lets it stop importing `orchestrator/` |
...
|---|---|---|---|
| `packages/stations/phases/project-manager.ts` | `existsSync` | 3 | 1 |
| `packages/stations/phases/project-manager.ts` | `readFileSync` | 4 | 2 |
| `packages/stations/phases/project-manager.ts` | `readdirSync` | 1 | **row deleted** |
| `packages/stations/phases/project-manager.ts` | `writeFileSync` | 2 | 1 |
| `packages/stations/phases/pm-prompt-context.ts` | `existsSync` / `readFileSync` / `readdirSync` | — | 2 / 2 / 1 |
| `packages/stations/phases/pm-decomposition-doc.ts` | `writeFileSync` | — | 1 |
...
their own branch changed. `packages/stations/gates/docs-gate.ts` existed and was
...
|---|---|---|---|---|
| `packages/stations/phases/merge-boundary.ts` | `execFileSync` | `input.worktreePath` as `cwd` | guarded `[exec]` | fixed argv (`git diff --name-only main...HEAD`), no request-derived argument; `cwd` is the cycle's own worktree, the same shape `phases/integrate.ts`'s `gitCapture` already carries |
| `packages/stations/gates/docs-gate.ts` | `existsSync` (2) | the markdown paths in the branch diff | guarded `[read]` | every path is resolved by `changedMarkdownFiles` through `guardedFile(worktreePath, rel.split('/'), 'read')` — the worktree is a FIXED root, each path segment is its own element, and a path that will not resolve inside it is DROPPED rather than read. The gate is only ever handed an already-contained absolute path |
| `packages/stations/gates/docs-gate.ts` | `readFileSync` (1) | as above | guarded `[read]` | as above; the same resolution, one call site upstream, so the gate has no unguarded entry point of its own |

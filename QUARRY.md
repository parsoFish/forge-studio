...
| `agents` | 30 | 8,798 | **11,030** | **Re-seeded 9,600 → 11,030 (M4-agents s3, T1 ruling 76's method applied to the routes carve).** A transfer is priced at what the HOST GAVE UP: `cli/ui-bridge.ts` 3,704 → 2,274 = **−1,430**, so 9,600 + 1,430. Measured on the carve PR's own tree: package prod 9,802 → **11,570**, i.e. **540 over** — being PR #337's 202 (carried, disclosed there) plus this carve's own **+338**: the `AgentsRouteDeps` / `AgentHistoryDeps` / `AgentRunStateDeps` declarations the rank boundary forces the package to state, the five `RouteEntry` rows, and the module headers recording the seam decisions. No duplicated logic — the request-path-sink census is conserved kind for kind (host −14, package +14, host-minus-package = 0, COMMON §15.73). **No growth is granted; the payback obligation is 540**, due at Task 7, and reported line-for-line against the measured number rather than trimmed to fit this cell. Earlier history: seeded from the quarried total rounded to the next 500 (9,000); re-seeded 9,000 → 9,600 by rulings 75 + 76 for the band-agent-run and agents-md-compose carve-ins. |

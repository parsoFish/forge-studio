...
| `rewritten` | **cannot** move without a behaviour change; stays where it is until rewritten | 100 |
...
| `factory` | 15 | 2,625 | **2,297** | ratified 2,297 — F3 re-attribution of the station executor to `stations` (operator ruling items 81/83); see git history for prior raises. |
| `stations` | 40 | 11,475 | **11,268** | ratified 11,268 — D8 (mfv5.2.1) collectCapturedMedia binds `.webm` → before/afterVideoSrc and `.filmstrip.png` → the side's image, +37 on row 112's 11,231, lane-ratified under ruling 666; see git history for prior raises. |
...
| **total** | **464** | **126,618** |  | F3 (operator ruling, items 81/83): +2 files / +150 lines — `class-profile-port.ts` and `stations/index.ts`, the only genuinely new content in an otherwise pure `factory → stations` transfer (10,905 lines moved, re-attributed, no change to this total). |
...
| packages/stations/phases/integrate.ts | stations | verbatim | 330 |
...
| packages/factory/demo.ts | factory | verbatim | 279 |

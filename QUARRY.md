...
| `flows` | 82 | 23,382 | **23,182** | ratified 23,182 — row 86 post-merge worktree prune, id-segment guarded (forge-8vfn.8.1.3), lane-ratified under ruling 666; see git history for prior raises. |
...
| **total** | **457** | **125,137** |  | F3 (operator ruling, items 81/83): +2 files / +150 lines — `class-profile-port.ts` and `stations/index.ts`, the only genuinely new content in an otherwise pure `factory → stations` transfer (10,905 lines moved, re-attributed, no change to this total). |

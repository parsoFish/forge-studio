...
| `agents` | 30 | 8,798 | **9,600** | seeded from the quarried total, rounded up to the next 500. **Re-seeded 9,000 → 9,600 (M4-agents s3, T1 rulings 75 + 76).** PURE RE-ATTRIBUTION, not a growth grant: 9,000 + 233 (the Agent kind out of `orchestrator/studio/registry.ts`, #329) + 242 (`orchestrator/band-agent-run.ts`) + 109 (`packages/projects/agents-md-compose.ts`), rounded. **A transfer is priced at what the HOST GAVE UP** (ruling 72's method, reaffirmed as ruling 76) — the destination file is larger than the source, and that difference is growth, not transfer: `band-agent-run.ts` 242 → 368 and `agents-md-compose.ts` 109 → 116, being the `BandAgentDeps` port declaration the rank boundary forces the package to state (~60 lines that could not exist while the file sat in `orchestrator/` importing factory and flows directly), the header's record of the port decision, and `dispatchStandaloneBand` — the dispatch branch's policy, moved here from `agent-run.ts`. Measured on the band-carve PR's own merged tree: **package prod 9,837 against this 9,600 — 237 over, merged disclosed under ruling 76.** Growth over M4 entry is 877 against an entry headroom of 669 = **208 over**, being s2's two P0 incident fixes (rulings 68/69: `spawn-marker.ts` 276 + `dispatch-terminal.ts` 194) plus this carve's port. **No growth is granted and the payback obligation is 208**, due at Task 7 (the `agent-run.ts` split + the routes carve's cull), measured against 9,600 plus the routes carve's exact host delta, which that PR adds to this cell from its own numbers (COMMON §15.76). If the cull cannot pay 165 without cutting port declarations or guards, the residual is reported line-for-line and re-ruled on the measured number — never trimmed to fit this figure. |
...
|---|---|---|---|
| packages/agents/agent-run.ts | agents | verbatim | 925 |

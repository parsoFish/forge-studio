...
| `flows` | 82 | 23,370 | **23,170** | ratified 23,170 — row 86 post-merge worktree prune (forge-8vfn.8.1.3), lane-ratified under ruling 666; see git history for prior raises. |
...
| **total** | **456** | **125,031** |  | F3 (operator ruling, items 81/83): +2 files / +150 lines — `class-profile-port.ts` and `stations/index.ts`, the only genuinely new content in an otherwise pure `factory → stations` transfer (10,905 lines moved, re-attributed, no change to this total). |
...
| packages/flows/finalize-merged.ts | flows | verbatim | 507 |

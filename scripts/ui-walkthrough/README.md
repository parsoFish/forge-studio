...
| `--min-routes <n>` | harness sanity floor (default 40 for a live full crawl, 1 under `--only`, 0 under `--from`): fewer crawled routes is a **harness error** (exit 2 — no verdict, and no baseline written) — together with the bridge `/api/health` precheck it stops "the bridge was down, every page rendered its empty state" from reading as green or being baselined |

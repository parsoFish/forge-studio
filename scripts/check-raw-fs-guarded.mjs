...
export const ALLOWLIST = [
  ...
  { file: 'cli/bridge-studio-kbs.ts', line: 1120, sink: 'existsSync',
    reason: 'RETAIN (contained + boolean): DELETE /api/studio/kbs/:id — id is URL-derived (newly tainted) but SLUG_RE-gated at line 1110 (blocks / and ..), and dir = resolveKbBrainDir(forgeRoot, id) runs the per-segment realpath identity walk (choke-point containment; see resolveKbBrainDir + resolveGuardedPath in orchestrator/brain-paths.ts) returning null on any escape; existsSync(dir) is a boolean 404 probe, no bytes flow. Same manifest-path-guard category as the isContainedWorktreePath rows above. (Line-drift remap from 1068 — R6-08 WI-1/WI-2 inserted code earlier in the file.)' },
  { file: 'cli/bridge-studio-kbs.ts', line: 1045, sink: 'mkdirSync',
    reason: 'CREATE-LITERAL-SUBDIR: kbDir = kbGuard.realPath (resolveGuardedPath) and the route 409s if kbGuard.exists (line 1039), so this runs only create-mode on a FRESH dir; the appended leaf `themes` is a literal — a just-created dir cannot host a pre-planted symlink. (Line-drift remap from 993.)' },
  { file: 'cli/bridge-studio-kbs.ts', line: 1046, sink: 'mkdirSync',
    reason: 'CREATE-LITERAL-SUBDIR: as line 1045, literal `_raw` subdir under the freshly-created guarded kbDir. (Line-drift remap from 994.)' },

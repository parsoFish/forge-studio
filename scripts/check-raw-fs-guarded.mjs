...
export const ALLOWLIST = [
  ...
  { file: 'cli/ui-bridge.ts', line: 2038, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: spawnAgentTurn — sessionId isSafeRunId-gated at line 1986 (SAFE_RUN_ID_RE + explicit .. check); `_<logPrefix>-<sessionId>` under trusted forgeRoot/_logs. (Line-drift remap from 2036 — R4-21 WI-2, +2 lines total, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 2446, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: sibling spawn helper — sessionId/runId isSafeRunId-gated before this log-dir create under trusted forgeRoot/_logs. (Line-drift remap from 2439 — R4-21 T3 BLOCKER-2 fix added the MAX_AUTHORING_PROMPT_LENGTH const block earlier in the file, +7 lines; same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 3415, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: onboarding — sessionId SAFE_ID_RE-gated at line 3409; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3408 — R4-21 T3 BLOCKER-2 fix, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3416, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created, sessionId SAFE_ID_RE-gated. (Line-drift remap from 3409 — R4-21 T3 BLOCKER-2 fix, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3424, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as the row above. (Line-drift remap from 3417 — R4-21 T3 BLOCKER-2 fix, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3452, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: authoring (R4-21 T3 BLOCKER-2 fix) — writeAuthoringSession, byte-for-byte the same shape as writeOnboardingSession\'s rows directly above: sessionId SAFE_ID_RE-gated at line 3446; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed.' },
  { file: 'cli/ui-bridge.ts', line: 3453, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created (line 3452), sessionId SAFE_ID_RE-gated (R4-21 T3 BLOCKER-2 fix).' },
  { file: 'cli/ui-bridge.ts', line: 3458, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as line 3453 (R4-21 T3 BLOCKER-2 fix).' },
  { file: 'cli/ui-bridge.ts', line: 4016, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: onboardingParent = join(realProjectDir, "_onboarding") where realProjectDir is realpathSync-resolved; this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification that refuses a symlinked _onboarding. (Line-drift remap from 3975 — R4-21 T3 BLOCKER-2 fix added the POST /api/studio/authoring/start route + writeAuthoringSession earlier in the file; same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 4126, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: authoringParent = join(realProjectDir, "_authoring") where realProjectDir is realpathSync-resolved (R4-21 T3 BLOCKER-2 fix) — byte-for-byte the same shape as the onboardingParent row above: this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification (lines 4127-4128) that refuses a symlinked _authoring.' },
  { file: 'cli/ui-bridge.ts', line: 4180, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(join(repoPath, ".forge","demo","demo.lock.json")) picks the create/update mode default; body.project was already proven contained by resolveDemoSessionDir above; boolean-only, no bytes flow. (Line-drift remap from 3975→4044 chain, now 4180 — R4-21 T3 BLOCKER-2 fix added the POST /api/studio/authoring/start route earlier in the file, same cause as the rows above.)' },

...
export const ALLOWLIST = [
  ...
  { file: 'cli/ui-bridge.ts', line: 1892, sink: 'mkdirSync',
    reason: 'RETAIN (isSafeRunId-gated logdir-create): runId = `_agent-${slug}-${newRunStamp()}` with slug URL-derived (newly tainted), but isSafeRunId(runId) (SAFE_RUN_ID_RE + explicit .. check) THROWS a few lines above BEFORE this recursive mkdir of the run\'s own log dir under trusted ctx.logsRoot — the SAME deliberate guard-symmetry check its already-allowlisted siblings at (now) 2107/2515 carry. (Merge remap: feat/w6-b12-kb-drain merging parsoFish/main, same +2 cause as the ui-bridge.ts block below — main\'s own current number (1890) + this branch\'s 2-line ui-bridge.ts insertion; verified against the merged tree.) (Line-drift remap from 1841 — W6-B2\'s ensureSessionTail helper + ctx call-site comment, cumulative +32. Further remap from 1873 — W6-B2 review fix\'s +1 import line, same cause as the cli/ui-bridge.ts rows above.) (Merge remap from 1874 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 1876 -- the feat/w6-b5-model-seam merge with parsoFish/main (W6-B5 kickoff model-tier seam combined with mains own concurrent ui-bridge.ts/architect-runner.ts edits), +2 lines; same function, same guard, unchanged.)' },

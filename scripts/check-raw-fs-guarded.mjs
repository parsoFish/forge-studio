...
export const ALLOWLIST = [
  ...
  { file: 'cli/bridge-studio-kbs.ts', line: 212, sink: 'mkdirSync',
  ...
  { file: 'cli/bridge-studio-runs.ts', line: 815, sink: 'existsSync',
  ...
  { file: 'cli/bridge-studio-runs.ts', line: 819, sink: 'existsSync',
    reason: 'QUEUE-PROBE: as line 797, trusted queuePaths.done.' },
  { file: 'cli/bridge-studio-runs.ts', line: 825, sink: 'existsSync',
    reason: 'QUEUE-PROBE: as line 797, trusted queuePaths.pending.' },
  { file: 'cli/bridge-studio-runs.ts', line: 835, sink: 'existsSync',
    reason: 'QUEUE-PROBE: candidate = join(<trusted queuePaths.readyForReview|failed>, INIT_ID_RE filename); boolean.' },
  { file: 'cli/bridge-studio-runs.ts', line: 851, sink: 'writeFileSync',
  ...
  { file: 'cli/bridge-studio-kb-drain.ts', line: 262, sink: 'mkdirSync',
  ...
  { file: 'cli/bridge-studio-kb-drain.ts', line: 265, sink: 'writeFileSync',
  ...
  { file: 'cli/bridge-studio-kb-drain.ts', line: 276, sink: 'existsSync',
  ...
  { file: 'cli/bridge-studio-kb-drain.ts', line: 278, sink: 'readFileSync',
  ...
  { file: 'cli/bridge-studio-kb-drain.ts', line: 353, sink: 'existsSync',
    reason: 'LOG-READ (SERVER-ENUMERATED, W7-B2 knowledge-20): readConsolidateRunRow — its runId comes ONLY from the GET /api/studio/kbs/:id/runs route\'s own readdirSync enumeration of `_logs` dir names (never a request parameter); `_brainfix-<runId>/events.jsonl` is a single enumerated segment under trusted forgeRoot/_logs; boolean existence probe in the same ternary as the read below.' },
  { file: 'cli/bridge-studio-kb-drain.ts', line: 354, sink: 'readFileSync',
    reason: 'LOG-READ (SERVER-ENUMERATED, W7-B2 knowledge-20): readConsolidateRunRow\'s events.jsonl read — same server-enumerated runId as the existsSync row immediately above (same expression), same class as cli/bridge-studio-kbs.ts:151 (readBrainFixState).' },
  { file: 'cli/bridge-studio-kb-drain.ts', line: 492, sink: 'mkdirSync',
  ...
  { file: 'cli/bridge-studio-kb-drain.ts', line: 493, sink: 'writeFileSync',
    reason: 'LOGDIR-WRITE (TRUSTED-AT-CONSTRUCTION, W7-B2 knowledge-14): requestKbDrainCancel\'s cancel.json flag write — same runId trust chain as the mkdirSync row immediately above (same function, same call).' },
  { file: 'cli/bridge-studio-kb-drain.ts', line: 497, sink: 'existsSync',
    reason: 'LOG-READ (TRUSTED-AT-CONSTRUCTION, W7-B2 knowledge-14): isKbDrainCancelRequested — boolean probe of the cancel flag, called from the drain loop with its OWN server-minted runId (and from the cancel route with the enumerated one, per the :488 row); no bytes flow.' },

  // ---- orchestrator/interactive-session.ts ----
  { file: 'orchestrator/interactive-session.ts', line: 686, sink: 'existsSync',
  ...
  { file: 'orchestrator/interactive-session.ts', line: 688, sink: 'readFileSync',
  ...
  { file: 'orchestrator/interactive-session.ts', line: 705, sink: 'writeFileSync',
  ...
  // ---- orchestrator/architect-runner.ts ----
  { file: 'orchestrator/architect-runner.ts', line: 1423, sink: 'existsSync',
  ...
  { file: 'orchestrator/architect-runner.ts', line: 1425, sink: 'readFileSync',
  ...
  { file: 'orchestrator/architect-runner.ts', line: 1434, sink: 'writeFileSync',
  ...
  { file: 'orchestrator/architect-runner.ts', line: 1612, sink: 'existsSync',
  ...
  { file: 'orchestrator/architect-runner.ts', line: 1614, sink: 'readFileSync',
  ...
  { file: 'cli/bridge-studio-kbs.ts', line: 1224, sink: 'existsSync',
    reason: 'RETAIN (contained + boolean, W7-B2 knowledge-29): the kb-detail route\'s pinned-guidance queue listing — gDir = resolveKbBrainDir(forgeRoot, kbId) (the per-segment realpath identity walk, choke-point containment; kbId KB_ID_RE-gated at the route) + the literal `_guidance` leaf; existsSync is a boolean probe, no bytes flow. Same containment category as the DELETE route\'s existsSync row below.' },
  { file: 'cli/bridge-studio-kbs.ts', line: 1225, sink: 'readdirSync',
    reason: 'RETAIN (contained + names-only, W7-B2 knowledge-29): same gDir as the row above — readdirSync enumerates entry NAMES only (filtered to `*.md`, returned as display strings); no file contents flow and no name is joined back into any further fs path.' },
  { file: 'cli/bridge-studio-kbs.ts', line: 1475, sink: 'existsSync',
  ...
  { file: 'cli/bridge-studio-kbs.ts', line: 1392, sink: 'mkdirSync',
  ...
  { file: 'cli/bridge-studio-kbs.ts', line: 1393, sink: 'mkdirSync',
  ...
  { file: 'cli/bridge-studio.ts', line: 641, sink: 'existsSync',
    reason: 'LOG-READ (W7-FIX-A3 round-2 finding 11, boolean sibling of line 636): existsSync(literalPath) is the phase-log route\u2019s literal-id probe -- it decides only whether `_logs/<runId>/events.jsonl` exists so findRun (a full queue walk + a second parse of the active run\u2019s event log, refetched per WebSocket event) can be skipped. The path is the SAME lexically resolved, containment-checked value the pre-existing eventsPath rows carry, and the check runs in the SAME expression before the probe (`literalPath.startsWith(safeLogsBase + sep) && existsSync(literalPath)`, && short-circuit). Boolean only -- no bytes flow through it; the read below is the already-allowlisted eventsPath readFileSync. Same disclosed _logs-symlink residual as its siblings; uniform with the log-read family. (Merge remap to 633 -- reconciling fix/w7-a3-loop-closure (W7-FIX-A3 round-2 findings 5+11 above) with parsoFish/main\'s independent W7-FIX-A2 (W7A2-01) CANCELLED_PHASE relocation to the orchestrator status-write seam (+5 net lines earlier in the file); same function, same guard, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs --json` run against the merged tree.) (Merge remap to 641 -- reconciling with fix/w7-a4-identity\'s own W7-FIX-A4 review round 1 kb:null fix (+8 lines earlier in the file, independent edit region in loadProjectsWithMeta), applied on top of the above during the wave-A merge of parsoFish/main; same function, same guard, unchanged -- re-verified by `grep -n readPreflightFixState cli/bridge-studio.ts` and sed -n "527,536p" cli/bridge-studio.ts against the merged tree.)' },
  { file: 'cli/bridge-studio.ts', line: 649, sink: 'existsSync',
    reason: 'LOG-READ (SEC-04 residual #2, boolean pair of line 565): existsSync(eventsPath) on the same lexically-resolved, .. -blocked, symlink-blind URL-derived path — boolean 404 probe, no bytes flow. Same disclosed-_logs-symlink residual as line 565; uniform with the log-read family. (Line-drift remap from 506. Further remap from 513 — forge-3oq, same cause as the row above. Further remap from 521 — W6-B2 review fix, same +24 cause as the row above.) (Merge remap from 545 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Merge remap from 550 - parsoFish/main P1/B1/P2 merged into feat/w6-rv1-initiative-detail: same +1 net cause as the row above; same function, same guard, unchanged - verified by sed -n "551p" cli/bridge-studio.ts against the merged tree.) (Line-drift remap from 551 — W6-CR-1, same +1 import-line cause as the row above; same function, same guard, unchanged — verified by sed -n "552p" cli/bridge-studio.ts.) (Line-drift remap to 560 -- W7-A3 loop closure: findRun in cli/bridge-studio.ts gained an 8-line doc comment (initiativeId fallback, the stable run handle); same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Merge remap from 560 -- W7-A3 (feat/w7-a3-loop-closure) merged with parsoFish/main; same function, same guard, unchanged -- verified via a direct `node scripts/check-raw-fs-guarded.mjs` run against the merged tree.) (Merge remap from 603 -- W7-A4 (identity + not-found hygiene) merged with parsoFish/main (post-W7-A3); same function, same guard, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run against the merged tree.) (Line-drift remap from 612 -- W7-FIX-A3 loop-closure regressions (the phase-log route resolves the run through findRun before building eventsPath, +9 lines above); same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Line-drift remap from 621 -- W7-FIX-A3 round-2 findings 5+11 in cli/bridge-studio.ts: loadAllFlows stopped swallowing a thrown readdirSync into [] (+3 lines) and the phase-log route probes the literal `_logs/<runId>/events.jsonl` before falling back to findRun (+15 lines for rows past it). Same function, same sink expression, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Merge remap to 641 -- reconciling fix/w7-a3-loop-closure (W7-FIX-A3 round-2 findings 5+11 above) with parsoFish/main\'s independent W7-FIX-A2 (W7A2-01) CANCELLED_PHASE relocation to the orchestrator status-write seam (+5 net lines earlier in the file); same function, same guard, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs --json` run against the merged tree.) (Merge remap to 649 -- reconciling with fix/w7-a4-identity\'s own W7-FIX-A4 review round 1 kb:null fix (+8 lines earlier in the file, independent edit region in loadProjectsWithMeta), applied on top of the above during the wave-A merge of parsoFish/main; same function, same guard, unchanged -- re-verified by `grep -n readPreflightFixState cli/bridge-studio.ts` and sed -n "527,536p" cli/bridge-studio.ts against the merged tree.)' },
  { file: 'cli/bridge-studio.ts', line: 662, sink: 'readFileSync',
    reason: 'LOG-READ (SEC-04 residual #2, disposed UNIFORMLY with the _logs log-read family — kbs.ts:223/225, bridge-studio.ts:418/420, architect-runner.ts:1596/1598): phase-log GET /api/runs/<runId>/phases/<node>/log reads readFileSync(resolve(logsRoot, runId, "events.jsonl")). runId is URL-derived but behind a lexical resolve()+startsWith(logsRoot+sep) gate that BLOCKS .. traversal (resolve normalizes, startsWith rejects escape) — symlink-BLIND like the SAFE_ID_RE-gated family, same residual class. NOT wire-reachable: the _logs/<runId> symlink precondition is unplantable via any route (nothing writes attacker-chosen symlinks under _logs). Allowlisted (not guarded) so the whole _logs event-read family stays uniform — no split treatment; the migrate-to-guardedFile follow-up for the family is echoed in openConcerns. (Line-drift remap from 519. Further remap from 526 — forge-3oq, +8 lines. Further remap from 534 — W6-B2 review fix added the LEGACY_SESSION_TERMINAL_PHASES constant earlier in the file, +24 lines; same function, same guard, unchanged.) (Merge remap from 558 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Merge remap from 563 - parsoFish/main P1/B1/P2 merged into feat/w6-rv1-initiative-detail: same +1 net cause as the readPreflightFixState rows above; same function, same guard, unchanged - verified by sed -n "564p" cli/bridge-studio.ts against the merged tree.) (Line-drift remap from 564 — W6-CR-1, same +1 import-line cause as the readPreflightFixState rows above; same function, same guard, unchanged — verified by sed -n "565p" cli/bridge-studio.ts.) (Line-drift remap to 573 -- W7-A3 loop closure: findRun in cli/bridge-studio.ts gained an 8-line doc comment (initiativeId fallback, the stable run handle); same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Merge remap from 573 -- W7-A3 (feat/w7-a3-loop-closure) merged with parsoFish/main; same function, same guard, unchanged -- verified via a direct `node scripts/check-raw-fs-guarded.mjs` run against the merged tree.) (Merge remap from 616 -- W7-A4 (identity + not-found hygiene) merged with parsoFish/main (post-W7-A3); same function, same guard, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run against the merged tree.) (Line-drift remap from 625 -- W7-FIX-A3 loop-closure regressions (the phase-log route resolves the run through findRun before building eventsPath, +9 lines above); same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Line-drift remap from 634 -- W7-FIX-A3 round-2 findings 5+11 in cli/bridge-studio.ts: loadAllFlows stopped swallowing a thrown readdirSync into [] (+3 lines) and the phase-log route probes the literal `_logs/<runId>/events.jsonl` before falling back to findRun (+15 lines for rows past it). Same function, same sink expression, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Merge remap to 654 -- reconciling fix/w7-a3-loop-closure (W7-FIX-A3 round-2 findings 5+11 above) with parsoFish/main\'s independent W7-FIX-A2 (W7A2-01) CANCELLED_PHASE relocation to the orchestrator status-write seam (+5 net lines earlier in the file); same function, same guard, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs --json` run against the merged tree.) (Merge remap to 662 -- reconciling with fix/w7-a4-identity\'s own W7-FIX-A4 review round 1 kb:null fix (+8 lines earlier in the file, independent edit region in loadProjectsWithMeta), applied on top of the above during the wave-A merge of parsoFish/main; same function, same guard, unchanged -- re-verified by `grep -n readPreflightFixState cli/bridge-studio.ts` and sed -n "527,536p" cli/bridge-studio.ts against the merged tree.)' },
  // ---- GUARD-NEXT: route the FULL path (incl. leaf) through the guard ----
  { file: 'cli/bridge-studio.ts', line: 1446, sink: 'readFileSync',

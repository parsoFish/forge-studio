...
export const ALLOWLIST = [
  ...
  { file: 'cli/ui-bridge.ts', line: 1103, sink: 'existsSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 1106, sink: 'readdirSync',
    reason: 'FILTER-PREDICATE-FP + READDIR-ENUM: as line 1103 — enumerates sessionIds under the registry-derived kindDir. (Line-drift remap from 1099 — R4-19-F2 WI-2, same import-block cause as the row above.)' },
  ...
  { file: 'cli/ui-bridge.ts', line: 2064, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: spawnAgentTurn — sessionId isSafeRunId-gated at line 2057 (SAFE_RUN_ID_RE + explicit .. check; corrected while remapping — the prior "1986+" no longer named any isSafeRunId call site), 7 lines above this sink, unconditionally on the path to it; `_<logPrefix>-<sessionId>` under trusted forgeRoot/_logs. (Line-drift remap from 2053 — R4-19-F2 WI-2\'s import-block growth (+7) plus its new `kb-cleanup` row in SPAWN_AGENT_SPECS (+4), cumulative +11; same function, same guard, unchanged — verified by sed -n "2064p" cli/ui-bridge.ts.)' },
  { file: 'cli/ui-bridge.ts', line: 2472, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: sibling spawn helper — sessionId/runId isSafeRunId-gated before this log-dir create under trusted forgeRoot/_logs. (Line-drift remap from 2461 — R4-19-F2 WI-2, same cumulative +11 cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3458, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: onboarding — sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3447 — R4-19-F2 WI-2, same cumulative +11 cause as the rows above; same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 3459, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created, sessionId SAFE_ID_RE-gated. (Line-drift remap from 3448 — R4-19-F2 WI-2, same cumulative +11 cause as the rows above.)' },
  { file: 'cli/ui-bridge.ts', line: 3467, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as the row above. (Line-drift remap from 3456 — R4-19-F2 WI-2, same cumulative +11 cause as the rows above.)' },
  { file: 'cli/ui-bridge.ts', line: 3504, sink: 'mkdirSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 4455, sink: 'existsSync',
  ...
  // ---- orchestrator/interactive-session.ts ----
  { file: 'orchestrator/interactive-session.ts', line: 279, sink: 'existsSync',
    reason: 'RETAINED-RAW-PRIMITIVE: readSessionStatus(sessionDir) — a DESIGNATED_UNGUARDED_FUNCTION superseded by the leaf-guarded sibling guardedReadSessionStatus(projectsRoot, dirSegments). NO production route calls the raw primitive (every session route resolves projectsRoot+segments through resolveGuardedPath and uses the guarded sibling — see the in-file SEC-04 notes). Boolean probe on join(sessionDir, file); a future raw caller trips the sibling caller-count ratchet. (Remap from 243 — W6-B1 added the onThinking callback + REDACTED_THINKING_MARKER export earlier in the file, +36 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: readSessionStatus.)' },
  { file: 'orchestrator/interactive-session.ts', line: 281, sink: 'readFileSync',
    reason: 'RETAINED-RAW-PRIMITIVE: readSessionStatus(sessionDir) — same as line 279; reads only after the existsSync probe, only from an already-guarded dir handed by the (now guarded-sibling-only) call path. Superseded primitive kept as the base + for tests. (Remap from 245 — W6-B1 added the onThinking callback + REDACTED_THINKING_MARKER export earlier in the file, +36 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: readSessionStatus.)' },
  { file: 'orchestrator/interactive-session.ts', line: 298, sink: 'writeFileSync',
    reason: 'RETAINED-RAW-PRIMITIVE: writeSessionStatus(sessionDir) — a DESIGNATED_UNGUARDED_FUNCTION superseded by the leaf-guarded guardedWriteSessionStatus. NO production route writes through the raw primitive; a future reachable caller trips the sibling ratchet. (Remap from 262 — W6-B1 added the onThinking callback + REDACTED_THINKING_MARKER export earlier in the file, +36 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: writeSessionStatus.)' },

  // ---- orchestrator/architect-runner.ts ----
  { file: 'orchestrator/architect-runner.ts', line: 1427, sink: 'existsSync',
    reason: 'RETAINED-RAW-PRIMITIVE: readStatus(sessionDir) — superseded by the leaf-guarded sibling guardedReadStatus(projectsRoot, dirSegments). listArchitectSessions now routes the status.json LEAF through guardedReadStatus, so NO production caller hands a request-derived dir to the raw primitive (the remaining callers are architect-runner.test.ts constructing their own trusted tmp dirs). readStatus is a DESIGNATED_UNGUARDED_FUNCTION — a future reachable caller trips the sibling caller-count ratchet (check-request-path-sinks). Boolean probe on join(sessionDir, "status.json"). (Further remap from 1379 — W6-B1 added the onThinking sink (sampler opts + a coalescing thinking-sink) earlier in the file, +48 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: readStatus.)' },
  { file: 'orchestrator/architect-runner.ts', line: 1429, sink: 'readFileSync',
    reason: 'RETAINED-RAW-PRIMITIVE: readStatus(sessionDir) — same as line 1427; reads status.json only after the existsSync probe, and only from test-supplied trusted dirs (production uses the leaf-guarded guardedReadStatus). Superseded primitive, kept as the base + for tests. (Further remap from 1381 — W6-B1 added the onThinking sink (sampler opts + a coalescing thinking-sink) earlier in the file, +48 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: readStatus.)' },
  { file: 'orchestrator/architect-runner.ts', line: 1438, sink: 'writeFileSync',
    reason: 'RETAINED-RAW-PRIMITIVE: writeStatus(sessionDir) — superseded by the leaf-guarded guardedWriteStatus. NO production caller (the runner writes via writeArchitectStatus → guardedWriteStatus); only architect-runner.test.ts calls the raw primitive on its own trusted tmp dirs. A future reachable caller trips the sibling ratchet. (Further remap from 1390 — W6-B1 added the onThinking sink (sampler opts + a coalescing thinking-sink) earlier in the file, +48 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: writeStatus.)' },
  { file: 'orchestrator/architect-runner.ts', line: 1616, sink: 'existsSync',
    reason: 'LOG-READ: readArchitectSessionStats — `_architect-<sessionId>/events.jsonl` single segment under resolve(logsRoot) (trusted); sessionId is the architect session id (SAFE_ID_RE convention at creation); best-effort stats (returns null on any error). Boolean. (Further remap from 1568 — W6-B1 added the onThinking sink (sampler opts + a coalescing thinking-sink) earlier in the file, +48 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: readArchitectSessionStats.)' },
  { file: 'orchestrator/architect-runner.ts', line: 1618, sink: 'readFileSync',
    reason: 'LOG-READ: as line 1616 — reads only the internal architect event log for cost/duration stats (symlink-blind residual disclosed in openConcerns). (Further remap from 1570 — W6-B1 added the onThinking sink (sampler opts + a coalescing thinking-sink) earlier in the file, +48 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: readArchitectSessionStats.)' },

...
export const ALLOWLIST = [
  ...
  { file: 'cli/bridge-studio.ts', line: 427, sink: 'existsSync',
    reason: 'LOG-READ: readPreflightFixState — runId SAFE_ID_RE-gated at the GET route (line 884); `_preflight-fix-<runId>/events.jsonl` single validated segment under trusted forgeRoot/_logs; boolean. (Line-drift remap from 411 — R1-06 added a single import + a small loadAllFlows change earlier in the file, +7 lines; same function, same guard, unchanged. Further remap from 418 — forge-3oq added a `./studio-provenance.ts` import earlier in the file, +8 lines; same function, same guard, unchanged. Further remap from 426 — W6-RV-1 added a `gray-matter` import (title-source frontmatter read, mock finding I3) plus an `import type { WorkItem }` line, +2 lines cumulative earlier in the file; same function, same guard, unchanged. Further remap from 428 — W6-RV-1 review fix moved the title read onto `InitiativeManifest.title` (orchestrator/manifest.ts) and dropped the now-unneeded second `matter()` call + its `gray-matter` import, -1 line net; same function, same guard, unchanged.)' },
  { file: 'cli/bridge-studio.ts', line: 429, sink: 'readFileSync',
    reason: 'LOG-READ: as line 427 — internal preflight-fix event log only (symlink-blind residual disclosed in openConcerns). (Line-drift remap from 413. Further remap from 420 — forge-3oq, same cause as the row above. Further remap from 428 — W6-RV-1, same +2 import cause as the row above. Further remap from 430 — W6-RV-1 review fix, same -1 cause as the row above.)' },
  ...
  { file: 'cli/bridge-studio.ts', line: 535, sink: 'readFileSync',
    reason: 'LOG-READ (SEC-04 residual #2, disposed UNIFORMLY with the _logs log-read family — kbs.ts:223/225, bridge-studio.ts:418/420, architect-runner.ts:1596/1598): phase-log GET /api/runs/<runId>/phases/<node>/log reads readFileSync(resolve(logsRoot, runId, "events.jsonl")). runId is URL-derived but behind a lexical resolve()+startsWith(logsRoot+sep) gate (line 518) that BLOCKS .. traversal (resolve normalizes, startsWith rejects escape) — symlink-BLIND like the SAFE_ID_RE-gated family, same residual class. NOT wire-reachable: the _logs/<runId> symlink precondition is unplantable via any route (nothing writes attacker-chosen symlinks under _logs). Allowlisted (not guarded) so the whole _logs event-read family stays uniform — no split treatment; the migrate-to-guardedFile follow-up for the family is echoed in openConcerns. (Line-drift remap from 519. Further remap from 526 — forge-3oq added a `./studio-provenance.ts` import + the agents/flows/projects provenance wiring earlier in the file, +8 lines; same function, same guard, unchanged. Further remap from 534 — W6-RV-1, same +2 import cause as the bridge-studio.ts rows above. Further remap from 536 — W6-RV-1 review fix, same -1 net cause as the bridge-studio.ts rows above (dropped the second matter() parse + its now-unneeded import); gate-line citation also corrected 509→518, its own prior remaps having drifted uncorrected.)' },
  { file: 'cli/bridge-studio.ts', line: 522, sink: 'existsSync',
    reason: 'LOG-READ (SEC-04 residual #2, boolean pair of line 535): existsSync(eventsPath) on the same lexically-resolved, .. -blocked, symlink-blind URL-derived path — boolean 404 probe, no bytes flow. Same disclosed-_logs-symlink residual as line 535; uniform with the log-read family. (Line-drift remap from 506. Further remap from 513 — forge-3oq, same cause as the row above. Further remap from 521 — W6-RV-1, same +2 import cause as the bridge-studio.ts rows above. Further remap from 523 — W6-RV-1 review fix, same -1 net cause as the row above.)' },
  // ---- GUARD-NEXT: route the FULL path (incl. leaf) through the guard ----
  { file: 'cli/bridge-studio.ts', line: 1291, sink: 'readFileSync',
    reason: 'GUARD-NEXT (SEC-04, caught by broadened dir-param): tryReadWorkItemDir(dir) reads readFileSync(join(dir, file)); its callers pass snapshotDir=join(logsRoot, cycleId, "work-items-snapshot") and liveDir=join(forgeRoot, "_worktrees", initId, ".forge","work-items") — the DIR is built from request-derived cycleId/initId by lexical join with NO realpath containment (the WI-*.md leaf is readdir-enumerated). Symlink-blind coverage gap (full route-reachability not traced this stage). NEXT: route the dir+leaf through guardedReadDir/guardedReadFile; delete when guarded. (Line-drift remap from 1236 — unrelated pre-existing gap, unchanged by R1-06; still open. Further remap from 1243 — forge-3oq, same cause as the rows above, +11 lines cumulative; still open. Further remap from 1254 — forge-3oq review added a `provenanceOfOrigin(flow.origin)` map at the GET /api/studio/flows/:id route earlier in the file, +4 lines; same tryReadWorkItemDir call, same open gap, unchanged; still open. Further remap from 1258 — W6-RV-1 (mock finding I3, title-source fix) added the `frontmatterTitle`-carrying `ScannedManifestEntry` field + `matter()` read in the scan loop + the `deriveInitiativeTitle`/`BOILERPLATE_HEADINGS` helpers earlier in the file, +44 lines cumulative incl. the +2 import-block growth shared with the rows above; same tryReadWorkItemDir call, same open gap, unchanged. Further remap from 1302 — W6-RV-1 review fix moved `title` onto `InitiativeManifest` (orchestrator/manifest.ts) and deleted the `frontmatterTitle` field + its `matter()` re-parse (the perf fix this review round exists for — "never parse the same buffer twice on a polled endpoint"), -11 lines net; same tryReadWorkItemDir call, same open gap, unchanged — verified by sed -n "1291p" cli/bridge-studio.ts.)' },

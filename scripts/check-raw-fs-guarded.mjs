...
export const ALLOWLIST = [
  ...
  { file: 'cli/bridge-studio.ts', line: 455, sink: 'existsSync',
    reason: 'LOG-READ: readPreflightFixState — runId SAFE_ID_RE-gated at the GET route; `_preflight-fix-<runId>/events.jsonl` single validated segment under trusted forgeRoot/_logs; boolean. (Line-drift remap from 411 — R1-06, +7 lines. Further remap from 418 — forge-3oq, +8 lines. Further remap from 426 — W6-B2 review fix added the LEGACY_SESSION_TERMINAL_PHASES constant earlier in the file, +24 lines; same function, same guard, unchanged — verified by sed -n "450p" cli/bridge-studio.ts.) (Merge remap from 450 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Merge remap from 454 - parsoFish/main P1/B1/P2 merged into feat/w6-rv1-initiative-detail: W6-RV-1s title-source perf fix (dropped a second matter() re-parse of the manifest buffer + its now-unneeded gray-matter import near the top of the file) nets +1 line over this branchs pre-merge base, additive with the unrelated main-side shift above; same function, same guard, unchanged - verified by sed -n "455p" cli/bridge-studio.ts against the merged tree.)' },
  { file: 'cli/bridge-studio.ts', line: 457, sink: 'readFileSync',
    reason: 'LOG-READ: as line 455 — internal preflight-fix event log only (symlink-blind residual disclosed in openConcerns). (Line-drift remap from 413. Further remap from 420 — forge-3oq, same cause as the row above. Further remap from 428 — W6-B2 review fix, same +24 cause as the row above.) (Merge remap from 452 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Merge remap from 456 - parsoFish/main P1/B1/P2 merged into feat/w6-rv1-initiative-detail: same +1 net cause as the row above; same function, same guard, unchanged - verified by sed -n "457p" cli/bridge-studio.ts against the merged tree.)' },

  // ---- cli/ui-bridge.ts ----
  { file: 'cli/ui-bridge.ts', line: 459, sink: 'existsSync',
    reason: 'LOG-TAIL: ensureTailFor(cycleId) — boolean guard before establishing a READ-ONLY setInterval tail of <logsRoot>/<cycleId>/events.jsonl; cycleId is an internal broadcast cycle id, path under trusted logsRoot. (Line-drift remap from 447 — R4-19-F2 WI-2, +7 lines. Further remap from 454 — W6-B2 review fix added a LEGACY_SESSION_TERMINAL_PHASES import to the top-of-file import block, +1 line; same function, same guard, unchanged — verified by sed -n "455p" cli/ui-bridge.ts.) (Merge remap from 455 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 456 — W6-B4 added a bridge-studio-affordances.ts import line earlier in the file, +1 line; same function, same guard, unchanged — verified by sed -n "457p" cli/ui-bridge.ts.) (Merge remap from 457 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "458p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 784, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(reviewCommentsPath(logsRoot, cycleId)) — boolean "does the sidecar exist" only; the paired writeReviewComments(logsRoot, cycleId) enforces containment and throws (→500, never writes) on a traversal cycleId; route params are isSafeCycleId-gated. (Line-drift remap from 752 — W6-B2 added ensureSessionTail, +27 lines. Further remap from 779 — W6-B2 review fix, same +1 import-line cause as the row above; same function, same guard, unchanged — verified by sed -n "780p" cli/ui-bridge.ts.) (Merge remap from 780 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 781 — W6-B4 added a bridge-studio-affordances.ts import line earlier in the file, +1 line; same function, same guard, unchanged — verified by sed -n "782p" cli/ui-bridge.ts.) (Merge remap from 782 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "784p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 1136, sink: 'existsSync',
    reason: 'FILTER-PREDICATE-FP + READDIR-ENUM: kindDir = join(ctx.projectsRoot, <readdir-enumerated project>, `_${descriptor.id}`); `slug` (the taint) appears ONLY in the `.filter(d => d.agent === slug)` predicate that SELECTS descriptors — it is never part of the path VALUE (a server-side session-kind registry id is). Boolean. (Line-drift remap from 1103 — W6-B2, +27 lines. Further remap from 1130 — W6-B2 review fix, same +1 cause as the row above.) (Merge remap from 1131 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 1133 — W6-B4 added a bridge-studio-affordances.ts import line earlier in the file, +1 line; same function, same guard, unchanged — verified by sed -n "1134p" cli/ui-bridge.ts.) (Merge remap from 1134 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "1135p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 1139, sink: 'readdirSync',
    reason: 'FILTER-PREDICATE-FP + READDIR-ENUM: as line 1131 — enumerates sessionIds under the registry-derived kindDir. (Line-drift remap from 1106 — W6-B2, +27 lines. Further remap from 1133 — W6-B2 review fix, same +1 cause as the row above.) (Merge remap from 1134 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 1136 — W6-B4 added a bridge-studio-affordances.ts import line earlier in the file, +1 line; same function, same guard, unchanged — verified by sed -n "1137p" cli/ui-bridge.ts.) (Merge remap from 1137 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "1138p" cli/ui-bridge.ts against the merged tree.)' },
  ...
  { file: 'cli/ui-bridge.ts', line: 2131, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: spawnAgentTurn — sessionId isSafeRunId-gated a few lines above (SAFE_RUN_ID_RE + explicit .. check), unconditionally on the path to it; `_<logPrefix>-<sessionId>` under trusted forgeRoot/_logs. (Line-drift remap from 2064 — W6-B2\'s ensureSessionTail helper + ctx call-site comment, cumulative +32. Further remap from 2096 — W6-B2 review fix\'s exported-SPAWN_AGENT_SPECS doc comment (+10) plus a +1 import line, cumulative +11 more; same function, same guard, unchanged — verified by sed -n "2107p" cli/ui-bridge.ts.) (Merge remap from 2107 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 2109 — W6-B4 added a bridge-studio-affordances.ts import line + the generic session-affordance route-wiring block in handleHttp + an export-comment on spawnAgentTurn earlier in the file, cumulative +20 lines; same function, same guard, unchanged — verified by sed -n "2129p" cli/ui-bridge.ts.) (Merge remap from 2129 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "2131p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 2539, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: sibling spawn helper — sessionId/runId isSafeRunId-gated before this log-dir create under trusted forgeRoot/_logs. (Line-drift remap from 2472 — W6-B2, cumulative +32. Further remap from 2504 — W6-B2 review fix, same +11 cause as the row above.) (Merge remap from 2515 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 2517 — W6-B4, same cumulative +20 cause as the row above; same function, same guard, unchanged — verified by sed -n "2537p" cli/ui-bridge.ts.) (Merge remap from 2537 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "2539p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3585, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: onboarding — sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3458 — W6-B2, cumulative +32. Further remap from 3490 — W6-B2 review fix, same +11 cause as the rows above; same function, same guard, unchanged.) (Merge remap from 3501 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 3503 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "3523p" cli/ui-bridge.ts.) (Merge remap from 3523 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "3585p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3586, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created, sessionId SAFE_ID_RE-gated. (Line-drift remap from 3459 — W6-B2, cumulative +32. Further remap from 3491 — W6-B2 review fix, same +11 cause as the rows above.) (Merge remap from 3502 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 3504 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "3524p" cli/ui-bridge.ts.) (Merge remap from 3524 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "3586p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3594, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as the row above. (Line-drift remap from 3467 — W6-B2, cumulative +32. Further remap from 3499 — W6-B2 review fix, same +11 cause as the rows above.) (Merge remap from 3510 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 3512 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "3532p" cli/ui-bridge.ts.) (Merge remap from 3532 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "3594p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3637, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: authoring — writeAuthoringSession, byte-for-byte the same shape as writeOnboardingSession\'s rows directly above: sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3504 — W6-B2, cumulative +32. Further remap from 3536 — W6-B2 review fix, same +11 cause as the rows above.) (Merge remap from 3547 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 3549 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "3569p" cli/ui-bridge.ts.) (Merge remap from 3569 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "3637p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3638, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created (line 3547), sessionId SAFE_ID_RE-gated. (Line-drift remap from 3505 — W6-B2, cumulative +32. Further remap from 3537 — W6-B2 review fix, same +11 cause as the row above; the write itself also includes `prompt` in the JSON payload per the earlier R4-21 phase 2 WI-2 change, same sink, same guard.) (Merge remap from 3548 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 3550 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "3570p" cli/ui-bridge.ts.) (Merge remap from 3570 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "3638p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3654, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as line 3548. (Line-drift remap from 3510 — W6-B2, cumulative +32. Further remap from 3542 — W6-B2 review fix, same +11 cause as the row above.) (Merge remap from 3553 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 3555 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "3575p" cli/ui-bridge.ts.) (Merge remap from 3575 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "3654p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 4220, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: onboardingParent = join(realProjectDir, "_onboarding") where realProjectDir is realpathSync-resolved; this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification that refuses a symlinked _onboarding. (Line-drift remap from 4068 — W6-B2, cumulative +32. Further remap from 4100 — W6-B2 review fix, same +11 cause as the rows above; same function, same guard, unchanged — verified by sed -n "4111p" cli/ui-bridge.ts.) (Merge remap from 4111 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 4113 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "4133p" cli/ui-bridge.ts.) (Merge remap from 4133 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "4220p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 4326, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: authoringParent = join(realProjectDir, "_authoring") where realProjectDir is realpathSync-resolved — byte-for-byte the same shape as the onboardingParent row above: this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification a couple lines below that refuses a symlinked _authoring. (Line-drift remap from 4166 — W6-B2, cumulative +32. Further remap from 4198 — W6-B2 review fix, same +11 cause as the row above.) (Merge remap from 4209 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 4211 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "4231p" cli/ui-bridge.ts.) (Merge remap from 4231 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "4326p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 4633, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(join(repoPath, ".forge","demo","demo.lock.json")) picks the create/update mode default; boolean-only, no bytes flow — and per this repo\'s standing rule ("guard the paths you WRITE, not the paths you merely probe") it deliberately stays a probe: a guarded read here would false-reject a legitimate project whose `.forge` is a symlink and silently flip its mode. RE-AUDITED (R4-19-F2 resync, not a rubber-stamped remap): `repoPath` is derived by the if/else at (current) lines 4485-4495. The `else` branch (`body.project`) is genuinely guard-derived — `repoPath = resolveGuardedPath(ctx.projectsRoot,[body.project]).realPath`, the guard\'s OWN resolved output, structurally tamper-proof. The `if` branch (`body.projectRepoPath`) is caller-supplied but NOT unguarded: `invalidProjectRepoPath(body.projectRepoPath, {forgeRoot, projectsRoot})` runs at line 4458, strictly before `repoPath` is ever assigned or read, and internally calls `isContainedProjectRepoPath` → `containedUnder` → `resolveGuardedPath` — the SAME real per-segment realpath identity walk the `else` branch uses, just validating the caller\'s string instead of returning a resolved replacement for it. That is the SAME "PROJECTROOT-GUARDED" pattern this file already accepts at cli/bridge-studio-writes.ts:854/919 (validate-the-string-then-reuse-the-ORIGINAL-string, not the guard\'s realPath) — not a weaker, ad-hoc trust. Residual, disclosed honestly: because the branch reuses the caller\'s original string rather than the guard\'s realPath, a symlink swapped between the line-4458 check and this line-4498 use would in principle reopen containment — but no `await` sits between them in this handler (`readJson` already ran at line 4451, before 4458; `resolveDemoSessionDir` at 4468 is synchronous), so there is no scheduling point for such a swap, and even a successful one only flips a boolean create/update default, never reads or writes bytes through the path. Verdict: an acceptable audited residual on the strength of the established PROJECTROOT-GUARDED precedent plus the boolean-only blast radius — not a fresh unguarded hole. (Line-drift remap from 4455 — W6-B2, cumulative +32. Further remap from 4487 — W6-B2 review fix, same +11 cause as the rows above; same function, same probe, byte-for-byte unchanged — verified by sed -n "4498p" cli/ui-bridge.ts.) (Merge remap from 4498 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 4500 — W6-B4, same cumulative +20 cause as the rows above; same function, same guard, unchanged — verified by sed -n "4520p" cli/ui-bridge.ts.) (Merge remap from 4520 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "4633p" cli/ui-bridge.ts against the merged tree.)' },

  // ---- orchestrator/interactive-session.ts ----
  { file: 'orchestrator/interactive-session.ts', line: 439, sink: 'existsSync',
    reason: 'RETAINED-RAW-PRIMITIVE: readSessionStatus(sessionDir) — a DESIGNATED_UNGUARDED_FUNCTION superseded by the leaf-guarded sibling guardedReadSessionStatus(projectsRoot, dirSegments). NO production route calls the raw primitive (every session route resolves projectsRoot+segments through resolveGuardedPath and uses the guarded sibling — see the in-file SEC-04 notes). Boolean probe on join(sessionDir, file); a future raw caller trips the sibling caller-count ratchet. (Further remap from 279 — W6-B1 review round 2 factored the shared makeThinkingSink/makeReasoningSink pair (+ the row-cap/coalescing fix) into this file earlier in the file, +160 lines; same function, same guard, byte-unchanged. Sink expression and enclosing function re-verified at the new line: readSessionStatus.)' },
  { file: 'orchestrator/interactive-session.ts', line: 441, sink: 'readFileSync',
  ...
  { file: 'cli/bridge-studio-kbs.ts', line: 1125, sink: 'mkdirSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 1891, sink: 'mkdirSync',
    reason: 'RETAIN (isSafeRunId-gated logdir-create): runId = `_agent-${slug}-${newRunStamp()}` with slug URL-derived (newly tainted), but isSafeRunId(runId) (SAFE_RUN_ID_RE + explicit .. check) THROWS a few lines above BEFORE this recursive mkdir of the run\'s own log dir under trusted ctx.logsRoot — the SAME deliberate guard-symmetry check its already-allowlisted siblings at (now) 2107/2515 carry. (Line-drift remap from 1841 — W6-B2\'s ensureSessionTail helper + ctx call-site comment, cumulative +32. Further remap from 1873 — W6-B2 review fix\'s +1 import line, same cause as the cli/ui-bridge.ts rows above.) (Merge remap from 1874 - parsoFish/main B2 session-tails merged into feat/w6-p1-run-list-cache shifted earlier-file line counts; same function, same guard, unchanged - verified against the merged tree.) (Line-drift remap from 1876 — W6-B4 added a bridge-studio-affordances.ts import line + the generic session-affordance route-wiring block in handleHttp earlier in the file, cumulative +13 lines; same function, same guard, unchanged — verified by sed -n "1889p" cli/ui-bridge.ts.) (Merge remap from 1889 - feat/w6-b4-affordance-endpoint merged into feat/w6-b6-generic-panel; concurrent main-side commits not on B4\'s base (incl. W6-B7 ActivityLog) shifted earlier-file line counts; same function, same guard, unchanged - verified by sed -n "1891p" cli/ui-bridge.ts against the merged tree.)' },

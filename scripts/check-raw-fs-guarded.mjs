...
export const ALLOWLIST = [
  ...
  // MERGE REMAP (W6-B9 merging parsoFish/main [W6-B12 kb-drain, W6-CR-2/3,
  // etc.] AND parsoFish/feat/w6-b8-migrate-cleanup-authoring's fix-round
  // [kb-cleanup's bespoke /cleanup/apply route + its approveKbCleanup/
  // invalidSessionIdReason imports DELETED as dead surface] into this
  // branch's own +11-line /api/instructions/brief byte-cap insertion): every
  // cli/ui-bridge.ts `line:` value below is RE-MEASURED directly off the
  // real merged tree (grep on each sink's own call-site text), not a
  // re-derived arithmetic sum of the three branches' independent deltas --
  // the combination of an earlier-file deletion (B8) and a later-file
  // insertion (this branch) makes a purely additive remap error-prone.
  // Each individual row's own PRIOR historical remap chain is preserved
  // verbatim below this note.
  { file: 'cli/ui-bridge.ts', line: 2131, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: spawnAgentTurn -- sessionId isSafeRunId-gated a few lines above (SAFE_RUN_ID_RE + explicit .. check), unconditionally on the path to it; `_<logPrefix>-<sessionId>` under trusted forgeRoot/_logs. (Merge remap: re-measured directly against the W6-B9 3-way merge (main + B8 fix-round + this branch); same function, same guard, unchanged -- verified by sed -n "2131p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 2539, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: sibling spawn helper -- sessionId/runId isSafeRunId-gated before this log-dir create under trusted forgeRoot/_logs. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "2539p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3596, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: onboarding -- sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "3596p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3597, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: status.json write with flag `wx` (O_EXCL) -- never follows an existing symlink; parent sessionDir just exclusively created, sessionId SAFE_ID_RE-gated. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "3597p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3605, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: prompt.md write with flag `wx` (O_EXCL) -- same exclusive-create discipline as the row above. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "3605p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3648, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: authoring -- writeAuthoringSession, byte-for-byte the same shape as writeOnboardingSession\'s rows directly above: sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "3648p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3649, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring status.json write with flag `wx` (O_EXCL) -- never follows an existing symlink; parent sessionDir just exclusively created, sessionId SAFE_ID_RE-gated; the write itself also includes `prompt` in the JSON payload per the earlier R4-21 phase 2 WI-2 change, same sink, same guard. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "3649p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 3665, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring prompt.md write with flag `wx` (O_EXCL) -- same exclusive-create discipline as the row above. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "3665p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 4231, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: onboardingParent = join(realProjectDir, "_onboarding") where realProjectDir is realpathSync-resolved; this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification that refuses a symlinked _onboarding. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "4231p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 4337, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: authoringParent = join(realProjectDir, "_authoring") where realProjectDir is realpathSync-resolved -- byte-for-byte the same shape as the onboardingParent row above: this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification a couple lines below that refuses a symlinked _authoring. (Merge remap: re-measured directly against the W6-B9 3-way merge; same function, same guard, unchanged -- verified by sed -n "4337p" cli/ui-bridge.ts against the merged tree.)' },
  { file: 'cli/ui-bridge.ts', line: 4516, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(join(repoPath, ".forge","demo","demo.lock.json")) picks the create/update mode default; boolean-only, no bytes flow -- and per this repo\'s standing rule ("guard the paths you WRITE, not the paths you merely probe") it deliberately stays a probe: a guarded read here would false-reject a legitimate project whose `.forge` is a symlink and silently flip its mode. RE-AUDITED (R4-19-F2 resync, not a rubber-stamped remap): `repoPath` is derived by a nearby if/else. The `else` branch (`body.project`) is genuinely guard-derived -- `repoPath = resolveGuardedPath(ctx.projectsRoot,[body.project]).realPath`, the guard\'s OWN resolved output, structurally tamper-proof. The `if` branch (`body.projectRepoPath`) is caller-supplied but NOT unguarded: `invalidProjectRepoPath(body.projectRepoPath, {forgeRoot, projectsRoot})` runs strictly before `repoPath` is ever assigned or read, and internally calls `isContainedProjectRepoPath` -> `containedUnder` -> `resolveGuardedPath` -- the SAME real per-segment realpath identity walk the `else` branch uses, just validating the caller\'s string instead of returning a resolved replacement for it. That is the SAME "PROJECTROOT-GUARDED" pattern this file already accepts at cli/bridge-studio-writes.ts:854/919 (validate-the-string-then-reuse-the-ORIGINAL-string, not the guard\'s realPath) -- not a weaker, ad-hoc trust. Residual, disclosed honestly: because the branch reuses the caller\'s original string rather than the guard\'s realPath, a symlink swapped between the earlier check and this use would in principle reopen containment -- but no `await` sits between them in this handler, so there is no scheduling point for such a swap, and even a successful one only flips a boolean create/update default, never reads or writes bytes through the path. Verdict: an acceptable audited residual on the strength of the established PROJECTROOT-GUARDED precedent plus the boolean-only blast radius -- not a fresh unguarded hole. (Merge remap: re-measured directly against the W6-B9 3-way merge -- an earlier-file deletion (B8) combined with a later-file insertion (this branch) made a naive arithmetic sum unreliable; same function, same probe, byte-for-byte unchanged -- verified by sed -n "4516p" cli/ui-bridge.ts against the merged tree.)' },

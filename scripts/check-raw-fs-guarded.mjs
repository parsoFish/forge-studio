...
export const ALLOWLIST = [
  ...
  // MERGE REMAP (feat/w6-b12-kb-drain merging parsoFish/main, W6-CR-2
  // community-sorts + others landed): every cli/ui-bridge.ts `line:` value
  // below through the end of this file's ui-bridge.ts rows is main's own
  // (already-current-as-of-main) number PLUS 2 — this branch's own
  // cli/ui-bridge.ts diff is exactly 2 lines (the KB-drain route import +
  // its dispatch call, both earlier in the file than every row here), so
  // merging main's tree in shifts everything below that point by a uniform
  // +2. Each individual row's own historical remap chain is otherwise
  // untouched (main's copy of each row, not a re-derivation) — spot-verified
  // against the merged tree for every row in this block.
  { file: 'cli/ui-bridge.ts', line: 2132, sink: 'mkdirSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 2540, sink: 'mkdirSync',

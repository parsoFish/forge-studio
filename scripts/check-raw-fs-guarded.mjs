...
export const ALLOWLIST = [
  ...
  { file: 'cli/ui-bridge.ts', line: 779, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(reviewCommentsPath(logsRoot, cycleId)) — boolean "does the sidecar exist" only; the paired writeReviewComments(logsRoot, cycleId) enforces containment and throws (→500, never writes) on a traversal cycleId; route params are isSafeCycleId-gated. (Line-drift remap from 752 — W6-B2 added a generalized `ensureSessionTail(kind, sessionId)` helper (~28 lines) plus supporting doc comments directly after `ensureTailFor`, +27 lines; same function, same guard, unchanged — verified by sed -n "779p" cli/ui-bridge.ts.)' },
  { file: 'cli/ui-bridge.ts', line: 1130, sink: 'existsSync',
    reason: 'FILTER-PREDICATE-FP + READDIR-ENUM: kindDir = join(ctx.projectsRoot, <readdir-enumerated project>, `_${descriptor.id}`); `slug` (the taint) appears ONLY in the `.filter(d => d.agent === slug)` predicate that SELECTS descriptors — it is never part of the path VALUE (a server-side session-kind registry id is). Boolean. (Line-drift remap from 1103 — W6-B2, same +27 cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 1133, sink: 'readdirSync',
    reason: 'FILTER-PREDICATE-FP + READDIR-ENUM: as line 1130 — enumerates sessionIds under the registry-derived kindDir. (Line-drift remap from 1106 — W6-B2, same +27 cause as the row above.)' },
  ...
  { file: 'cli/ui-bridge.ts', line: 2096, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: spawnAgentTurn — sessionId isSafeRunId-gated a few lines above (SAFE_RUN_ID_RE + explicit .. check), unconditionally on the path to it; `_<logPrefix>-<sessionId>` under trusted forgeRoot/_logs. (Line-drift remap from 2064 — W6-B2\'s `ensureSessionTail` helper + doc-comment growth after `ensureTailFor` (+27) plus the `handleStudioSessionsRoutes` ctx call-site comment (+5), cumulative +32; same function, same guard, unchanged — verified by sed -n "2096p" cli/ui-bridge.ts.)' },
  { file: 'cli/ui-bridge.ts', line: 2504, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: sibling spawn helper — sessionId/runId isSafeRunId-gated before this log-dir create under trusted forgeRoot/_logs. (Line-drift remap from 2472 — W6-B2, same cumulative +32 cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3490, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: onboarding — sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3458 — W6-B2, same cumulative +32 cause as the rows above; same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 3491, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created, sessionId SAFE_ID_RE-gated. (Line-drift remap from 3459 — W6-B2, same cumulative +32 cause as the rows above.)' },
  { file: 'cli/ui-bridge.ts', line: 3499, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as the row above. (Line-drift remap from 3467 — W6-B2, same cumulative +32 cause as the rows above.)' },
  { file: 'cli/ui-bridge.ts', line: 3536, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: authoring — writeAuthoringSession, byte-for-byte the same shape as writeOnboardingSession\'s rows directly above: sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3504 — W6-B2, same cumulative +32 cause as the rows above.)' },
  { file: 'cli/ui-bridge.ts', line: 3537, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created (line 3536), sessionId SAFE_ID_RE-gated. (Line-drift remap from 3505 — W6-B2, same cumulative +32 cause as the row above; the write itself also includes `prompt` in the JSON payload per the earlier R4-21 phase 2 WI-2 change, same sink, same guard.)' },
  { file: 'cli/ui-bridge.ts', line: 3542, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as line 3537. (Line-drift remap from 3510 — W6-B2, same cumulative +32 cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 4100, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: onboardingParent = join(realProjectDir, "_onboarding") where realProjectDir is realpathSync-resolved; this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification that refuses a symlinked _onboarding. (Line-drift remap from 4068 — W6-B2, same cumulative +32 cause as the rows above; same function, same guard, unchanged — verified by sed -n "4100p" cli/ui-bridge.ts.)' },
  { file: 'cli/ui-bridge.ts', line: 4198, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: authoringParent = join(realProjectDir, "_authoring") where realProjectDir is realpathSync-resolved — byte-for-byte the same shape as the onboardingParent row above: this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification a couple lines below that refuses a symlinked _authoring. (Line-drift remap from 4166 — W6-B2, same cumulative +32 cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 4487, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(join(repoPath, ".forge","demo","demo.lock.json")) picks the create/update mode default; boolean-only, no bytes flow — and per this repo\'s standing rule ("guard the paths you WRITE, not the paths you merely probe") it deliberately stays a probe: a guarded read here would false-reject a legitimate project whose `.forge` is a symlink and silently flip its mode. RE-AUDITED (R4-19-F2 resync, not a rubber-stamped remap): `repoPath` is derived by the if/else at (current) lines 4474-4484. The `else` branch (`body.project`) is genuinely guard-derived — `repoPath = resolveGuardedPath(ctx.projectsRoot,[body.project]).realPath`, the guard\'s OWN resolved output, structurally tamper-proof. The `if` branch (`body.projectRepoPath`) is caller-supplied but NOT unguarded: `invalidProjectRepoPath(body.projectRepoPath, {forgeRoot, projectsRoot})` runs at line 4447, strictly before `repoPath` is ever assigned or read, and internally calls `isContainedProjectRepoPath` → `containedUnder` → `resolveGuardedPath` — the SAME real per-segment realpath identity walk the `else` branch uses, just validating the caller\'s string instead of returning a resolved replacement for it. That is the SAME "PROJECTROOT-GUARDED" pattern this file already accepts at cli/bridge-studio-writes.ts:854/919 (validate-the-string-then-reuse-the-ORIGINAL-string, not the guard\'s realPath) — not a weaker, ad-hoc trust. Residual, disclosed honestly: because the branch reuses the caller\'s original string rather than the guard\'s realPath, a symlink swapped between the line-4447 check and this line-4487 use would in principle reopen containment — but no `await` sits between them in this handler (`readJson` already ran at line 4440, before 4447; `resolveDemoSessionDir` at 4457 is synchronous), so there is no scheduling point for such a swap, and even a successful one only flips a boolean create/update default, never reads or writes bytes through the path. Verdict: an acceptable audited residual on the strength of the established PROJECTROOT-GUARDED precedent plus the boolean-only blast radius — not a fresh unguarded hole. (Line-drift remap from 4455 — W6-B2, same cumulative +32 cause as the rows above; same function, same probe, byte-for-byte unchanged — verified by sed -n "4487p" cli/ui-bridge.ts.)' },
  ...
  { file: 'cli/ui-bridge.ts', line: 1873, sink: 'mkdirSync',
    reason: 'RETAIN (isSafeRunId-gated logdir-create): runId = `_agent-${slug}-${newRunStamp()}` with slug URL-derived (newly tainted), but isSafeRunId(runId) (SAFE_RUN_ID_RE + explicit .. check) THROWS a few lines above BEFORE this recursive mkdir of the run\'s own log dir under trusted ctx.logsRoot — the SAME deliberate guard-symmetry check its already-allowlisted siblings at (now) 2096/2504 carry. (Line-drift remap from 1841 — W6-B2\'s `ensureSessionTail` helper + doc-comment growth after `ensureTailFor` (+27) plus the `handleStudioSessionsRoutes` ctx call-site comment (+5), cumulative +32, same cause as the cli/ui-bridge.ts rows above.)' },

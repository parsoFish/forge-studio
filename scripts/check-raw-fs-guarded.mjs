...
export const ALLOWLIST = [
  ...
  { file: 'cli/ui-bridge.ts', line: 2439, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: sibling spawn helper — sessionId/runId isSafeRunId-gated before this log-dir create under trusted forgeRoot/_logs. (Line-drift remap from 2437 — R4-21 WI-2, +2 lines total, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3408, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: onboarding — sessionId SAFE_ID_RE-gated at line 3356; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3406 — R4-21 WI-2, +2 lines total, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3409, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created (line 3362), sessionId SAFE_ID_RE-gated. (Line-drift remap from 3407 — R4-21 WI-2, +2 lines total, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3417, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as line 3363. (Line-drift remap from 3415 — R4-21 WI-2, +2 lines total, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 3975, sink: 'mkdirSync',
    reason: 'MANUAL-CONTAIN: onboardingParent = join(realProjectDir, "_onboarding") where realProjectDir is realpathSync-resolved; this mkdir is immediately followed by realpathSync + startsWith(realProjectDir + sep) re-verification (lines 3930-3932) that refuses a symlinked _onboarding. (Line-drift remap from 3973 — R4-21 WI-2, +2 lines total, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 4044, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(join(repoPath, ".forge","demo","demo.lock.json")) picks the create/update mode default; body.project was already proven contained by resolveDemoSessionDir above; boolean-only, no bytes flow. (Line-drift remap from 4042 — R4-21 WI-2, +2 lines total, same cause as the row above.)' },

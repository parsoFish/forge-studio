...
export const ALLOWLIST = [
  ...
  { file: 'cli/bridge-studio-writes.ts', line: 1131, sink: 'existsSync',
    reason: 'PROJECTROOT-GUARDED: projectRoot is validated by isContainedProjectRepoPath (manifest-path-guard, per-segment realpath) at line 835; this is a boolean duplicate-project probe. (Line-drift remap from 854 -- W7-A4 (one id rule: PROJECT_ID_RE gates + isReservedId on the agents/projects/flows create routes), +9 lines; same function, same guard, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs --json` run.) (Line-drift remap from 863 -- W7-B3 community-registry CRUD (imports +3 near the top; the parseRegistryItemBody/mutateCommunityRegistry helpers + the three registry routes inserted ahead of the pre-existing handlers, +223 cumulative for rows past the insertion); same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Further remap from 1086 -- W7-B3 round 2: the PUT/DELETE registry-item arms split into two explicit dispatch lines (dry-bridge coverage derivation) + the shared decodeRegistryItemIdOrRespond helper, +16 lines; same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Line-drift remap from 1102 -- W7-B3 review round: skill-only kind restriction in parseRegistryItemBody, server-owned signals policy, shared decodeIdOrRespond import replacing the local helper, hoisted registryItemMatch + DELETE-scoping guard, PUT carry-forward block; same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.)' },
  { file: 'cli/bridge-studio-writes.ts', line: 1196, sinks: ['existsSync', 'mkdirSync'],
    reason: 'PROJECTROOT-GUARDED: SEC-03 Defect-5 create-mode — projectRoot proven contained by isContainedProjectRepoPath (line 835) AND Phase-1 checkContractArtifactContainment; `if (!existsSync(projectRoot)) mkdirSync(projectRoot, {recursive})` — BOTH sinks on this line ride that proof (the in-file writeup is definitive). (Line-drift remap from 919 -- W7-A4 (one id rule: PROJECT_ID_RE gates + isReservedId on the agents/projects/flows create routes), +9 lines; same function, same guard, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs --json` run.) (Line-drift remap from 928 -- W7-B3 community-registry CRUD (imports +3 near the top; the parseRegistryItemBody/mutateCommunityRegistry helpers + the three registry routes inserted ahead of the pre-existing handlers, +223 cumulative for rows past the insertion); same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Further remap from 1151 -- W7-B3 round 2: the PUT/DELETE registry-item arms split into two explicit dispatch lines (dry-bridge coverage derivation) + the shared decodeRegistryItemIdOrRespond helper, +16 lines; same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Line-drift remap from 1167 -- W7-B3 review round: skill-only kind restriction in parseRegistryItemBody, server-owned signals policy, shared decodeIdOrRespond import replacing the local helper, hoisted registryItemMatch + DELETE-scoping guard, PUT carry-forward block; same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.)' },
  ...
  { file: 'cli/bridge-studio-writes.ts', line: 148, sink: 'existsSync',
    reason: 'RETAIN (boolean probe — MUST NOT guard): checkContractArtifactContainment(projectRoot) — projectRoot isContainedProjectRepoPath-validated at the route (line 835, per the 854/919 rows) before this Phase-1 checker; existsSync(join(projectRoot, ".forge","project.json")) is a boolean gate that RUNS resolveGuardedPath when the file is ABSENT (no bytes flow). Guarding an idempotent existence probe would false-reject; newly visible only because projectRoot is now a dir-shaped param. (Line-drift remap from 141 -- W7-B3 community-registry CRUD (imports +3 near the top; the parseRegistryItemBody/mutateCommunityRegistry helpers + the three registry routes inserted ahead of the pre-existing handlers, +3 cumulative for rows past the insertion); same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.) (Line-drift remap from 144 -- W7-B3 review round: skill-only kind restriction in parseRegistryItemBody, server-owned signals policy, shared decodeIdOrRespond import replacing the local helper, hoisted registryItemMatch + DELETE-scoping guard, PUT carry-forward block; same function, same guard/probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs` run.)' },

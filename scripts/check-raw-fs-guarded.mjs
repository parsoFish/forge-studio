...
export const ALLOWLIST = [
  ...
  // ---- cli/ui-bridge.ts ----
  { file: 'cli/ui-bridge.ts', line: 447, sink: 'existsSync',
    reason: 'LOG-TAIL: ensureTailFor(cycleId) — boolean guard before establishing a READ-ONLY setInterval tail of <logsRoot>/<cycleId>/events.jsonl; cycleId is an internal broadcast cycle id, path under trusted logsRoot. (Line-drift remap from 446 — R4-21 WI-2 added a handleStudioAuthoringRoutes import earlier in the file, +1 line; same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 745, sink: 'existsSync',
    reason: 'BOOL-PROBE: existsSync(reviewCommentsPath(logsRoot, cycleId)) — boolean "does the sidecar exist" only; the paired writeReviewComments(logsRoot, cycleId) enforces containment and throws (→500, never writes) on a traversal cycleId; route params are isSafeCycleId-gated (lines 1876/1886/1900). (Line-drift remap from 744 — R4-21 WI-2, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 1096, sink: 'existsSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 2461, sink: 'mkdirSync',
    reason: 'LOGDIR-CREATE: sibling spawn helper — sessionId/runId isSafeRunId-gated before this log-dir create under trusted forgeRoot/_logs. (Line-drift remap from 2446 — R4-21 phase 2, WI-2, same cause as the row above (SPAWN_AGENT_SPECS header growth shifts everything below it); same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 3447, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: onboarding — sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3415 — R4-21 phase 2, WI-2, same cause as the rows above; same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 3448, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created, sessionId SAFE_ID_RE-gated. (Line-drift remap from 3416 — R4-21 phase 2, WI-2, same cause as the rows above.)' },
  { file: 'cli/ui-bridge.ts', line: 3456, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as the row above. (Line-drift remap from 3424 — R4-21 phase 2, WI-2, same cause as the rows above.)' },
  { file: 'cli/ui-bridge.ts', line: 3493, sink: 'mkdirSync',
    reason: 'EXCLUSIVE-CREATE: authoring — writeAuthoringSession, byte-for-byte the same shape as writeOnboardingSession\'s rows directly above: sessionId SAFE_ID_RE-gated a few lines above; mkdirSync(sessionDir) has NO recursive flag, so a pre-existing entry (incl. a symlink) is a hard EEXIST, never reused/followed. (Line-drift remap from 3452 — R4-21 phase 2, WI-2 added the "prompt is ALSO written INTO status.json" doc comment to writeAuthoringSession\'s header, +9 lines; same function, same guard, unchanged.)' },
  { file: 'cli/ui-bridge.ts', line: 3494, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring status.json write with flag `wx` (O_EXCL) — never follows an existing symlink; parent sessionDir just exclusively created (line 3493), sessionId SAFE_ID_RE-gated. (Line-drift remap from 3453 — R4-21 phase 2, WI-2, same cause as the row above; the write itself now also includes `prompt` in the JSON payload, same sink, same guard.)' },
  { file: 'cli/ui-bridge.ts', line: 3499, sink: 'writeFileSync',
    reason: 'EXCLUSIVE-CREATE: authoring prompt.md write with flag `wx` (O_EXCL) — same exclusive-create discipline as line 3494. (Line-drift remap from 3458 — R4-21 phase 2, WI-2, same cause as the row above.)' },
  { file: 'cli/ui-bridge.ts', line: 4057, sink: 'mkdirSync',
  ...
  { file: 'cli/bridge-studio-kbs.ts', line: 1126, sink: 'existsSync',
    reason: 'RETAIN (contained + boolean): DELETE /api/studio/kbs/:id — id is URL-derived (newly tainted) but SLUG_RE-gated earlier in the route (blocks / and ..), and dir = resolveKbBrainDir(forgeRoot, id) runs the per-segment realpath identity walk (choke-point containment; see resolveKbBrainDir + resolveGuardedPath in orchestrator/brain-paths.ts) returning null on any escape; existsSync(dir) is a boolean 404 probe, no bytes flow. Same manifest-path-guard category as the isContainedWorktreePath rows above. (Line-drift remap from 1232 — forge-2am moved the findings-scoping + per-check itemization helpers to cli/kb-lint-summary.ts, -135 lines earlier in the file; same function, same guard, unchanged. Further remap from 1097 — forge-3oq added the `./studio-provenance.ts` import, an `origin?:string` KbWithCounts field, a `.map(provenanceOfOrigin(...))` in the GET list route, and an `origin:` doc comment on the POST create write, +15 lines cumulative earlier in the file; same function, same guard, unchanged. Further remap from 1112 — forge-3oq review relocated loadKbDescriptors (byte-identical body) further down the file (net +14 lines cumulative: -53 removed at its old spot, +67 re-added — including a new justification comment — at its new spot next to KB_SEEDING_ANCHOR_PREFIX); same function, same guard, unchanged.)' },
  { file: 'cli/bridge-studio-kbs.ts', line: 1046, sink: 'mkdirSync',
    reason: 'CREATE-LITERAL-SUBDIR: kbDir = kbGuard.realPath (resolveGuardedPath) and the route 409s if kbGuard.exists, so this runs only create-mode on a FRESH dir; the appended leaf `themes` is a literal — a just-created dir cannot host a pre-planted symlink. (Line-drift remap from 1157 — forge-2am, same cause as the row above. Further remap from 1022 — forge-3oq, same cause as the row above, +10 lines. Further remap from 1032 — forge-3oq review, same loadKbDescriptors relocation as the DELETE-route row above, +14 lines cumulative.)' },
  { file: 'cli/bridge-studio-kbs.ts', line: 1047, sink: 'mkdirSync',
    reason: 'CREATE-LITERAL-SUBDIR: as line 1046, literal `_raw` subdir under the freshly-created guarded kbDir. (Line-drift remap from 1158 — forge-2am. Further remap from 1023 — forge-3oq, same cause as the row above. Further remap from 1033 — forge-3oq review, same relocation as the row above, +14 lines cumulative.)' },
  { file: 'cli/ui-bridge.ts', line: 1834, sink: 'mkdirSync',
    reason: 'RETAIN (isSafeRunId-gated logdir-create): runId = `_agent-${slug}-${newRunStamp()}` with slug URL-derived (newly tainted), but isSafeRunId(runId) (SAFE_RUN_ID_RE + explicit .. check) THROWS at line 1781 BEFORE this recursive mkdir of the run\'s own log dir under trusted ctx.logsRoot — the SAME deliberate guard-symmetry check its already-allowlisted siblings at 1992/2393 carry. (Line-drift remap from 1832 — R4-21 WI-2, +2 lines total, same cause as the cli/ui-bridge.ts rows above.)' },

...
export const ALLOWLIST = [
  ...
  { file: 'packages/agents/agent-run.ts', line: 867, sink: 'existsSync',
    reason: 'CLI-ARG + BOOL-PROBE: findSessionProject(sessionId) — sessionId is a `forge <verb>` CLI argument (operator trust boundary), NOT an HTTP request; both existsSync calls are boolean status.json/PLAN.md probes under readdir-enumerated projects/*, no bytes read/written through the path. (Line-drift remap from 695 — SEC-07 added the cmdAgentDispatch --project resolveGuardedPath segment-guard, the isSafeRunId import, and the findSessionProject defensive bound earlier in the file, +23 lines; same function, same probe, byte-for-byte unchanged — verified by sed -n "718p" cli/agent-run.ts.) (Line-drift remap to 725 -- W7-FIX-A2 (W7A2-01): writeSessionTerminalPhase now rides guardedWriteSessionStatus (sticky-cancel seam) — a new import line + the seam comment block earlier in the file, +7 lines; same function, same probe, unchanged -- re-verified via a direct `node scripts/check-raw-fs-guarded.mjs --json` run.) (Line-drift remap from 838 -- W8-A2 (ON-7): cli/ui-bridge.ts gained servedFileHeaders (7 served-file routes hardened), the four bespoke session-list routes wired to deriveSessionLifecycleFor, the sessionStaleMs helper, and the standalone-run stalled derivation; cli/agent-run.ts gained the turn-throw catch. Same function, same sink expression, byte-for-byte unchanged. Re-paired by SINK IDENTITY (file+sink+path-expression), not arithmetic: base-vs-current runLint with an EMPTY allowlist reports 70 residual sinks on BOTH sides, ZERO new and ZERO removed identities, so all 65 rows matched 1:1.) (M4-agents 5.38 line-drift remap from 928 -- `cmdAgentDispatch` gained the signal guard and lost the inline terminal-marker emit to packages/agents/dispatch-terminal.ts, net -2 lines above this sink. Re-paired by SINK IDENTITY, never by arithmetic: same enclosing function `findSessionProject`, same single line carrying BOTH `existsSync(join(sessionDir, ...))` probes, byte-identical to `git show HEAD:packages/agents/agent-run.ts` line 928 -- diffed, not assumed.) (M4-sessions s3 line-drift remap from 926 -- the project-brain port deleted this file\'s `project-brain` AGENT_RUNNERS row and added the SESSION_KIND_RUNNERS lookup + knownAgentIds, net -5 lines above this sink. Re-paired by SINK IDENTITY, never by arithmetic: same enclosing function `findSessionProject`, same single line carrying BOTH `existsSync(join(sessionDir, ...))` probes, verified byte-identical against `git show HEAD:packages/agents/agent-run.ts` line 926 -- diffed, not assumed.) (M4-sessions s3 line-drift remap from 921 -- porting instructions and demo-builder moved their two AGENT_RUNNERS rows into `@forge/sessions/kinds/registry.ts` and dropped their two imports, net -35 lines above this sink. Re-paired by SINK IDENTITY, never by arithmetic: same enclosing function `findSessionProject`, same single line carrying BOTH `existsSync(join(sessionDir, ...))` probes, verified byte-identical against `git show parsoFish/main:packages/agents/agent-run.ts` line 921 -- diffed, not assumed.) (M4-sessions s3 port 3 line-drift remap from 886 -- architect became the `architect` KIND module and the last AGENT_RUNNERS row moved to `@forge/sessions/kinds/registry.ts`. Re-paired by SINK IDENTITY, never arithmetic: same enclosing function `findSessionProject`, source line verified byte-identical against `git show parsoFish/main:packages/agents/agent-run.ts` line 886 -- diffed, not assumed.)' },
  ...
  { file: 'packages/projects/project-contract-scaffold.ts', line: 265, sink: 'existsSync',
  ...
  { file: 'packages/projects/project-contract-scaffold.ts', line: 319, sink: 'existsSync',
  ...
  { file: 'packages/projects/project-contract-scaffold.ts', line: 468, sink: 'existsSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 451, sink: 'existsSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 923, sink: 'existsSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 1623, sink: 'existsSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 1626, sink: 'readdirSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 3099, sink: 'mkdirSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 3100, sink: 'openSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 3398, sink: 'openSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 3397, sink: 'mkdirSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 622, sink: 'mkdirSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 623, sink: 'writeFileSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 631, sink: 'writeFileSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 674, sink: 'mkdirSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 675, sink: 'writeFileSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 691, sink: 'writeFileSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 166, sink: 'mkdirSync',
  ...
  { file: 'packages/sessions/bridge-studio-kickoff.ts', line: 372, sink: 'mkdirSync',
  ...
  { file: 'packages/sessions/bridge-studio-demo.ts', line: 469, sink: 'existsSync',
  ...
  { file: 'cli/ui-bridge.ts', line: 2717, sink: 'mkdirSync',

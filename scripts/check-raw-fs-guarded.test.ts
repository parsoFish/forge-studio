...
test('I5 (RED, bead forge-8vfn.5.63): a destructured declaration with a LATER default does not blind findBinding to an EARLIER, defaultless name', () => {
  // Kills: findBinding's old `[^=;]*` char-class scan of the pattern, which
  // excluded `=` ENTIRELY — so a later property's default value (`env =
  // process.env`) made an earlier, plain `id` read as UNRESOLVED (the value
  // "came from the caller"), same as an unbound function parameter. `id` is
  // not itself in the curated bare-taint list, so an unresolved `id` silently
  // resolves to NOT-tainted and the finding below vanishes — a false
  // NEGATIVE, measured live on hook-runtime.ts's `{ ..., parentEnv =
  // process.env, timeoutMs = HOOK_SPAWN_TIMEOUT_MS } = input` (there via
  // `initiativeId`, which IS on the bare list, so it mis-fired the OTHER way
  // — tainted via the bare fallback instead of via `body` — masking the real
  // reason with a coincidentally-similar verdict).
  const shapes = {

 ...
 *
 * Register row "lanes.test.ts:446": registering FIRST cuts the opposite way for argv/env —
 * `launch` can confirm (roster row visible) before THIS shell has reached the `printf`/
 * `printenv` lines below it, under the same contention. That race is not fixed here: it
 * belongs to the READER (`argvOf`/`envOf` in lanes.test.ts, which now poll for the file
 * rather than assume `launch`'s success implies every later line of this script has run).
 *

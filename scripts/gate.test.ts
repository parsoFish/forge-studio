...
// ----------------------------------------------------------- forge-s9g1
/**
 * `lock_holder_pids` MISREAD A BLOCKED WAITER, and the garbage reached
 * production output twice:
 *
 *     suite-lock: WAITING on stranger pid(s) 3048432 WRITE — another lane's suite holds it
 *
 * A holder row is `6: FLOCK ADVISORY WRITE 784079 08:30:<ino> 0 EOF`, where `$5`
 * is the pid. A blocked waiter is a CONTINUATION row — `2: -> FLOCK ADVISORY
 * WRITE 1677053 …` — and the `->` shifts every field by one, so `$5` is the
 * literal `WRITE`. It was cosmetic only because `is_ancestor` compares that
 * string against numeric pids and never matches; it stops being cosmetic the
 * moment the list is read as pids, and it is NOT cosmetic even now once the
 * pids are right, because a blocked waiter that happens to be the gate's own
 * ancestor would classify ANCESTOR for a lock nobody holds.
 *
 * THESE DOORS NEED A SEAM AND SAY WHY: `/proc/locks` cannot be made to hold a
 * chosen row, so the two-row case the bug lives in is unreachable without one.
 * `FORGE_PROC_LOCKS` points the parser at a fixture and `--lock-state` reaches
 * the classifier without running a suite — the same bargain `lock-guard.mjs`
 * made with `procRoot` for its twelve doors.
 */
function lockState(locksBody: string): { out: string; status: number | null } {
...
describe('forge-s9g1 — a blocked waiter is not a holder, and position was never the property', () => {
  test('holder + blocked waiter on ONE inode returns exactly one pid, and it is numeric', () => {
    const { out } = lockState(
      '1: FLOCK  ADVISORY  WRITE 784079 08:30:<INO> 0 EOF\n' +
      '2: -> FLOCK  ADVISORY  WRITE 1677053 08:30:<INO> 0 EOF\n',
    );
    const pids = out.replace(/^STRANGER:/, '').split(/\s+/).filter(Boolean);
    assert.equal(pids.length, 1, `exactly one pid, got ${JSON.stringify(out)}`);
    assert.match(pids[0], /^\d+$/, `the pid must be numeric — the bug printed the literal WRITE: ${out}`);
  });
  ...
  test('a waiter with NO holder is FREE — nobody holds it', () => {
    // The case that would misclassify once the pids are right: a continuation
    // row alone means the lock was released and somebody is queued for it.
    const { out } = lockState('2: -> FLOCK  ADVISORY  WRITE 1677053 08:30:<INO> 0 EOF\n');
    assert.equal(out, 'FREE');
  });

  test('another inode\'s holder is not this lock\'s', () => {
    // The old parser matched `":$ino "` anywhere in the line, so a start/end
    // offset could collide. This keys off the MAJ:MIN:INODE token itself.
    const { out } = lockState('3: FLOCK  ADVISORY  WRITE 999999 08:30:99999999 0 EOF\n');
    assert.equal(out, 'FREE');
  });
});

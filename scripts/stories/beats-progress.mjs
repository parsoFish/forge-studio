...
export function progressExpiry(s) {
  ...
  if (s.source === 'ambiguous') {
    return (
      `ambiguous-progress-key: \`${s.key}\` is carried by ${s.carriers} elements at once, so there is no single ` +
  ...
  if (s.firstSeenAt === null) {
    return (
      `no-progress-key: this beat declared \`perTransition: ${s.perTransition}\` against \`${s.key}\`, and that ` +
  ...
  if (s.source === 'absent') {
    return (
      `progress-key-vanished: \`${s.key}\` appeared ${Math.round((s.firstSeenAt - s.startedAt) / 1000)}s into this ` +
  ...
  return (
    `stalled-no-transition: \`${s.key}\` first appeared ${Math.round((s.firstSeenAt - s.startedAt) / 1000)}s into ` +

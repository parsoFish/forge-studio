...
/**
 * One ground's manifest, `null` when genuinely absent, or
 * {@link GROUND_MANIFEST_UNKNOWN} when the read failed for any other reason.
 *
 ...
 *
 * @param {string} dir
 * @returns {{digest: string, files: Map<string,string>}|null|typeof GROUND_MANIFEST_UNKNOWN}
 */
export function groundManifest(dir) {
  ...
  try {
    ...
    if (e?.code === 'ENOENT') return null; // genuinely absent — nothing to compare
    return GROUND_MANIFEST_UNKNOWN; // a real read failure, never folded into "absent"
  }
  const files = new Map();
 ...
 *
 * @param {{files: Map<string,string>}|null|typeof GROUND_MANIFEST_UNKNOWN} before
 * @param {{files: Map<string,string>}|null|typeof GROUND_MANIFEST_UNKNOWN} after
 */
export function groundChanges(before, after) {
  // ROW 102b/8 — UNKNOWN must never render as unchanged (both `?? new Map()`)
  // nor as a flood of false added/removed. ONE named, undeclared path: nothing
  // minted/declared/ignored can match it, so `classifyOwnGroundDrift` reds it.
  if (before === GROUND_MANIFEST_UNKNOWN || after === GROUND_MANIFEST_UNKNOWN) {
    return { added: ['<GROUND STATE UNKNOWN — the ground could not be hashed at this checkpoint>'], removed: [], modified: [] };
  }
...
export function siblingGroundEscapes(project, baseline, opts = {}) {
  ...
  for (const [dir, before] of baseline) {
    ...
    // ROW 102b/8 — a digest read off the UNKNOWN symbol is `undefined` on
    // both sides, so `=== ` would silently pass a sibling never actually
    // checked. Reported explicitly instead.
    const unknown = before === GROUND_MANIFEST_UNKNOWN || after === GROUND_MANIFEST_UNKNOWN;
    if (!unknown && (before?.digest ?? null) === (after?.digest ?? null)) continue;
    out.push({
      root: dir,
      before: before === GROUND_MANIFEST_UNKNOWN ? 'UNKNOWN' : (before?.digest ?? null),
      after: after === GROUND_MANIFEST_UNKNOWN ? 'UNKNOWN' : (after?.digest ?? null),

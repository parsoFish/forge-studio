...
export function renderDevUserPrompt(input: DevUserPromptInput): string {
  ...
  return [
    ...
    '',
    '1. **Check `.forge/last-gate-failure.md` FIRST if it exists** — it is the authoritative output of forge\'s **live** quality gate from your last attempt (the same gate that decides done vs failed). Fix EXACTLY what it reports. Your own `make test`/offline gate run can show a **false pass** — live acceptance tests silently skip without `TF_ACC` and print `ok ... 0.00s`, which is NOT a pass — so trust that file over your own run; the work item is NOT done until it is gone. Then **orient on prior progress:** run `git log --oneline main..HEAD` + `git diff --stat main..HEAD` to see what is already built, and read `AGENT.md` (what has been tried) and `fix_plan.md` (the checklist). Don\'t re-research anything `AGENT.md` already records.',

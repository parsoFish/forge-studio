...
export function renderDevUserPrompt(input: DevUserPromptInput): string {
  ...
  return [
    ...
    '',
    '1. **Orient on prior progress first:** run `git log --oneline main..HEAD` + `git diff --stat main..HEAD` to see what is already built, then read `AGENT.md` (what has been tried) and `fix_plan.md` (the checklist). Don\'t re-research anything `AGENT.md` already records.',
    ...
    '4. Run the project\'s quality gates with `Bash`. Don\'t claim a pass without running them.',

...
export function buildDemoScriptSystemPrompt(_brainCwd?: string): string {
  return [
    ...
    '  detecting versions.',
    '- Tolerance is mandatory: every checkpoint screenshots unconditionally;',
    '  an interaction that targets a changed-tree-only element must be wrapped',
    '  so the baseline run still captures the prior screen and continues.',
...
export function renderDemoScriptUserPrompt(input: DemoScriptUserPromptInput): string {
  return [
    ...
    '',
    'Every `demo-manifest.json` checkpoint `label` MUST equal a',
    '`${DEMO_SCREENSHOT_DIR}/<label>.png` your spec writes. Keep it tight:',
    'the single behavioural delta that is the essence of this initiative,',
    'framed as prior behaviour → new behaviour. The baseline is a valid',
    'working state, never an error.',
  ].join('\n');
}

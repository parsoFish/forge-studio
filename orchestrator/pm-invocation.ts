...
export function renderPmUserPrompt(input: PmUserPromptInput): string {
  ...
  return [
    ...
    '- One or more `brain/cycles/themes/*.md` covering work-item sizing, file-scope discipline, and feature-parallelism inheritance — these are the load-bearing PM-discipline patterns.',
    ...
    '',
    "**CI / build / quality initiatives — the gate MUST BE the project's CI, never a proxy.** If a work item's acceptance criteria require the project's full CI or build to pass (e.g. a 'make CI green', 'fix the failing build', or quality initiative — its `then` clause is \"CI/`make test`/the build passes\"), and `.forge/project.json` declares a **`ci_gate`**, set that WI's `quality_gate_cmd` to the project's `ci_gate` **verbatim**. Do NOT substitute a narrower proxy (a lint-only command, or a single-package test) — a narrow proxy passes forge's gate while the real CI stays red, which is exactly the gate-escape that let a 'make CI green' WI ship with a still-failing CI check. The whole-CI gate legitimately fails at iter-0 (that's the point — it proves green only once the agent has actually fixed everything CI flags) while the project's normal baseline gate stays green, so this does not trip the baseline check.",
    '',
    ...
    `- Decompose the initiative into the work items it actually needs. Forge handles 1→N WIs — choose the shape that matches the work. For sizing reference, **brain-query for past successful work items in this project** (or, if this is the project's first cycle, in similar projects). The brain themes under \`projects/${input.projectName}/brain/themes/\` and \`brain/cycles/themes/\` are the source of truth for what shape lands; they accumulate over time as cycles complete.`,

...
export function renderUnifierUserPrompt(input: UnifierUserPromptInput): string {
  ...
  return [
    ...
    '',
    '## ⚠ WRITE-FIRST DISCIPLINE (post-2026-05-25 cycle-7 audit)',
    '',
    'On EACH iteration, the FIRST tool calls you make MUST include AT LEAST',
    'ONE `Write` (or `Edit`) targeting either:',
    '- `demo/' + input.initiativeId + '/DEMO.md`, OR',
    '- `.forge/pr-description.md`.',
    '',
    'Reading + planning without writing is the failure mode that wedges the',
    'unifier (observed 4+ times: 10 iters of `ls` + `git log` + `cat` + 0',
    'writes, then iteration-budget terminal-fail). **A draft is enough** —',
    'write a skeleton DEMO.md / pr-description.md in iteration 1 BEFORE',
    'investigating, then refine in subsequent iterations. The worst draft',
    'is infinitely better than no draft when the iteration budget is',
    'finite. You can always overwrite in the next iteration.',
    '',
    '## Inputs',
    '',

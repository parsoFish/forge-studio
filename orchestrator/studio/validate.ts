...
// ---------------------------------------------------------------------------
// validateConnections (R3-04-F1/F4, `_wave5/specs/R3-04.md` D5/D6/D8/D14/D15)
//
...
// validateCatalog.
//
// Six rules, each independently an ERROR (never a flag — every one of these
// is a real operational hazard: an unpinned install is non-reproducible, a
// missing probe means readiness can never be executed, a bad env-var name is
// silently never satisfiable, zero capabilities reads as a fabricated "no
...
function validateConnectionEntry(entry: CatalogConnectionEntry, kind: 'tool' | 'mcp'): Finding[] {
  const findings: Finding[] = [];
...
export function validateConnections(c: Catalog): Finding[] {
  return [

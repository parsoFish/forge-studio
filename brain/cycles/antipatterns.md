...
- [`silent-auto-discover-fallback-blast-radius`](./themes/silent-auto-discover-fallback-blast-radius.md) — A lookup helper that silently auto-discovers and mutates any matching live resource when its expected target is missing must fail loud instead; an unattended agent under gate pressure won't question why the fallback found something.
...
- [`declared-data-fails-open`](./themes/declared-data-fails-open.md) — a field/status/flag that is declared, parsed and surfaced but enforced NOWHERE: the campaign's #1 recurring defect class across waves 4-5 (trigger `projects:` scoping passed by none of its four dispatch call sites; `parseRun` silently dropping `trigger`, then `reflectionLost`/`reflectionLostNote`; a client validator discarding every non-empty server response). Enforce end-to-end, mirror the lint on the same evidence the dispatch reads, and never let a declared path carry less friction than an undeclared one.
- [`suppression-env-fakes-the-pass`](./themes/suppression-env-fakes-the-pass.md) — the environment decides the result: a gate that doesn't export CI's env is not the same gate; tests that inherit a precondition instead of establishing it; a wrapper's exit read as the job's; a library module with no CLI entry exiting 0 with no output. Acceptance is environment-INDEPENDENCE (green both ways), never "green now".
- [`journey-never-gate-on-async-lagging-ui-display`](./themes/journey-never-gate-on-async-lagging-ui-display.md) — a journey assertion on a value that reaches the DOM only after an async UI refresh flakes on render lag, not the property under test — assert the robust terminal state + an API-pinned data transition; the two-reopen-stop fires when the class reopens under a new symptom (R6-08 kb-maintain inverted on two check names).
- [`2026-08-22-progressive-disclosure-breaks-link-crawlers`](./themes/2026-08-22-progressive-disclosure-breaks-link-crawlers.md) — collapse/paging/lazy-loading is a coverage regression for any harness that walks anchors: W7-B5's `pageSize={15}` on the history ledger cut the walkthrough crawl 924 → 657 routes and the coverage floor read it as a Studio regression. The lost coverage was HIDING two live defects. Fix belongs in the harness (expand disclosure before harvesting, same PR as the UI change) — and check the fix moved the number: forcing the RunRail's collapsed group open via localStorage expanded it 2 → 60 cards and bought ZERO routes, because rail cards are `<div onClick>`, not anchors.
- [`2026-08-28-scope-filters-reship-the-class-a-sound-audit-catches`](./themes/2026-08-28-scope-filters-reship-the-class-a-sound-audit-catches.md) — a scope filter wrapped around a sound audit re-ships the class the audit would catch: wave 8's drain edit-soundness audit held under every shape tried, but an edit-class filter and a one-directory snapshot each alone let forge-d8l recur a fourth time through the instrument built to catch it. Verify the wrapper (every caller, every narrowing parameter), not the core; derive scope, never accept it.
- [`2026-08-28-a-pin-that-reads-the-checkouts-git-history-is-environment-shaped`](./themes/2026-08-28-a-pin-that-reads-the-checkouts-git-history-is-environment-shaped.md) — a test that derives its fixture from the repository's real git history is green on every full-history worktree and red on CI's depth-1 clone (the oldest reachable commit IS HEAD). Build the repo the test needs; treat a CI-only red as a finding about the test's shape first — and ask whether shallow breaks the CLI too (it did, twice).

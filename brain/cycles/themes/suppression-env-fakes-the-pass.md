...
- [`packages/agents/tests/integration/run-agent-ceiling.test.ts`](../../../packages/agents/tests/integration/run-agent-ceiling.test.ts) — the `withoutSpawnSuppressionEnv()` pattern, applied 15 times.
- [`apps/forge/tests/integration/ui-bridge-agent-run-ceiling.test.ts`](../../../apps/forge/tests/integration/ui-bridge-agent-run-ceiling.test.ts) — the sibling that inherited the precondition and could not pass in CI.
- [`apps/forge/studio-lint.ts`](../../../apps/forge/studio-lint.ts) — a library module with no CLI entry; invoked directly it exits 0 with no output.

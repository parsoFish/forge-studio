...
|---|---|---|
| Never carried from the producer | R2-08-F1 `projects:` trigger scoping. Four production `stageFlowRunRequest(` sites — `orchestrator/flow-trigger.ts`, `orchestrator/flow-runner.ts`, `orchestrator/cron-triggers.ts`, `cli/bridge-hooks.ts` — and *"not one passes `projects` or `eventProject`"*, so the drain's guard never ran outside tests. An operator could declare `projects: [gitpulse]`, `forge studio lint` would confirm the id, and the trigger fired for every project: *"parsed, lint-validated, surfaced, enforced nowhere"*. The sibling `concurrency` was already carried in the same file. | L6836–6850 |
...
- [`forge-ui/lib/studio-client.ts`](../../../forge-ui/lib/studio-client.ts) — `parseRun`, the last client hop where `trigger` and `reflectionLost` were dropped.
- [`forge-ui/lib/agent-ledger.ts`](../../../forge-ui/lib/agent-ledger.ts) — the R6-06 row validator and `deriveAgentLedgerRows`.

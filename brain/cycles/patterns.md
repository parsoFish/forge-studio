...
- [`new-session-kind-needs-ui-wiring`](./themes/new-session-kind-needs-ui-wiring.md) — a new OOTB agent/session kind needs three layers — additive definitions (agent + descriptor + artifact), UI wiring (launcher + KindSummary panel + finalize action), and a runtime runner; "the generic R2-10 shell handles it" covers only the session page, route-works is not feature-works (R4-21 was backend-green across 4339 tests yet unreachable).
- [`per-kb-health-honesty-na-invariant`](./themes/per-kb-health-honesty-na-invariant.md) — a per-scope health status from a check that only inspected some scopes must show 'n/a' for scopes it never looked at, never a fabricated 'pass'; drive per-check status off a registry-derived CHECK_NAMES to kill drift (R6-08 project-KB checks reported pass while scanning only cycles/forge-dev).
- [`generalise-plumbing-not-composition`](./themes/generalise-plumbing-not-composition.md) — a generic spine dissolves what implementations SHARE (plumbing: containment preamble, spec derivation, telemetry, dispatch), not what makes each one itself (per-kind prompt/state composition, usually the majority of the lines); before scheduling "migrate N onto the primitive", drive the primitive against ONE implementation's golden-capture scenario and let the byte-diff size the initiative — batch E killed a 4-migration feature by measurement in hours, zero fixture edits (R4-22-F4 → R4-23).
- [`derive-status-dont-store-it`](./themes/derive-status-dont-store-it.md) — the constructive cure for declared-data-fails-open: a read-only aggregate (dashboard/card/health strip) computes each object's status from the source-of-truth run model and gives the object type NO status field of its own, so a wrong impl has nowhere to read a stale value from; reuse the owning surface's derivation (byte-match, don't copy) and add no new polling path (R6-07 Home dashboard, batch F).
- [`expression-pinned-allowlist`](./themes/expression-pinned-allowlist.md) — a security-ratchet allowlist row keyed by file+line/count cannot express "this is the expression I audited": a same-count substitution passes silently and every edit above the site forces a resync; key the row by the audited expression text — survives drift, rejects substitutes, residual failures are DX-shaped extra findings, never silent suppression (SEC-H, batch H, PR #131).
- [`2026-08-22-verifier-without-a-repro-produces-agreement`](./themes/2026-08-22-verifier-without-a-repro-produces-agreement.md) — "try to refute it" is satisfiable by reading source and agreeing: wave 7's ten cluster verifiers returned 146 verdicts and refuted ONE. A second hostile pass whose brief made the ARTIFACT the gate (default position "the finding is wrong"; CONFIRMED needs a script you wrote, ran and paste the output of, plus a control probe and a `git log -S`-proven attribution; NOT-REPRODUCED explicitly welcome) confirmed 5 of 14 and DOWNGRADED 9, every downgrade backed by a repro showing a real workaround. Watch the refute rate — a pass that refutes almost nothing has agreed, not verified.

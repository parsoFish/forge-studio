...
- **Brain 1 (forge-dev):** forge code + ADRs + engineering notes (decision/reference themes) at `brain/forge-dev/` (**13 theme pages**).
- **Brain 2 (cycles):** cycle-derived patterns, antipatterns, operations, raw archives at `brain/cycles/` (**72 theme pages**, **39 raw cycle archives**).
- **Brain 3 (per project):** project brain lives inside each project repo at `projects/<name>/brain/` (**40 total project-level theme pages** across 3 active project(s)).
...
- [mdtoc](../projects/mdtoc/forge/brain/profile.md) — > The project's knowledge base, read by planners and reflectors through the
- [terraform-provider-betterado](../projects/terraform-provider-betterado/forge/brain/profile.md) — > The PM/architect reads this first when planning an initiative.

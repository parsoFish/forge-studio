...
**Architecture shift:** the prior closure shipped review stage 1 as a one-shot SDK call. Per the user's reframe, stage 1 + stage 2 collapse into a single review-Ralph runner — same generic `packages/agents/ralph/runner.ts` that the dev-loop uses, parameterised by:
...
- `146cf5c feat(grading): parametric sweep harness + locked design-frontier baselines` — `projects/trafficGame/scripts/grading/runSweep.mjs` library + 8 per-theory sweep scripts + `capture-notable.mjs` + `docs/baselines/grading-frontier-*.md` + `docs/baselines/screenshots/`.

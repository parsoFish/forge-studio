...
the one source of truth but never states the derivation rule in those words: `run-model.ts`
declares the shapes, `run-model-derive.ts` computes every one from events. `Run.trigger` states it
verbatim — "derived and never stored/authored … NEVER a fabricated default"
(`orchestrator/run-model.ts:129-133`) — and the run-detail timeline derives its rows from the
**flow definition**, since events alone "would silently vanish" the nodes that never ran
...
user-visible badges; the DOM contract now pins the survivor, `data-phase-cost-usd` **exactly**
`run.phaseMeta[node].costUsd`, "never re-summed from events" (`docs/forge-ui-dom-and-harness.md:524-528`),
via the single derivation `sumAuthoritativeCostUsd` (`orchestrator/event-cost.ts:54`).
...
`review-loop→review` all FALSE) — conforming exactly to those ATs would have shipped the feature
non-functional for dev/pm/reflect/review **with a green suite**. Amended: `RunPhaseMeta.lastEventAt`
derives in `computeLastEventAt` (`run-model-derive.ts:262`) over every event the authoritative
`eventToNodeId` (`run-model-derive.ts:817`) attributes; the client collapsed to
`phaseLogRefreshSignal(run, nodeId)`. The AT also asserts the phantom's absence
(`phaseMeta['developer-loop'] === undefined`), not just the hit.
...
(`orchestrator/run-model.ts:80-89`) — N rows must not mean N HTTP calls to an unguarded path sink.
...
acceptance weight lands in the `npm test` CI home the derivation lives in, not a browser-only unit
test. A second derivation is a defect even when both are correct today: only one gets fixed when the schema moves.

## The bounded-memoization corollary

[ADR-044](../../../docs/decisions/044-read-path-memoization.md) narrows this posture, not reverses
it: a read-path cache is still exactly this same single derivation wrapped in an mtime+size-keyed
memo — never a second derivation, never a persisted artifact. `_queue/done/` grows unbounded and a
...
derived value into a manifest/status file and reading it back, or deriving the same fact twice. A
memo lives only in the serving process and fails open to this same derivation on any doubt.

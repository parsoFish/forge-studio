...
- [`orchestrator-owned-execution-beats-heuristic-verification`](./themes/orchestrator-owned-execution-beats-heuristic-verification.md) — When the evidence itself (not just pass/fail) is agent-produced, forensic checks against it are an arms race the verifier loses; move the evidence-producing run into the orchestrator and hand the agent a read-only, pre-verified artifact (ADR 036).
- [`env-leak-must-be-fixed-at-spawn-seam-not-launcher`](./themes/env-leak-must-be-fixed-at-spawn-seam-not-launcher.md) — Ambient operator-shell env (headroom proxy vars, ANTHROPIC_BASE_URL) leaking into spawned agents must be scrubbed at the shared SDK spawn seam; per-launcher fixes let the same defect class recur on every new launch path.
- [`reflector-completion-requires-artifact-diff`](./themes/reflector-completion-requires-artifact-diff.md) — An end event's status/output_refs is not proof of persistence; diff should-exist (done/ initiatives) vs does-exist (archived reflections) to catch silent loss.
...
- [`forge-ui-data-attribute-contract-discipline`](./themes/forge-ui-data-attribute-contract-discipline.md) — the maintenance contract behind forge-ui's `data-*` DOM-as-metrics convention: attribute + reference doc + affected journey change in the SAME PR, a beat absent from `RUN_ORDER` never executes, and attribute-projection tests do not prove element identity (a `<div>` shipped where the contract named `main`, past 45 green tests).
- [`derived-never-stored-run-model`](./themes/derived-never-stored-run-model.md) — everything on a run surface is derived from the event log, never stored (ADR-008's posture, stated). Where ONE correct derivation exists, re-deriving it elsewhere is the documented trap: `phaseMeta[node].costUsd` is the single correct per-node cost (two independent re-summations inflated dev-loop cost 2-3x), and `lastEventAt` is derived server-side over the authoritative `eventToNodeId` because the naive `phase === nodeId` is wrong for 8 of 12 phase strings.
- [`adr042-park-recurrence-and-the-generic-primitive`](./themes/adr042-park-recurrence-and-the-generic-primitive.md) — a new orchestrator/ runtime executor (a GATE_KIND row, an interactive *-runner.ts) is ADR-042 ask-first; it parked three times in batch D (R4-18, R4-19-F2 mislabelled, R4-21) — the signal to generalise into the operator-directed generic, operator-authorable interactive-surface primitive (input/output-artifact-like, multi-instance) that dissolves the per-agent-runner cap pressure.
- [`background-work-server-owned-client-observes`](./themes/background-work-server-owned-client-observes.md) — every multi-step operator intent that outlives one request is a server-minted runId backed by on-disk status, reconstructed by GET; the client is a pure poller rendering exactly three states (watching / timed-out-but-running / terminal) — silence is never a state. The identical bounded-poll-with-silent-give-up defect recurred four times (`RunPanel`, `OnboardWithAgent`, KB consolidate, `ContractResolutionPanel`) before W6-B14 closed the class on `pollUntilTerminal` + reattach-on-mount.
